...
- **v7.0 stabilization pass completed (2026-04-15)** — patched `TieredExtractorV70` with a generic non-job title post-filter (`Show N more`, `Working with us`, `/show_more` pagination URLs) to remove Type-1 leakage observed on context fixtures. Local harness on `storage/auto_improve_context/v7_0` held total job volume flat (374→374) while improving filled descriptions (14→327), filled locations (254→349), and reducing obvious non-job titles (2→0). Smoke tests: 6/6 pass.
- **Version-controlled champion/challenger state landed (2026-04-15)** — every auto-improve iteration now commits `database/models_snapshot.sql` (plain SQL, ~32 KB, diff-friendly, NOT Git LFS) plus mirrors of `auto_improve_memory.json` + `auto_improve_history.json`. A fresh clone restores champion/challenger state via `make models-restore` after `make db-migrate`. Two commit paths: (1) trigger-file consumer in the daemon fires within ~30 s of any A/B test completion, (2) end of `run_improvement()` commits source outcome + new challenger code together. Deduplicated by embedding `test_run_id` in commit messages. Full-DB LFS dump (`database/jobharvest_latest.dump`) retained for disaster recovery — serves a different purpose (jobs/companies/leads crash-recovery) vs the new snapshot (model-state portability).
- **Auto-Improve v2 foundation landed (2026-04-15)** — shipped the v6.10 hotfix candidate (`v6.9 + DetailEnricher`), structured site diff packages + rejection post-mortems, AST-based challenger linting, a 15-fixture smoke corpus with a stricter `>=12/15` verifier gate, and the first pass of the evolutionary-search stack under `backend/app/ml/evo/`. The daemon now supports manual `register_existing` triggers for prewritten challengers like v6.10 and can shell out to `python -m scripts.evo_cycle` when `EVO_ENABLED=1`.
...
- **Next queued shipment:** `v6.10` is a deliberately minimal `v6.9 + DetailEnricher` hotfix in [`backend/app/crawlers/tiered_extractor_v610.py`](backend/app/crawlers/tiered_extractor_v610.py). The goal is to isolate the field-completeness lift before broader evolutionary-search changes muddy attribution.
...
- **Prompt signal is now diff-grounded.** `failure_analysis.build_site_diff_package(...)` writes structured champion-vs-baseline-vs-gap JSON per target site, and `memory_store.render_recent_rejections_for_prompt(...)` injects the latest structured lessons directly after the scorecard in the Codex prompt.
- **Broken challengers now fail fast.** `challenger_lint.py` rejects deep inheritance chains, unsafe `extract()` overrides, domain-literal hacks, oversized rewrites, and banned-approach retries before a 30- to 90-minute A/B can even start.
