/**
 * Generic run logs page — used by Site Config Runs, Company Config Runs, Discovery Runs.
 * Shows a paginated history table filtered by crawl_type, with a trigger button.
 */
import { useState } from 'react';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { getCrawlHistory, triggerRun, getQueueStats } from '../../lib/api';
...
interface Props {
  title: string;
  description: string;
  crawlType: string;   // value stored in crawl_logs.crawl_type
  runType: string;     // value sent to POST /crawl/trigger/{run_type}
  icon: React.ElementType;
  iconColor: string;
}

export function RunLogsPage({ title, description, crawlType, runType, icon: Icon, iconColor }: Props) {
  ...
  const { data: history, isLoading } = useQuery<{ items: Record<string, unknown>[]; total: number }>({
    queryKey: ['run-logs', crawlType, page, statusFilter],
    queryFn: () => getCrawlHistory(page, 50, statusFilter || undefined, crawlType),
  ...
  const queueDepth = queueStats?.[runType]?.pending ?? null;

  const triggerMut = useMutation({
    mutationFn: () => triggerRun(runType),
    onSuccess: () => {
      setTimeout(() => qc.invalidateQueries({ queryKey: ['run-logs', crawlType] }), 1500);
    },
  });
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        >
          {triggerMut.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />}
          Run Now
        </button>
      </div>
      ...
      {triggerMut.isSuccess && (
        <div className="rounded-lg bg-green-50 border border-green-200 px-4 py-3 text-sm text-green-700">
          Run triggered successfully — it will appear in the log below shortly.
        </div>
      )}
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="card p-4 flex items-center gap-3">
          ...
          <div>
            <div className="text-xl font-bold text-gray-900">{successCount}</div>
            <div className="text-xs text-gray-500">Success (page)</div>
          </div>
        </div>
        <div className="card p-4 flex items-center gap-3">
          <Loader2 className="w-5 h-5 text-blue-500 flex-shrink-0" />
          <div>
            <div className="text-xl font-bold text-gray-900">{runningCount}</div>
        ...
        <div className="card p-4 flex items-center gap-3">
          ...
          <div>
            <div className="text-xl font-bold text-gray-900">{failedCount}</div>
            <div className="text-xs text-gray-500">Failed (page)</div>
          </div>
        </div>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  {['Company', 'Status', 'Started', 'Duration', 'Error'].map(h => (
