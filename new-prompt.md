...
3. **Core fields populated from actual page data** (not inferred/guessed):
   ...
   - `description` — **clean, human-readable text** (see Description Quality below)

4. **Bonus fields** (when present on the page):
   - `salary_raw` — exactly as stated (never inferred)
   - `employment_type` — Full-Time, Part-Time, Contract, etc.

### Description Quality — The Hidden Gap

A job description is not just "any text from the page." Compare these two extractions of the same job:
...
**Good (Jobstream baseline):**
```
We are looking for a Senior Software Engineer to join our team.

Requirements:
- 5+ years experience in Python or Java
- Strong understanding of distributed systems
- Experience with cloud platforms (AWS/GCP/Azure)
```

The baseline version is clean, readable, properly formatted. The model version is full of `\t`, `\n`, excess whitespace, HTML artifacts, and garbled formatting. **This is a Type 4 error** and it matters — downstream consumers (search indexes, LLMs, job boards) need clean text.

When extracting job descriptions, the model MUST:

1. **Strip HTML properly** — convert to clean plaintext or markdown. No raw tags, entities (`&amp;`, `&nbsp;`), or CDATA remnants.
2. **Normalize whitespace** — collapse runs of `\t`, `\n`, spaces into single spaces or clean line breaks. No leading/trailing whitespace per line.
3. **Preserve meaningful structure** — paragraph breaks, bullet lists, numbered lists should be converted to clean text equivalents (newlines, `- ` prefixes), not flattened into a wall of text.
4. **Remove boilerplate** — cookie notices, nav text, footer content, "Apply Now" buttons, social sharing links that get swept into the description.
5. **Extract from the right element** — the description should come from the job detail content area, not from the entire page body. The Jobstream baseline uses specific selectors (e.g. `details_page_description_paths`) to target the exact description container.

When comparing model output vs Jobstream baseline for the SAME job, ask:
- Is the model's description **as readable** as the baseline's? If not, why?
- Does it contain **garbage characters** (excess tabs, carriage returns, encoded entities)?
- Is it **complete** (full description) or **truncated/partial**?
- Does it include **irrelevant page content** (nav, footer, sidebar) that the baseline cleanly excludes?
- What selector or extraction method would give us **cleaner text**?

This is an area where general improvements have huge impact — better HTML-to-text conversion, smarter description container detection, and whitespace normalization will improve quality across ALL sites, not just one.
...
| **Type 4 (Description Noise)** | Job extracted with garbled/unreadable description | Tabs, excess newlines, HTML artifacts, boilerplate | **Medium** — degrades downstream usability |

**Type 1 errors are the worst.** A system that extracts 10 real jobs is better than one that extracts 15 where 5 are garbage. Quality over quantity, always. But Type 4 errors are pervasive and cumulative — fixing description quality across the board is a high-impact general improvement.
...
- **Description quality**: Compare model vs baseline descriptions side-by-side. Is the model's description clean, readable text? Or is it full of `\t`, `\n`, HTML artifacts, excess whitespace, boilerplate? The Jobstream baseline typically produces clean, well-formatted descriptions — your model should match that quality.
...
| **Description noise** | Descriptions have tabs, newlines, HTML artifacts, boilerplate vs clean baseline | Improve HTML-to-text conversion, description container targeting, whitespace normalization |
...
### Step 7: Validate with Quick Tests BEFORE Deploying

**CRITICAL: Always run quick tests before declaring your implementation done.**
The A/B test takes 10-30 minutes and is expensive. Catch bugs NOW with pytest (takes seconds):

```bash
# Run smoke tests — validates import, inheritance, extraction, title quality
cd /path/to/jobharvest
python -m pytest backend/tests/test_extractor_smoke.py -v --tb=short

# Quick manual test against a context HTML file
python3 -c "
import sys; sys.path.insert(0, 'backend')
from app.crawlers.tiered_extractor_v{VER} import TieredExtractorV{VER}
ext = TieredExtractorV{VER}()

# Test against a failure/gap HTML file from context dir
with open('storage/auto_improve_context/v{VER_DIR}/failure_1_example.html') as f:
    html = f.read()
...
for j in jobs[:3]:
    print(f'  {j.get(\"title\", \"?\")} | loc={j.get(\"location_raw\", \"\")} | desc={len(j.get(\"description\", \"\"))} chars')
"
```

**What to check:**
- Does the module import? (syntax errors, missing deps)
- Does `extract()` return jobs without crashing?
- Do extracted jobs have **all core fields** (title, source_url, location_raw)?
- Are descriptions clean text (not tabs/newlines/HTML artifacts)?
- Are titles real job names (not nav labels, headings, CMS artifacts)?

If any test fails, fix it BEFORE proceeding. Do NOT deploy broken code and waste an A/B test cycle.

### Step 8: Register and Test

After validating, trigger an A/B test against the champion and Jobstream baseline:
