"""
Career Page Finder v2.7 — direct from v1.6 finder base (CareerPageFinderV4).
...
_CAREER_HREF_PATTERN_V27 = re.compile(
...
_CAREER_TEXT_PATTERN_V27 = re.compile(
...
_REJECT_LINK_TEXT_PATTERN_V27 = re.compile(
...
_REJECT_LINK_HREF_PATTERN_V27 = re.compile(
...
_LISTING_URL_HINT_PATTERN_V27 = re.compile(
...
_JOB_DETAIL_URL_PATTERN_V27 = re.compile(
...
_FEED_URL_PATTERN_V27 = re.compile(r"(?:rss|feed|downloadrssfeed|comments/feed)", re.IGNORECASE)

_LOGIN_PAGE_PATTERN_V27 = re.compile(
    r"(?:sign\s+in\s+to\s+your\s+account|candidate\s+portal\s+login|login\.aspx|mydayforce)",
    re.IGNORECASE,
)

_SCRIPT_URL_PATTERN_V27 = re.compile(
    r"https?://[^\"'\\s]+/(?:jobs/search|job-openings|requisitions?|embed/job_board|candidateexperience[^\"'\\s]*)",
    re.IGNORECASE,
)

_ERROR_PAGE_PATTERN_V27 = re.compile(
...
_ATS_HOST_HINT_PATTERN_V27 = re.compile(
...
_NON_LISTING_SECTION_PATTERN_V27 = re.compile(
    r"/(?:sectors?|services?|insights?|resources?|news|blog|about|team|culture)(?:/|$)",
    re.IGNORECASE,
)

_LOWONGAN_CONTEXT_PATTERN_V27 = re.compile(
...
class CareerPageFinderV27(CareerPageFinderV4):
    """v2.7 finder with stronger recovery from wrong discovery targets."""
    ...
    async def find(self, domain: str, company_name: str = "") -> dict:
        ...
        async with httpx.AsyncClient(
            ...
            derived_candidate = await self._probe_derived_targets_v27(client, domain, company_name, current)
            ...
            ats_candidate = await self._probe_ats_paths_v27(client, domain, company_name, current)
            if ats_candidate:
            ...
            sub_candidate = await self._try_subpage_discovery_v27(client, current)
            if sub_candidate:
            ...
            if current_bad:
                homepage_candidate = await self._homepage_recovery_v27(client, domain)
    ...
    async def _try_subpage_discovery_v27(
        self,
        ...
        for a_el in root.iter("a"):
            ...
            text = self._safe_text_v27(a_el)

            if self._is_rejected_link_v27(href, text):
                continue
            ...
            if _CAREER_HREF_PATTERN_V27.search(href):
                score += 6.0
                reason_parts.append("href")
            if text and _CAREER_TEXT_PATTERN_V27.search(text):
                score += 6.0
                reason_parts.append("text")
            if _LOWONGAN_CONTEXT_PATTERN_V27.search(href) or _LOWONGAN_CONTEXT_PATTERN_V27.search(text):
                score += 4.0
                reason_parts.append("localized_listing")
            if _LISTING_URL_HINT_PATTERN_V27.search(full_url):
                score += 3.0
                reason_parts.append("listing_path")
            if "search=" in full_url or ("?" in full_url and "job" in full_url.lower()):
                score += 2.0
                reason_parts.append("query")
            if _JOB_DETAIL_URL_PATTERN_V27.search(full_url):
                score -= 8.0
                reason_parts.append("detail_penalty")
            if _NON_LISTING_SECTION_PATTERN_V27.search(full_url):
                score -= 6.0
        ...
        for candidate_url, candidate_weight, reason in candidates[:10]:
            ...
            if total_score > best_score:
                ...
                best = {
                    "url": str(resp.url),
                    "method": disc.get("method", "") + f"+subpage_v27:{reason}",
    ...
    async def _probe_derived_targets_v27(
        self,
        client: httpx.AsyncClient,
        domain: str,
        company_name: str,
        ...
        if _JOB_DETAIL_URL_PATTERN_V27.search(current_url) or _FEED_URL_PATTERN_V27.search(current_url):
        ...
        if "oraclecloud.com" in host or "candidateexperience" in html_body.lower():
            site_ids = self._oracle_site_ids_v27(current_url, html_body)
        ...
        for script_url in self._extract_script_urls_v27(current_url, html_body):
            weight = 6.0 if _LISTING_URL_HINT_PATTERN_V27.search(script_url) else 3.0
        ...
        if root is not None:
            for a_el in root.iter("a"):
                ...
                if not _LISTING_URL_HINT_PATTERN_V27.search(full) and not _CAREER_TEXT_PATTERN_V27.search(text):
                    continue
                base_weight = 4.2 if _LISTING_URL_HINT_PATTERN_V27.search(full) else 2.8
                if "job-openings" in full or "requisitions" in full or "Portal.na" in full:
                    base_weight += 1.5
                if _LOWONGAN_CONTEXT_PATTERN_V27.search(full) or _LOWONGAN_CONTEXT_PATTERN_V27.search(text):
                    base_weight += 2.2
                if _NON_LISTING_SECTION_PATTERN_V27.search(full):
                    base_weight -= 2.8
                candidates.append((full, "anchor", base_weight))

        return await self._probe_url_candidates_v27(client, current, candidates, limit=36)

    async def _probe_url_candidates_v27(
        self,
        client: httpx.AsyncClient,
        current: dict,
        candidates: list[tuple[str, str, float]],
        ...
        for candidate_url, reason, weight in ordered[:limit]:
            ...
            if current_bad or score > best_score:
                ...
                best = {
                    "url": str(resp.url),
                    "method": current.get("method", "") + f"+probe_v27:{reason}",
    ...
    @staticmethod
    def _extract_script_urls_v27(page_url: str, html_body: str) -> list[str]:
        urls = [m.group(0) for m in _SCRIPT_URL_PATTERN_V27.finditer(html_body or "")]
        normalized: list[str] = []
        seen: set[str] = set()
        for raw in urls:
            full = raw.strip()
    ...
    async def _probe_ats_paths_v27(
        self,
        client: httpx.AsyncClient,
        domain: str,
        company_name: str,
        ...
        if "oraclecloud.com" in host:
            ...
            for site_id in self._oracle_site_ids_v27(current_url, html):
                candidate_urls.extend(
                    [
        ...
        if not candidate_urls and _ATS_HOST_HINT_PATTERN_V27.search(host):
    ...
    async def _homepage_recovery_v27(
        self,
        client: httpx.AsyncClient,
        domain: str,
    ) -> Optional[dict]:
        ...
        home_disc = {
            "url": str(resp.url),
            "method": "homepage_recovery_v27",
            "candidates": [str(resp.url)],
            "html": body,
        }
        sub = await self._try_subpage_discovery_v27(client, home_disc)
    ...
    def _listing_page_score(self, url: str, html: str) -> int:
        ...
        if _ERROR_PAGE_PATTERN_V27.search(lower[:10000]):
            score -= 20
        if _LOGIN_PAGE_PATTERN_V27.search(lower[:12000]):
            score -= 10
        if _LISTING_URL_HINT_PATTERN_V27.search(url or ""):
            score += 4
        if _LOWONGAN_CONTEXT_PATTERN_V27.search(url or ""):
            score += 5
        if _JOB_DETAIL_URL_PATTERN_V27.search(url or ""):
            score -= 6
        if _FEED_URL_PATTERN_V27.search(url or ""):
            score -= 8
        if _NON_LISTING_SECTION_PATTERN_V27.search(url or ""):
            score -= 6
        ...
        score += min(len(_LOWONGAN_CONTEXT_PATTERN_V27.findall(lower)), 8)
        ...
        for a_el in root.iter("a"):
            ...
            txt = self._safe_text_v27(a_el)
            href_l = href.lower()

            if _CAREER_HREF_PATTERN_V27.search(href) or _CAREER_TEXT_PATTERN_V27.search(txt):
                career_links += 1

            if _JOB_DETAIL_URL_PATTERN_V27.search(href):
                detail_links += 1

            if 6 <= len(txt) <= 140 and _LISTING_URL_HINT_PATTERN_V27.search(href):
                jobish_links += 1
            if _NON_LISTING_SECTION_PATTERN_V27.search(href_l):
                nav_links += 1
        ...
        for el in root.iter():
            ...
            if not cls:
                continue
            if _ROW_CLASS_PATTERN_V27.search(cls):
    ...
    def _is_bad_target(self, url: str, html: str) -> bool:
        ...
        if _ERROR_PAGE_PATTERN_V27.search(lower):
            return True
        if _LOGIN_PAGE_PATTERN_V27.search(lower):
            return True
        if _FEED_URL_PATTERN_V27.search(url or "") and page_score < 12:
            return True
        if _JOB_DETAIL_URL_PATTERN_V27.search(url or "") and page_score < 8:
            return True
        if _NON_LISTING_SECTION_PATTERN_V27.search(url or "") and page_score < 16:
            return True
        return False

    @staticmethod
    def _safe_text_v27(el: etree._Element) -> str:
        try:
            txt = el.text_content()
            if txt:
    ...
    @staticmethod
    def _is_rejected_link_v27(href: str, text: str) -> bool:
        href_l = (href or "").lower()
        text_l = (text or "").lower()

        if _REJECT_LINK_HREF_PATTERN_V27.search(href_l):
            return True
        if _REJECT_LINK_TEXT_PATTERN_V27.search(text_l):
            return True
        return False
