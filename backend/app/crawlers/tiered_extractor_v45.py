"""
Tiered Extraction Engine v4.5 — direct from v1.6.

High-impact improvements:
1. Simplified Martian shell probing with tighter budgets/timeouts and
   browser-like request headers for faster, more reliable recovery.
2. Expanded script-chunk endpoint hint harvesting with broader API path
   detection for config-only Next.js job boards.
3. Added same-path query-matrix fallback probing for Martian/MyRecruitmentPlus
   pages when generic API endpoint probing misses.
4. Added generic heading+CTA card extraction for Wix/CMS card grids where
   titles and detail links are split across nested wrappers.
5. Made detail enrichment deadline-aware and bounded to reduce timeouts while
   still improving location/type/description depth.
"""
...
_ROLE_HINT_PATTERN_V38 = re.compile(
    ...
    r"associate|representative|agent|planner|crew|yardman|warehouse|liaison|"
...
_TITLE_STATUS_SUFFIX_PATTERN_V43 = re.compile(
    r"(?:\s*[-|]?\s*(?:just\s+posted|posted\s+(?:today|yesterday)|"
    r"\d+\s+(?:minutes?|hours?|days?|weeks?)\s+ago|new(?:\s+role)?))\s*!?\s*$",
    re.IGNORECASE,
)
_SCRIPT_SRC_PATTERN_V44 = re.compile(
    r"<script[^>]+src=['\"]([^\"']+)['\"][^>]*>",
    re.IGNORECASE,
)
_MARTIAN_RECRUITER_ENDPOINT_JS_PATTERN_V44 = re.compile(
    r"/api/recruiter/[0-9]{2,}/(?:jobAds|jobads|job-ads|jobs)"
    r"(?:/[A-Za-z0-9_-]+)?(?:\?[A-Za-z0-9=&_%.\-]+)?",
    re.IGNORECASE,
)
_MARTIAN_ENDPOINT_HINT_JS_PATTERN_V44 = re.compile(
    r"(?:https?://[A-Za-z0-9\.-]+)?/"
    r"(?:api|job-board|jobboard|embed-jobs|jobs/search|job-ads|jobads)"
    r"[A-Za-z0-9_\-/?=&.%]*",
    re.IGNORECASE,
)
_MARTIAN_ENDPOINT_HINT_JS_PATTERN_V45 = re.compile(
    r"(?:https?://[A-Za-z0-9\.-]+)?/"
    r"(?:api(?:/recruiter/[0-9]{2,})?/(?:jobs?|jobads?|job-ads|job-search|search/jobs)|"
    r"job-board|jobboard|embed-jobs|jobs/search|job-ads|jobads|search/jobs)"
    r"[A-Za-z0-9_\-/?=&.%]*",
    re.IGNORECASE,
)
_HTML_FRAGMENT_JOB_MARKER_V44 = re.compile(
    r"(?:\bp-4\b|\bmx-4\b|elementor-column|cursor-pointer|apply\s+now|info\s+lengkap|job-card)",
    re.IGNORECASE,
)
_CTA_CARD_LINK_TEXT_PATTERN_V45 = re.compile(
...
class TieredExtractorV45(TieredExtractorV16):
    """v4.5 extractor with targeted Martian recovery, CTA-card coverage, and faster enrichment."""
    ...
    async def extract(self, career_page, company, html: str) -> list[dict]:
        ...
        try:
            ...
            logger.warning("v4.5 parent extractor timeout for %s", page_url)
        except Exception:
            logger.exception("v4.5 parent extractor failed for %s", page_url)
        ...
        jsonld_jobs = self._extract_jsonld_jobs_v44(working_html, page_url)
        if jsonld_jobs:
            candidates.append(("jsonld_v44", jsonld_jobs))
        ...
        if best_label != "parent_v16" and self._should_enrich_detail_pages_v44(best_jobs, page_url):
            try:
                best_jobs = await asyncio.wait_for(self._enrich_from_detail_pages(best_jobs), timeout=16.0)
            except asyncio.TimeoutError:
                logger.warning("v4.5 enrichment timeout for %s", page_url)
            except Exception:
                logger.exception("v4.5 enrichment failed for %s", page_url)

        final_jobs = self._prepare_candidate_jobs_v38(best_jobs, page_url)
        return final_jobs[:MAX_JOBS_PER_PAGE]

    def _should_enrich_detail_pages_v44(self, jobs: list[dict], page_url: str) -> bool:
        if not jobs:
            return False
        ...
        off_page = [
            ...
            and not self._is_non_job_url_v38(j.get("source_url") or "")
        ]
        if not off_page:
            return False

        detailish_hits = sum(1 for j in off_page if self._is_job_like_url_v38(j.get("source_url") or ""))
        refresh_hits = sum(1 for j in off_page if self._needs_detail_refresh_v44(j))
        unique_off_page = len({(j.get("source_url") or "").rstrip("/") for j in off_page if j.get("source_url")})
        same_host_hits = sum(
            1 for j in off_page
            if (urlparse(j.get("source_url") or "").netloc or "").lower()
            == (urlparse(page_url or "").netloc or "").lower()
        )
        ...
        if refresh_hits >= 2:
            return True
        if unique_off_page >= 5 and same_host_hits >= max(4, unique_off_page - 1):
            return True
        if len(off_page) >= 4 and self._job_descriptions_look_teaser_v44(off_page):
            return True
        return len(off_page) >= 4

    def _needs_detail_refresh_v44(self, job: dict) -> bool:
        ...
        if len(desc) < 145:
            return True
        if re.search(r"\b(?:info\s+lengkap|read\s+more|learn\s+more|click\s+here)\b", desc, re.IGNORECASE):
            return True
        if re.search(r"\b(?:how\s+to\s+apply|cara\s+melamar)\b", desc, re.IGNORECASE):
            return True
        if len(set(re.findall(r"[a-z]{3,}", desc.lower()))) < 20 and len(desc) < 260:
            return True
        if ("elementor" in method or "job_links" in method) and len(desc) < 280:
            return True
        return False

    def _job_descriptions_look_teaser_v44(self, jobs: list[dict]) -> bool:
        snippets = [" ".join(str(j.get("description") or "").lower().split()) for j in jobs]
        snippets = [s for s in snippets if s]
        if not snippets:
            return True

        short_hits = sum(1 for s in snippets if len(s) < 220)
        cta_hits = sum(
            1 for s in snippets if re.search(r"\b(?:info\s+lengkap|read\s+more|click\s+here|learn\s+more)\b", s)
        )
        unique = len(set(snippets))
        if short_hits >= max(2, int(len(snippets) * 0.65)):
            return True
        if cta_hits >= max(2, int(len(snippets) * 0.4)):
            return True
        return unique <= max(2, int(len(snippets) * 0.45))

    async def _enrich_from_detail_pages(self, jobs: list[dict]) -> list[dict]:
        max_candidates = 12 if len(jobs) <= 12 else 16
        jobs_to_enrich = [
            (idx, job) for idx, job in enumerate(jobs)
            if self._needs_detail_refresh_v44(job)
        ][:max_candidates]
        if not jobs_to_enrich:
            return jobs

        loop = asyncio.get_running_loop()
        deadline = loop.time() + 13.5
        ...
        async with httpx.AsyncClient(
            ...
            headers={
                ...
                "Accept-Language": "en-US,en;q=0.9,id;q=0.6",
            ...
            async def _enrich_one(idx: int, job: dict) -> tuple[int, dict]:
                ...
                try:
                    ...
                    return idx, {}
                if resp.status_code != 200 or len(resp.text or "") < 220:
                    return idx, {}
                ...
                body_text = " ".join(_text(root).split())
                ...
                for sel in (
                    ...
                    for node in nodes[:3]:
                        ...
                        if len(text) > 180:
                            desc = self._clean_description_v38(text[:5000])
                            break
                    if desc:
                        break
                if not desc and len(body_text) > 220:
                    desc = self._clean_description_v38(body_text[:5000])
                if desc:
                ...
                for heading in root.xpath("//h2|//h3|//h4|//strong|//dt"):
                    ...
                    if (
                        not location
                        and next_text
                        and 2 < len(next_text) < 180
                        and (
                            has_loc_icon
                            or re.search(r"\b(?:location|lokasi|penempatan|office|city|country)\b", label)
                        )
                    ):
                        location = next_text
                    ...
                    if (
                        not employment_type
                        and next_text
                        and 2 < len(next_text) < 120
                        and (
                            has_type_icon
                            or re.search(r"\b(?:job\s*type|employment|jenis\s+pekerjaan|tipe)\b", label)
                        )
                    ):
                        employment_type = next_text
                ...
                if not employment_type:
                    ...
                    if m:
                        employment_type = m.group(1).strip()[:120]
                if not employment_type:
                    tm = _JOB_TYPE_PATTERN.search(body_text)
                    if tm:
                        employment_type = tm.group(0).strip()[:120]
                if employment_type:
                    enriched["employment_type"] = employment_type

                salary = _SALARY_PATTERN.search(body_text)
                if salary:
                    enriched["salary_raw"] = salary.group(0).strip()[:140]
                ...
                for idx, enriched in results:
                    ...
                    for key, value in enriched.items():
                        ...
                        if key == "description":
                            ...
                            if self._needs_detail_refresh_v44(current) or len(value) > len(current_desc) + 120:
                                current[key] = value
    ...
    def _extract_jsonld_jobs_v44(self, html_body: str, page_url: str) -> list[dict]:
        ...
        for match in re.finditer(
            ...
            for node in self._iter_json_dicts_v39(parsed):
                type_field = node.get("@type")
                if isinstance(type_field, list):
                    type_values = [str(v).lower() for v in type_field]
                    is_job = "jobposting" in type_values
                else:
                    is_job = str(type_field or "").lower() == "jobposting"
                if not is_job:
                    continue

                title = self._normalize_title_v38(str(node.get("title") or node.get("name") or ""))
                if not self._is_valid_title_v38(title) or not self._title_has_role_signal_v38(title):
                    continue
                ...
                location = None
                location_node = node.get("jobLocation") or node.get("location")
                if isinstance(location_node, dict):
                    addr = location_node.get("address")
                    if isinstance(addr, dict):
                        ...
                        location = addr.strip() or None
                elif isinstance(location_node, list):
                    for item in location_node:
                        if isinstance(item, dict):
                            ...
                            location = item.strip()
                            break
                elif isinstance(location_node, str):
                    location = location_node.strip() or None

                description = self._clean_description_v38(
                    str(
                        node.get("description")
                        or node.get("responsibilities")
                        or node.get("qualifications")
                        or ""
                    )
                )
                salary_raw = self._extract_salary_v38(json.dumps(node, ensure_ascii=True, default=str))
                ...
                jobs.append(
                    {
                        ...
                        "extraction_method": "tier0_jsonld_v44",
                        "extraction_confidence": 0.84 if self._is_job_like_url_v38(source_url) else 0.77,
                    }
                )
    ...
    def _extract_from_single_page_v38(self, page_url: str, html_body: str) -> list[dict]:
        ...
        utility_rows = self._extract_utility_card_rows_v43(root, page_url)
        if utility_rows:
            candidates.append(("utility_rows_v43", utility_rows))
        ...
        cta_cards = self._extract_heading_cta_cards_v45(root, page_url)
        if cta_cards:
            candidates.append(("cta_cards_v45", cta_cards))
    ...
    def _extract_utility_card_rows_v43(self, root: etree._Element, page_url: str) -> list[dict]:
        rows = root.xpath(
            "//*[self::div or self::article or self::li]"
            "[contains(concat(' ',normalize-space(@class),' '),' p-4 ') and .//a[@href]]"
        )
        if len(rows) < 2:
            return []

        jobs: list[dict] = []
        for row in rows[: MAX_JOBS_PER_PAGE * 4]:
            row_text = " ".join(_text(row).split())[:2600]
            title_nodes = row.xpath(
                ".//div[contains(@class,'cursor-pointer')]//a[@href][1] | "
                ".//h1//a[@href][1] | .//h2//a[@href][1] | .//h3//a[@href][1] | .//a[@href][1]"
            )
            if not title_nodes:
                continue
            anchor = title_nodes[0]

            source_url = _resolve_url(anchor.get("href"), page_url) or page_url
            if self._is_non_job_url_v38(source_url):
                continue
            ...
            role_hit = self._title_has_role_signal_v38(title)
            detail_hit = self._has_strong_detail_url_v42(source_url)
            apply_hit = bool(_APPLY_EVIDENCE_PATTERN_V38.search(row_text))
            if not role_hit and not detail_hit:
                continue
            if not (apply_hit or detail_hit or "cursor-pointer" in (_get_el_classes(row) or "").lower()):
                continue
            if len(row_text) < 55:
                continue
            ...
            jobs.append(
                {
                    ...
                    "employment_type": self._extract_employment_type_v38(row_text),
                    "description": self._clean_description_v38(row_text),
                    "extraction_method": "tier2_utility_cards_v43",
                    "extraction_confidence": 0.79 if detail_hit else 0.73,
                }
            )
    ...
    async def _extract_oracle_jobs_v39(self, page_url: str, html_body: str) -> list[dict]:
        ...
        try:
            ...
            logger.debug("v4.5 oracle probing failed for %s", page_url)
    ...
    async def _extract_martian_jobs_v39(self, page_url: str, html_body: str) -> list[dict]:
        ...
        extra_hints = await self._harvest_script_endpoint_hints_v44(page_url, html_body)
        if extra_hints:
            ...
            context["endpoint_hints"] = "|".join(merged_hints[:36])
        ...
        jobs: list[dict] = []
        request_budget = 18 if shell_like else 12
        ...
        try:
            async with httpx.AsyncClient(
                ...
                ordered_endpoints = [*fast_pass, *probe_plan[:26]]
                for endpoint in ordered_endpoints:
                    for probe_url in self._martian_paged_variants_v40(endpoint)[:4]:
                        norm_probe = probe_url.rstrip("/")
                        if request_budget <= 0:
                            break
                        if not probe_url or norm_probe in seen_probe_urls:
                            continue
                        seen_probe_urls.add(norm_probe)
                        request_budget -= 1
                        try:
                            resp = await asyncio.wait_for(client.get(probe_url), timeout=2.9)
                        except Exception:
                    ...
                    if len(jobs) >= MIN_JOBS_FOR_SUCCESS and request_budget <= 5:
                        break
                    if consecutive_misses >= 8 and len(jobs) < MIN_JOBS_FOR_SUCCESS:
                        break

                if request_budget > 0 and len(jobs) < MIN_JOBS_FOR_SUCCESS:
                    post_endpoints = self._martian_post_endpoints_v40(ordered_endpoints)
                    payloads = self._martian_post_payloads_v40(context)
                    for endpoint in post_endpoints[:10]:
                ...
                if request_budget > 0 and len(jobs) < MIN_JOBS_FOR_SUCCESS:
                    for probe_url in self._martian_same_path_query_urls_v45(page_url, context)[:14]:
                        norm_probe = probe_url.rstrip("/")
                        if request_budget <= 0 or norm_probe in seen_probe_urls:
                            continue
                        seen_probe_urls.add(norm_probe)
                        request_budget -= 1
                        try:
            ...
            logger.debug("v4.5 martian probing failed for %s", page_url)

        return self._prepare_candidate_jobs_v38(jobs, page_url)

    def _martian_same_path_query_urls_v45(self, page_url: str, context: dict[str, str]) -> list[str]:
        """Query-matrix fallback for client-scoped board paths (e.g. /<client>/)."""
        parsed = urlparse(page_url or "")
        if not parsed.netloc:
            return []

        path = parsed.path or "/"
        base = f"{parsed.scheme or 'https'}://{parsed.netloc}{path}"
        ...
        if recruiter_id:
            query_maps.extend(
                [
                    {"recruiterId": recruiter_id, "pageNumber": "1", "pageSize": "50", "isActive": "true"},
                    {"recruiterId": recruiter_id, "search": ""},
                ]
            )
        ...
        for qm in query_maps:
            query = urlencode({k: v for k, v in qm.items() if v is not None})
    ...
    def _martian_fastpass_endpoints_v42(self, page_url: str, endpoints: list[str]) -> list[str]:
        ...
        page_path = (urlparse(page_url or "").path or "").rstrip("/")
        ...
        for endpoint in endpoints:
            ...
            if "pagenumber=1" in low or "page=1" in low or "offset=0" in low:
                score += 2
            if page_path and (urlparse(norm).path or "").rstrip("/") == page_path and "?" in norm:
                score += 9
            if score > 0:
                ranked.append((score, norm))

        ranked.sort(key=lambda item: item[0], reverse=True)
        return [url for _score, url in ranked[:12]]

    async def _harvest_script_endpoint_hints_v44(self, page_url: str, html_body: str) -> list[str]:
        script_urls = self._script_src_urls_v44(page_url, html_body)
        if not script_urls:
            return []
        ...
        try:
            async with httpx.AsyncClient(
                ...
                for script_url in script_urls[:10]:
                    try:
                        resp = await asyncio.wait_for(client.get(script_url), timeout=3.1)
                    except Exception:
                        continue
                    payload = resp.text or ""
                    if resp.status_code >= 400 or len(payload) < 100:
                        continue

                    for match in _MARTIAN_ENDPOINT_HINT_JS_PATTERN_V44.finditer(payload):
                    ...
                    for match in _MARTIAN_ENDPOINT_HINT_JS_PATTERN_V45.finditer(payload):
                        candidate = (match.group(0) or "").strip().strip("\"' ")
                        if not candidate:
                            continue
                        if not candidate.startswith("http"):
                    ...
                    for match in _MARTIAN_RECRUITER_ENDPOINT_JS_PATTERN_V44.finditer(payload):
    ...
    def _script_src_urls_v44(self, page_url: str, html_body: str) -> list[str]:
        ranked: list[tuple[int, str]] = []
        seen: set[str] = set()
        page_host = (urlparse(page_url or "").netloc or "").lower()

        for match in _SCRIPT_SRC_PATTERN_V44.finditer(html_body or ""):
    ...
    def _martian_probe_urls_v39(self, page_url: str, context: dict[str, str]) -> list[str]:
        ...
        if page_path and page_path != "/":
            path_roots.append(page_path.rstrip("/"))
        if client_code:
            client_root = f"/{client_code}"
            if client_root not in path_roots:
                path_roots.append(client_root)
        path_roots.append("")
        ...
        if recruiter_id:
            query_templates.extend(
                [
                    ...
                    f"recruiterId={recruiter_id}&pageNumber=1&pageSize=50&isActive=true&sorting=PublishDateDesc",
        ...
        query_templates = list(dict.fromkeys(query_templates))
        ...
        for host in hosts:
            ...
            for root_path in path_roots:
                base = f"{host}{root_path}" if root_path else host
                urls.append(base)
                for query in query_templates:
                    urls.append(f"{base}?{query}")
                if client_code:
                    urls.append(f"{base}?client={client_code}")
                    urls.append(f"{base}?clientCode={client_code}")
                    urls.append(f"{base}?client={client_code}&sorting=PublishDateDesc")
                if recruiter_id:
                    urls.append(f"{base}?recruiterId={recruiter_id}&pageNumber=1&pageSize=50")

            if page_query.get("jobAdId"):
                ad_id = str(page_query.get("jobAdId") or "").strip()
                if ad_id:
                    urls.append(
                        f"{host}/?jobAdId={ad_id}&pageNumber=1&pageSize=50&isActive=true&sorting=PublishDateDesc"
                    )
                    if client_code:
                        urls.append(
                            f"{host}/?client={client_code}&jobAdId={ad_id}&pageNumber=1&pageSize=50&isActive=true&sorting=PublishDateDesc"
                        )
        ...
        page_path_norm = page_path.rstrip("/")
        preferred_page_paths: list[str] = []
        for candidate in deduped:
            cand_parsed = urlparse(candidate)
            cand_host = (cand_parsed.netloc or "").lower()
            cand_path = (cand_parsed.path or "").rstrip("/")
            if (
                cand_host == page_host
                and page_path_norm
                and cand_path == page_path_norm
                and cand_parsed.query
            ):
                preferred_page_paths.append(candidate)

        deduped.sort(
            key=lambda candidate: (
                self._martian_endpoint_priority_v40(candidate)
                + (12 if (urlparse(candidate).netloc or "").lower() == page_host else 0)
                + (
                    5
                    if page_path_norm
                    and (urlparse(candidate).path or "").rstrip("/") == page_path_norm
                    else 0
                )
            ),
            reverse=True,
        )
        prioritized = preferred_page_paths[:48] + deduped
        final_urls: list[str] = []
        final_seen: set[str] = set()
        for candidate in prioritized:
            norm = candidate.rstrip("/")
            if norm in final_seen:
                continue
            final_seen.add(norm)
            final_urls.append(candidate)
        return final_urls[:320]
    ...
    def _extract_jobs_from_probe_payload_v40(self, body: str, response_url: str, page_url: str) -> list[dict]:
        ...
        if payload.startswith("{") or payload.startswith("["):
            ...
            if parsed is not None:
                ...
                if not jobs:
                    jobs.extend(self._extract_high_confidence_json_jobs_v44(parsed, response_url))
        ...
        if root is not None:
            try:
                ...
                anchor_count = 0
            marker_hit = bool(_HTML_FRAGMENT_JOB_MARKER_V44.search(payload[:12000]))
            if anchor_count >= 3 or (anchor_count >= 1 and marker_hit):
                jobs.extend(self._extract_from_single_page_v38(response_url, payload))

        return self._prepare_candidate_jobs_v38(jobs, page_url)

    def _extract_high_confidence_json_jobs_v44(self, payload: Any, page_url: str) -> list[dict]:
        jobs: list[dict] = []
        for row in self._iter_json_dicts_v39(payload):
            ...
            detailish = self._is_job_like_url_v38(source_url) or bool(req_id)
            location_value = row.get("location") or row.get("primaryLocation")
            has_location = isinstance(location_value, (str, dict, list)) and bool(location_value)
            description = self._clean_description_v38(str(row.get("description") or row.get("summary") or ""))
            has_rich_desc = bool(description and len(description) >= 150)
            has_type = bool(row.get("employmentType") or row.get("jobType"))
            has_salary = bool(row.get("salary") or row.get("salaryRange"))

            if not detailish:
                continue
            if not (has_location or has_rich_desc or has_type or has_salary):
                continue
            ...
            jobs.append(
                {
                    "title": title,
                    "source_url": source_url,
                    "location_raw": str(location_value).strip()[:140] if isinstance(location_value, str) else None,
                    "salary_raw": self._extract_salary_v38(json.dumps(row, ensure_ascii=True, default=str)),
                    "employment_type": self._extract_employment_type_v38(json.dumps(row, ensure_ascii=True, default=str)),
                    "description": description,
                    "extraction_method": "tier0_json_salvage_v44",
    ...
    def _martian_paged_variants_v40(endpoint: str) -> list[str]:
        ...
        if "?" not in endpoint:
            variants.append(f"{endpoint}?search=")
        elif "search=" not in endpoint and "?" in endpoint:
            variants.append(f"{endpoint}&search=")
        if "?" in endpoint and "sorting=" not in endpoint:
            variants.append(f"{endpoint}&sorting=PublishDateDesc")
        if "?" in endpoint and "isactive=" not in endpoint.lower():
            variants.append(f"{endpoint}&isActive=true")
    ...
    def _extract_heading_cta_cards_v45(self, root: etree._Element, page_url: str) -> list[dict]:
        """Extract role cards where heading title and CTA link are split in nested wrappers."""
        jobs: list[dict] = []

        for a_el in root.xpath("//a[@href]")[:5000]:
            link_text = " ".join(_text(a_el).split())
            if not _CTA_CARD_LINK_TEXT_PATTERN_V45.match(link_text):
                continue
            ...
            if not (has_apply_context or detail_hit or len(row_text) >= 95):
                continue
            ...
            jobs.append(
                {
                    "title": title,
                    "source_url": source_url,
                    "location_raw": self._extract_location_v38(best_row, title),
                    "salary_raw": self._extract_salary_v38(row_text),
                    "employment_type": self._extract_employment_type_v38(row_text),
                    "description": self._clean_description_v38(row_text),
                    "extraction_method": "tier2_heading_cta_cards_v45",
                    "extraction_confidence": 0.77 if detail_hit else 0.72,
                }
            )
        ...
        role_hits = sum(1 for j in prepared if self._title_has_role_signal_v38(j.get("title", "")))
    ...
    def _normalize_title_v38(self, title: str) -> str:
        ...
        value = _TITLE_STATUS_SUFFIX_PATTERN_V43.sub("", value)
        value = re.sub(r"\s+OR\s+.*$", "", value, flags=re.IGNORECASE)
        value = value.strip(" \t\r\n-–|:;,")
        value = _TITLE_STATUS_SUFFIX_PATTERN_V43.sub("", value).strip(" \t\r\n-–|:;,")
