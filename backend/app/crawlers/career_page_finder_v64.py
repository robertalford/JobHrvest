"""
Career Page Finder v6.4 — Fast hint-aware discovery with listing-dense scoring
and subdomain detection. No slow pagination probing.

Inherits from v6.2, NOT v6.3 (keeps v6.3's useful features, skips the slow parts).

Changes from v6.2:
  1. URL hint support: set_hint(url) and find() tries hint first. If hint
     returns valid HTML, use it. Otherwise fall back to normal discovery.
  2. Listing-dense scoring from v6.3: bonus for pages with many job links.
  3. Subdomain detection from v6.3: careers.X.com / jobs.X.com probing.
  4. NO slow pagination probing — just find the listing page.
"""
...
class CareerPageFinderV64(CareerPageFinderV62):
    """v6.4 finder: URL hint support, listing-dense page preference,
    subdomain career detection. No slow pagination probing."""
    ...
    async def find(self, domain: str, company_name: str = "") -> dict:
        ...
        if self._hint_url:
            hint_result = await self._try_hint_url_v64(self._hint_url)
            if hint_result:
                logger.info(
                    "v6.4 hint URL successful: %s", hint_result.get("url")
                )
                return hint_result
            logger.info(
                "v6.4 hint URL failed (%s), falling back to discovery",
                self._hint_url,
            )

        # --- Change 3: Try career/jobs subdomains directly ---
        subdomain_result = await self._try_career_subdomains_v64(domain)
    ...
    async def _try_hint_url_v64(self, hint_url: str) -> Optional[dict]:
        ...
        try:
            ...
            logger.debug("v6.4 hint URL fetch failed for %s: %s", hint_url, e)
            return None

    # ==================================================================
    # Change 2: Listing density bonus scoring (from v6.3)
    # ==================================================================
    ...
    # ==================================================================
    # Change 3: Subdomain career detection (from v6.3)
    # ==================================================================

    async def _try_career_subdomains_v64(self, domain: str) -> Optional[dict]:
                ...
                for prefix in _CAREER_SUBDOMAINS:
                    ...
                    for path in ["/", "/jobs", "/jobs/"]:
                        ...
                        if score >= 3 and score > best_score:
                            ...
                            best_result = {
                                "url": final_url,
                                "method": f"subdomain_probe_v64:{prefix}{path}",
                                "html": html,
                            }
