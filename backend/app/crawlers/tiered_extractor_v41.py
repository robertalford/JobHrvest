"""
Tiered Extraction Engine v4.1 — direct from v1.6.

High-impact improvements:
1. Adds embedded state JSON extraction (`__NEXT_DATA__` + `application/json`) for hidden job objects.
2. Adds Greenhouse tenant recovery probes using company/title slug inference.
3. Promotes listing subpage traversal on career hub pages even when a small jobset was found.
4. Tightens small-set precision and job-evidence gating to suppress menu/section false positives.
5. Preserves existing Martian/Oracle/API fallbacks from v4.0.
"""
...
class TieredExtractorV41(TieredExtractorV16):
    """v4.1 extractor with stronger discovery recovery, state extraction, and precision gates."""
    ...
    async def extract(self, career_page, company, html: str) -> list[dict]:
        ...
        try:
            ...
            logger.warning("v4.1 parent extractor timeout for %s", page_url)
        except Exception:
            logger.exception("v4.1 parent extractor failed for %s", page_url)
        ...
        if best_label != "parent_v16" and any(
            ...
            try:
                ...
                logger.warning("v4.1 enrichment timeout for %s", page_url)
            except Exception:
                logger.exception("v4.1 enrichment failed for %s", page_url)
    ...
    async def _extract_oracle_jobs_v39(self, page_url: str, html_body: str) -> list[dict]:
        ...
        try:
            ...
            logger.debug("v4.1 oracle probing failed for %s", page_url)
    ...
    async def _extract_martian_jobs_v39(self, page_url: str, html_body: str) -> list[dict]:
        ...
        jobs: list[dict] = []
        request_budget = 14 if shell_like else 10
        try:
            async with httpx.AsyncClient(
                timeout=2.4,
                follow_redirects=True,
                ...
                for endpoint in probe_plan:
                    for probe_url in self._martian_paged_variants_v40(endpoint)[:2]:
                        ...
                        request_budget -= 1
                        try:
                            resp = await asyncio.wait_for(client.get(probe_url), timeout=2.4)
                        except Exception:
                            continue
                ...
                if request_budget > 0 and len(jobs) < MIN_JOBS_FOR_SUCCESS:
                    ...
                    for endpoint in post_endpoints[:16]:
                        for payload in payloads[:8]:
                            ...
                            request_budget -= 1
                            try:
                                resp = await asyncio.wait_for(client.post(endpoint, json=payload), timeout=2.4)
                            except Exception:
                                continue
                            if resp.status_code >= 400 or not resp.text:
                                continue
            ...
            logger.debug("v4.1 martian probing failed for %s", page_url)
