"""
Career Page Finder v3.5 — direct from CareerPageFinderV4.

v3.5 keeps the proven v3.0 discovery behavior and adds:
...
class CareerPageFinderV35(CareerPageFinderV4):
    """v3.5 finder wrapper with query-variant and Oracle tenant-path recovery."""
    ...
    async def find(self, domain: str, company_name: str = "") -> dict:
        ...
        canonical_upgrade = await self._canonical_listing_upgrade_v35(delegate, disc)
        if canonical_upgrade:
            disc = canonical_upgrade

        query_upgrade = await self._query_listing_upgrade_v35(delegate, disc)
        if query_upgrade:
            disc = query_upgrade
        ...
        improved = await self._oracle_tenant_recovery_v35(delegate, disc)
        return improved or disc

    async def _canonical_listing_upgrade_v35(self, delegate: CareerPageFinderV30, disc: dict) -> dict | None:
        ...
        return {
            "url": str(resp.url),
            "method": disc.get("method", "") + "+canonical_v35",
            "candidates": disc.get("candidates", []) + [canonical_url],
            "html": body,
        }

    async def _query_listing_upgrade_v35(self, delegate: CareerPageFinderV30, disc: dict) -> dict | None:
        current_url = disc.get("url", "")
        current_html = disc.get("html") or ""
        ...
        try:
            async with httpx.AsyncClient(timeout=8, follow_redirects=True, headers=_CLIENT_HEADERS) as client:
                for candidate_url in candidate_urls:
                    ...
                    if self._has_listing_markers_v35(body):
                        score += 0.8
                    ...
                    best = {
                        "url": str(resp.url),
                        "method": disc.get("method", "") + "+query_variant_v35",
                        "candidates": disc.get("candidates", []) + [candidate_url],
                        "html": body,
                    }
        except Exception:
            logger.debug("v3.5 query listing upgrade failed for %s", current_url)

        return best

    @staticmethod
    def _has_listing_markers_v35(html_body: str) -> bool:
        body = html_body or ""
        return bool(
            re.search(
    ...
    async def _oracle_tenant_recovery_v35(self, delegate: CareerPageFinderV30, disc: dict) -> dict | None:
        current_url = disc.get("url", "")
        current_html = disc.get("html") or ""
        ...
        site_ids = self._oracle_site_ids_v35(current_url, current_html)
        if not site_ids:
            return None
            ...
            async with httpx.AsyncClient(timeout=8, follow_redirects=True, headers=_CLIENT_HEADERS) as client:
                for candidate_url, candidate_site in candidate_urls:
                    ...
                    if should_upgrade:
                        ...
                        best = {
                            "url": str(resp.url),
                            "method": disc.get("method", "") + f"+oracle_tenant_v35:{candidate_site}",
                            "candidates": disc.get("candidates", []) + [candidate_url],
                            "html": body,
                        }
        except Exception:
            logger.debug("v3.5 oracle tenant recovery failed for %s", current_url)

        return best

    @staticmethod
    def _oracle_site_ids_v35(page_url: str, html_body: str) -> list[str]:
        seen: set[str] = set()
        ordered: list[str] = []
