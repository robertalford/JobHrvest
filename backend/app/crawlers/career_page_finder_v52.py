"""
Career Page Finder v5.2 — direct from CareerPageFinderV4.
...
_LISTING_TEXT_PATTERN_V52 = re.compile(
...
class CareerPageFinderV52(CareerPageFinderV4):
    """v5.2 finder with platform-aware recovery and bad-target rejection."""
    ...
    async def find(self, domain: str, company_name: str = "") -> dict:
        try:
            ...
            logger.warning("v5.2 parent finder timeout for %s", domain)
            parent_disc = {"url": None, "method": "parent_timeout", "candidates": [], "html": None}
        except Exception:
            logger.exception("v5.2 parent finder failed for %s", domain)
            parent_disc = {"url": None, "method": "parent_error", "candidates": [], "html": None}

        needs_recovery = not self._is_usable_discovery_v52(parent_disc)
        if not needs_recovery:
            upgraded = await self._try_platform_upgrade_v52(parent_disc)
            if upgraded:
                return upgraded
            return parent_disc

        recovered = await self._probe_platform_paths_v52(domain, company_name, parent_disc)
        if recovered:
            return recovered

        hub_recovered = await self._homepage_hub_recovery_v52(domain)
        if hub_recovered:
            return hub_recovered

        return parent_disc

    def _is_usable_discovery_v52(self, disc: dict) -> bool:
        url = disc.get("url") or ""
        body = disc.get("html") or ""
        if not url or not body or len(body) < 180:
            return False
        if self._is_bad_target_url_v52(url):
            return False
        if self._looks_non_html_payload_v52(body) and not self._looks_like_feed_xml_v52(body):
            return False
        if self._looks_like_error_shell_v52(body):
            return False

        score = self._score_listing_page_v52(url, body)
        return score >= 2

    async def _try_platform_upgrade_v52(self, disc: dict) -> dict | None:
        url = disc.get("url") or ""
        ...
        if "elmotalent.com" in host and self._looks_like_error_shell_v52(html_body):
            for slug in self._elmotalent_slug_candidates_v52(company_name="", page_url=url):
                probe_candidates.extend(
                    [
        ...
        best = await self._fetch_best_probe_v52(probe_candidates)
        if not best:
            return None

        best_url, best_html, best_score = best
        if best_score <= self._score_listing_page_v52(url, html_body):
            return None

        return {
            "url": best_url,
            "method": f"{disc.get('method') or 'v4'}+platform_upgrade_v52",
            "candidates": [best_url] + list(disc.get("candidates") or [])[:4],
            "html": best_html,
        }

    async def _probe_platform_paths_v52(self, domain: str, company_name: str, parent_disc: dict) -> dict | None:
        base_url = f"https://{domain}"

        paths = list(_LOCALIZED_CAREER_PATHS_V52)
        ...
        if "elmotalent.com" in domain:
            ...
            for slug in self._elmotalent_slug_candidates_v52(company_name=company_name, page_url=parent_url):
                paths.extend(
                    [
                        f"/careers/{slug}/jobs",
        ...
        if "job-boards.greenhouse.io" in domain:
            slug = self._slugify_company_v52(company_name)
            if slug:
                paths.extend([
        ...
        best = await self._fetch_best_probe_v52(probes)
        if not best:
            return None
    ...
    async def _fetch_best_probe_v52(self, urls: list[str]) -> tuple[str, str, int] | None:
        if not urls:
            return None
        ranked_urls = sorted(
            urls,
            key=lambda u: self._probe_priority_v52(u),
            reverse=True,
        )
        ...
        async with httpx.AsyncClient(timeout=4.5, follow_redirects=True, headers=_CLIENT_HEADERS) as client:
            ...
            async def _try(url: str) -> tuple[str, str | None, int]:
                ...
                if self._is_bad_target_url_v52(resolved):
                ...
                score = self._score_listing_page_v52(resolved, body)
    ...
    async def _homepage_hub_recovery_v52(self, domain: str) -> dict | None:
        base_url = f"https://{domain}"
        ...
        try:
            async with httpx.AsyncClient(timeout=4.5, follow_redirects=True, headers=_CLIENT_HEADERS) as client:
                ...
                body = resp.text or ""
                if len(body) < 200 or self._looks_non_html_payload_v52(body):
                    return None
                ...
                for a_el in root.xpath("//a[@href]")[:1800]:
                    ...
                    text = self._safe_text_v52(a_el)
                    score = 0
                    if _LISTING_TEXT_PATTERN_V52.search(text):
                        score += 10
                    if _LISTING_HREF_PATTERN_V52.search(parsed.path or ""):
                        score += 8
                    if self._is_bad_target_url_v52(full_url):
                        score -= 8
                    if score > 0:
                ...
                for _score, candidate_url, label in scored_links[:3]:
                    ...
                    for probe_url in probe_urls:
                        ...
                        if sub_resp.status_code != 200 or len(sub_body) < 180:
                            continue
                        if self._looks_non_html_payload_v52(sub_body) and not self._looks_like_feed_xml_v52(sub_body):
                            continue
                        if self._looks_like_error_shell_v52(sub_body):
                            continue

                        sub_score = self._score_listing_page_v52(str(sub_resp.url), sub_body)
                        if sub_score < 2:
                            continue

                        return {
                            "url": str(sub_resp.url),
                            "method": f"homepage_hub_v52:{label}",
                            "candidates": [u for _, u, _ in scored_links[:5]],
                            "html": sub_body,
                        }

        except Exception:
            logger.debug("v5.2 homepage hub recovery failed for %s", domain)

        return None

    def _score_listing_page_v52(self, page_url: str, html_body: str) -> int:
        ...
        if "downloadrssfeed" in (page_url or "").lower():
            score -= 6
        if self._is_bad_target_url_v52(page_url):
            score -= 5
        if self._looks_like_error_shell_v52(html_body):
            score -= 15
    ...
    @staticmethod
    def _looks_non_html_payload_v52(body: str) -> bool:
        if not body:
            return True
    ...
    @staticmethod
    def _is_bad_target_url_v52(url: str) -> bool:
        return bool(_BAD_TARGET_URL_PATTERN_V52.search((url or "").strip()))

    @staticmethod
    def _looks_like_error_shell_v52(body: str) -> bool:
        sample = (body or "")[:7000]
        if not sample:
            return True
        return bool(_ERROR_SHELL_PATTERN_V52.search(sample))

    @staticmethod
    def _probe_priority_v52(url: str) -> int:
        low = (url or "").lower()
        score = 0
    ...
    @staticmethod
    def _slugify_company_v52(company_name: str) -> str:
    ...
    def _elmotalent_slug_candidates_v52(self, company_name: str, page_url: str) -> list[str]:
        candidates: list[str] = []
    ...
    @staticmethod
    def _safe_text_v52(el: etree._Element) -> str:
        try:
            txt = el.text_content()
            if txt:
