"""
Tiered Extraction Engine v7.4 — focused ATS + JSON-feed recovery.

Strategy:
1. Keep v7.3 precision safeguards as the baseline behavior.
2. Add a dedicated PageUp listing extractor with bounded pagination follow-up.
3. Add a dedicated Recruitee `/o/<slug>` extractor.
4. Recover JS-shell listings via bounded `jobs.json` feed probing.
5. Keep strict title filtering to avoid Type-1 regressions.
"""
...
_V73_FAST_PATH_METHOD_PREFIXES = (
    ...
    "ats_pageup_listing_v74",
    "ats_recruitee_o_v74",
    "ats_jobs_json_feed_v74",
...
_V74_JOBS_JSON_FETCH = re.compile(
    r"""fetch\(\s*["']([^"']*jobs[^"']*\.json[^"']*)["']\s*\)""",
    re.IGNORECASE,
)
_V74_FEED_ROLE_HINT = re.compile(
    r"\b(?:"
    r"engineer|developer|manager|executive|assistant|analyst|specialist|"
    r"coordinator|consultant|designer|architect|technician|officer|"
    r"administrator|intern(?:ship)?|animator|artist|writer|producer|director|"
    r"secretary|teacher|nurse|chef|driver|cashier|operator|mechanic|scientist|"
    r"chemist|accountant|sales|marketing|product|support"
    r")\b",
    re.IGNORECASE,
)


class TieredExtractorV74(TieredExtractorV69):
    """v7.4 extractor: PageUp/Recruitee recovery + JSON feed fallback."""
    ...
    async def extract(self, career_page, company, html: str) -> list[dict]:
        ...
        if self._should_enrich_fast_path_v73(jobs, page_url):
            try:
                ...
                logger.warning("v7.4 fast-path enrichment timeout for %s", page_url)
            except Exception:
                logger.exception("v7.4 fast-path enrichment failed for %s", page_url)
    ...
    @staticmethod
    def _is_pageup_listing_page_v74(page_url: str, html: str) -> bool:
        host = (urlparse(page_url).netloc or "").lower()
        if "pageuppeople.com" in host:
            return True
        lower_html = (html or "")[:180000].lower()
        return "id=\"recent-jobs-content\"" in lower_html and "list-title" in lower_html
    ...
    def _extract_pageup_listing_jobs_v74(self, html: str, page_url: str) -> list[dict]:
        ...
        for title_node in title_nodes[:1200]:
            ...
            if not raw_title or not self._is_valid_title_v60(raw_title):
                continue
    ...
    def _pageup_pagination_urls_v74(self, html: str, page_url: str) -> list[str]:
        root = _parse_html(html)
        if root is None:
            return []
        ...
        page_host = (urlparse(page_url).netloc or "").lower()
        links = root.xpath(
            "//a[contains(@class,'more-link') and @href]"
            "|//a[contains(@href,'page=') and @href]"
            "|//a[@rel='next' and @href]"
        )
        urls: list[str] = []
        ...
        for link in links[:120]:
            ...
            if (parsed.netloc or "").lower() != page_host:
                continue
            low_full = full.lower()
            if "/listing" not in low_full and "page=" not in low_full:
                continue
            if full.rstrip("/") == page_url.rstrip("/"):
                continue
            if full in seen:
    ...
    async def _expand_pageup_pages_v74(
        self,
        seed_html: str,
        page_url: str,
        seed_jobs: list[dict],
        ...
        queue = self._pageup_pagination_urls_v74(seed_html, page_url)
        if not queue:
            return self._dedupe_basic_v66(seed_jobs)
        ...
        async with httpx.AsyncClient(
            ...
            while queue and fetch_count < 3:
                ...
                if resp.status_code != 200 or len(body) < 400:
                    continue
                merged.extend(self._extract_pageup_listing_jobs_v74(body, str(resp.url)))
                for next_url in self._pageup_pagination_urls_v74(body, str(resp.url)):
                    next_norm = next_url.rstrip("/")
                    if next_norm in seen_pages or next_url in queue:
                        continue
    ...
    def _extract_recruitee_jobs_v74(self, html: str, page_url: str) -> list[dict]:
        host = (urlparse(page_url).netloc or "").lower()
        if "recruitee.com" not in host and "offer-list-grid" not in (html or ""):
            return []
        root = _parse_html(html)
        if root is None:
            return []

        anchors = root.xpath("//a[@href and contains(@href,'/o/')]")
        if not anchors:
            return []

        jobs: list[dict] = []
        seen_urls: set[str] = set()
        for a_el in anchors[:600]:
            source_url = _resolve_url((a_el.get("href") or "").strip(), page_url) or page_url
            if source_url in seen_urls or self._is_non_job_url(source_url):
                continue

            title = self._normalize_title(_text(a_el) or "")
            if not title:
                continue
            if not self._is_valid_card_title_v67(title, has_strong_job_path=True):
                continue
            if self._is_obvious_non_job_card_v73(title, source_url, page_url, has_strong_job_path=True):
                continue

            row = self._find_row_container_v73(a_el)
            location = None
            if row is not None:
                loc_nodes = row.xpath(".//li[@data-testid='styled-location-list-item']|.//*[contains(@class,'location')]")
                for node in loc_nodes[:4]:
                    loc_text = " ".join((_text(node) or "").split()).strip(" ,|-")
                    if 2 <= len(loc_text) <= 120 and loc_text.lower() != title.lower():
                        location = loc_text
                        break
            ...
            jobs.append(
                {
                    ...
                    "salary_raw": None,
                    "employment_type": None,
                    "extraction_method": "ats_recruitee_o_v74",
                    "extraction_confidence": 0.9,
                }
            )
    ...
    async def _extract_jobs_json_feed_v74(self, page_url: str, html: str) -> list[dict]:
        host = (urlparse(page_url).netloc or "").lower()
        if host in {"example.com", "localhost", "127.0.0.1"}:
            return []
        if "jobs" not in (html or "").lower():
            return []

        feed_paths = [m.group(1).strip() for m in _V74_JOBS_JSON_FETCH.finditer(html or "")]
        if not feed_paths:
            return []

        candidate_urls: list[str] = []
        seen_urls: set[str] = set()
        for raw in feed_paths[:4]:
        ...
        async with httpx.AsyncClient(
            ...
            for target in candidate_urls:
                ...
                if len(jobs) >= 3 and self._passes_jobset_validation(jobs, page_url):
                    return jobs

        return []

    def _extract_jobs_json_items_v74(self, payload: Any, page_url: str) -> list[dict]:
        items: list[Any]
        ...
        for item in items[:800]:
            if not isinstance(item, dict):
                continue
            title = self._pick_first_json_text_v74(item, ("title", "name", "position", "job_title", "jobTitle"))
            title = self._normalize_title(title or "")
            if not title:
                continue
            if not self._is_valid_title_v60(title):
                # Feed-specific safe fallback for common 2D/3D-style role names.
                title_words = title.split()
                numeric_role = bool(re.match(r"^\d+[A-Za-z]{1,3}\s+[A-Za-z]", title))
                feed_role = bool(_V74_FEED_ROLE_HINT.search(title))
                if not (
                    len(title_words) <= 8
                    and not _V73_NON_JOB_HEADING.match(title)
                    and (numeric_role or feed_role)
                ):
                    continue

            source_raw = self._pick_first_json_text_v74(item, ("url", "link", "applyUrl", "apply_url", "source_url"))
            if source_raw:
                source_url = _resolve_url(source_raw, page_url) or page_url
            else:
                source_url = f"{page_url.rstrip('/')}/?job={quote_plus(title.lower())}"
            if self._is_non_job_url(source_url):
                continue

            location = self._pick_first_json_text_v74(item, ("location", "city", "location_raw", "region"))
            if isinstance(location, str):
                location = " ".join(location.split())[:120]

            description = self._pick_first_json_text_v74(item, ("description", "summary", "content", "body"))
            description = self._clean_description_v73(description) if isinstance(description, str) else None
            ...
            jobs.append(
                {
                    ...
                    "location_raw": location,
                    "description": description,
                    "salary_raw": self._pick_first_json_text_v74(item, ("salary", "salary_raw", "compensation")),
                    "employment_type": self._pick_first_json_text_v74(item, ("employment_type", "type", "jobType")),
                    "extraction_method": "ats_jobs_json_feed_v74",
                    "extraction_confidence": 0.88,
                }
            )
        return self._dedupe_basic_v66(jobs)
    ...
    @staticmethod
    def _pick_first_json_text_v74(node: dict[str, Any], keys: tuple[str, ...]) -> Optional[str]:
        for key in keys:
            value = node.get(key)
            if isinstance(value, str) and value.strip():
                return value.strip()
