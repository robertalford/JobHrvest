"""
Tiered Extraction Engine v4.0 — direct from v1.6.

High-impact improvements:
1. Keeps v3.8/v3.9 quality gates and container scoring.
2. Strengthens Martian/MyRecruitmentPlus fallback with host-diversified endpoint planning.
3. Adds Martian POST search probing plus HTML-payload extraction (not JSON-only).
4. Expands Next.js context and endpoint hint harvesting for app-shell boards.
5. Preserves dotted/acronym title prefixes (for example `.NET`) during title normalization.
"""
...
class TieredExtractorV40(TieredExtractorV16):
    """v4.0 extractor with stronger app-shell recovery for Martian and ATS boards."""
    ...
    async def extract(self, career_page, company, html: str) -> list[dict]:
        ...
        try:
            ...
            logger.warning("v4.0 parent extractor timeout for %s", page_url)
        except Exception:
            logger.exception("v4.0 parent extractor failed for %s", page_url)
        ...
        if best_label != "parent_v16" and any(
            ...
            try:
                ...
                logger.warning("v4.0 enrichment timeout for %s", page_url)
            except Exception:
                logger.exception("v4.0 enrichment failed for %s", page_url)
    ...
    async def _extract_oracle_jobs_v39(self, page_url: str, html_body: str) -> list[dict]:
        ...
        try:
            ...
            logger.debug("v4.0 oracle probing failed for %s", page_url)
    ...
    async def _extract_martian_jobs_v39(self, page_url: str, html_body: str) -> list[dict]:
        ...
        try:
            ...
            logger.debug("v4.0 martian probing failed for %s", page_url)
            ...
            jobs.append(
                {
                    ...
                    "description": description,
                    "extraction_method": "tier0_json_api_v40",
