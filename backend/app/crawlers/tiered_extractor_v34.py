"""
Tiered Extraction Engine v3.4 — direct from v1.6 with precision-first recovery.

High-impact improvements:
1. Same-page evidence validation (salary/job-type/location + rich context) to recover
   real prose listings without relying only on "Apply" strings in cleaned descriptions.
2. High-confidence small-set acceptance for real 1-2 vacancy pages with strict
   role/evidence checks (prevents false negatives on low-volume employers).
3. Stronger non-job suppression for editorial/CTA titles and ATS signup URLs
   (e.g. "Our Values", "Get Started", alert/register pages).
4. Expanded Martian/MyRecruitmentPlus probing with broader host/path variants and
   richer Next.js `_next/data` derivation for config-shell job boards.
5. Post-enrichment cleanup to trim application-instruction tails and keep richer,
   role-focused descriptions.
"""
...
_JOB_DETAILISH_URL_PATTERN_V33 = re.compile(
    ...
    r"/p/[a-z0-9_-]{6,}|fRecruit__ApplyJob|"
    r"[?&](?:jobid|job_id|requisitionid|positionid|vacancyno)=[A-Za-z0-9_-]{2,})",
    re.IGNORECASE,
)
...
_RICH_CONTEXT_PATTERN_V34 = re.compile(
...
class TieredExtractorV34(TieredExtractorV16):
    """v3.4 extractor with coverage-first fallbacks and stricter validation."""
    ...
    async def extract(self, career_page, company, html: str) -> list[dict]:
        ...
        try:
            ...
            logger.warning("v3.4 parent extractor timeout for %s", url)
        except Exception:
            logger.exception("v3.4 parent extractor failed for %s", url)

        parent_jobs = self._dedupe_jobs_v34(parent_jobs or [], url)
        if parent_jobs:
            candidates.append(("parent_v16", parent_jobs))

        jsonld_jobs = self._extract_jsonld_jobs_v34(working_html, url)
        if jsonld_jobs:
            candidates.append(("jsonld_v34", jsonld_jobs))

        root = _parse_html(working_html)
        if root is not None:
            prose_jobs = self._extract_prose_heading_blocks_v34(root, url)
            if prose_jobs:
                candidates.append(("prose_blocks_v34", prose_jobs))

            elementor_jobs = self._extract_elementor_cards_v34(root, url)
            if elementor_jobs:
                candidates.append(("elementor_cards_v34", elementor_jobs))

            heading_jobs = self._extract_heading_rows_v34(root, url)
            if heading_jobs:
                candidates.append(("heading_rows_v34", heading_jobs))

            link_jobs = self._extract_job_links_v34(root, url)
            if link_jobs:
                candidates.append(("job_links_v34", link_jobs))

            grouped_rows = self._extract_repeating_row_groups_v34(root, url)
            if grouped_rows:
                candidates.append(("row_groups_v34", grouped_rows))

        martian_jobs = await self._extract_martian_jobs_v34(url, working_html)
        if martian_jobs:
            candidates.append(("martian_api_v34", martian_jobs))

        oracle_jobs = await self._extract_oracle_jobs_v34(url, working_html)
        if oracle_jobs:
            candidates.append(("oracle_api_v34", oracle_jobs))

        best_label, best_jobs = self._pick_best_jobset_v34(candidates, url)
        if not best_jobs:
            return []

        if best_label != "parent_v16" and any(self._is_job_like_url_v34(j.get("source_url") or "") for j in best_jobs):
            try:
                best_jobs = await asyncio.wait_for(self._enrich_from_detail_pages(best_jobs), timeout=18.0)
            except asyncio.TimeoutError:
                logger.warning("v3.4 enrichment timeout for %s", url)
            except Exception:
                logger.exception("v3.4 enrichment failed for %s", url)
            best_jobs = self._dedupe_jobs_v34(best_jobs, url)

        best_jobs = self._postprocess_jobs_v34(best_jobs, url)
    ...
    def _extract_heading_rows_v34(self, root: etree._Element, page_url: str) -> list[dict]:
        ...
        for container in containers[:280]:
            ...
            for row in row_candidates[:80]:
                ...
                if not heading_nodes:
                    continue
                title = self._normalize_title_v34(_text(heading_nodes[0]))
                if not self._is_valid_title_v34(title):
                    continue
                ...
                if self._is_non_job_url_v34(source_url):
                    source_url = page_url
                ...
                job_url_hint = self._is_job_like_url_v34(source_url) or bool(_JOB_DETAILISH_URL_PATTERN_V33.search(source_url))
                ...
                has_role = self._title_has_role_signal_v34(title)
                if not has_role and not (
                    (job_url_hint and len(title.split()) >= 2)
                    or (job_url_hint and self._is_acronym_title_v34(title))
                ):
                    continue

                location = self._extract_location_v34(row, title)
                description = self._clean_description_v34(row_text)
                ...
                local_jobs.append(
                    {
                        ...
                        "salary_raw": self._extract_salary_v34(row_text),
                        "employment_type": self._extract_employment_type_v34(row_text),
                        "description": description,
                        "extraction_method": "tier2_heading_rows_v34",
        ...
        jobs = self._dedupe_jobs_v34(jobs, page_url)
        if not self._passes_jobset_validation_v34(jobs, page_url):
            return []
        return jobs

    def _extract_prose_heading_blocks_v34(self, root: etree._Element, page_url: str) -> list[dict]:
        ...
        for container in containers[:160]:
            ...
            for heading in headings:
                title = self._normalize_title_v34(_text(heading))
                if not self._is_valid_title_v34(title):
                    continue
                if not self._title_has_role_signal_v34(title):
                    continue
                ...
                if len(block_text) < 180:
                    continue
                if not self._prose_block_has_job_evidence_v34(block_text):
                    continue
                ...
                jobs.append(
                    {
                        "title": title,
                        "source_url": page_url,
                        "location_raw": self._extract_location_v34(container, title),
                        "salary_raw": self._extract_salary_v34(block_text),
                        "employment_type": self._extract_employment_type_v34(block_text),
                        "description": self._clean_description_v34(block_text),
                        "extraction_method": "tier2_prose_blocks_v34",
    ...
    def _extract_job_links_v34(self, root: etree._Element, page_url: str) -> list[dict]:
        jobs: list[dict] = []
        ...
        for a_el in anchors[:6000]:
            ...
            if not source_url or self._is_non_job_url_v34(source_url):
                continue
            ...
            title = self._normalize_title_v34(title_raw)
            if not self._is_valid_title_v34(title):
                continue
            ...
            job_url_hint = self._is_job_like_url_v34(source_url)
            same_page = source_url.rstrip("/") == page_url.rstrip("/")
            ...
            jobs.append(
                {
                    "title": title,
                    "source_url": source_url,
                    "location_raw": self._extract_location_v34(context_el, title),
                    "salary_raw": self._extract_salary_v34(context_text),
                    "employment_type": self._extract_employment_type_v34(context_text),
                    "description": self._clean_description_v34(context_text),
                    "extraction_method": "tier2_job_links_v34",
    ...
    def _extract_repeating_row_groups_v34(self, root: etree._Element, page_url: str) -> list[dict]:
        ...
        for rows in groups.values():
            ...
            for row in rows[:MAX_JOBS_PER_PAGE]:
                ...
                title = self._normalize_title_v34(job.get("title") or "")
                ...
                if not (job_url_hint or apply_hint or len(row_text) >= 180):
                    continue
                if not has_role and not (apply_hint or (job_url_hint and self._is_acronym_title_v34(title))):
                    continue
                ...
                jobs.append(
                    {
                        "title": title,
                        "source_url": source_url,
                        "location_raw": job.get("location_raw") or self._extract_location_v34(row, title),
                        "salary_raw": job.get("salary_raw") or self._extract_salary_v34(row_text),
                        "employment_type": job.get("employment_type") or self._extract_employment_type_v34(row_text),
                        "description": self._clean_description_v34(row_text),
                        "extraction_method": "tier2_row_groups_v34",
    ...
    def _extract_elementor_cards_v34(self, root: etree._Element, page_url: str) -> list[dict]:
        cards = root.xpath(
        ...
        for card in cards[:320]:
            ...
            for a_el in card.xpath(".//a[@href]"):
                ...
                if not href or self._is_non_job_url_v34(href):
                    continue
                if self._is_job_like_url_v34(href):
                    source_url = href
                    break
            ...
            jobs.append(
                {
                    "title": title,
                    "source_url": source_url,
                    "location_raw": self._extract_location_v34(card, title),
                    "salary_raw": self._extract_salary_v34(card_text),
                    "employment_type": self._extract_employment_type_v34(card_text),
                    "description": self._clean_description_v34(card_text),
                    "extraction_method": "tier2_elementor_cards_v34",
    ...
    def _extract_jsonld_jobs_v34(self, html_body: str, page_url: str) -> list[dict]:
        jobs: list[dict] = []
        for match in re.finditer(
            ...
            try:
                ...
                continue
            jobs.extend(
                self._extract_jobs_from_json_obj_v34(
                    parsed,
                    page_url,
                    method="tier0_jsonld_v34",
    ...
    async def _extract_martian_jobs_v34(self, page_url: str, html_body: str) -> list[dict]:
        ...
        context = self._extract_martian_context_v34(html_body, page_url)
        client_code = context.get("client_code") or ""
        if not client_code:
            return []

        endpoints = self._martian_probe_urls_v34(page_url, context)
        if not endpoints:
            return []
        ...
        try:
            async with httpx.AsyncClient(
                ...
                for endpoint in endpoints:
                    for probe_url in self._martian_paged_variants_v34(endpoint):
                        ...
                        extracted = self._extract_jobs_from_probe_payload_v34(resp.text, str(resp.url), page_url)
                        if extracted:
                ...
                if request_count < max_requests and len(jobs) < MIN_JOBS_FOR_SUCCESS:
                    post_endpoints = self._martian_post_endpoints_v34(endpoints)
                    payloads = self._martian_post_payloads_v34(context)
            ...
            logger.debug("v3.4 martian probing failed for %s", page_url)
    ...
    async def _extract_oracle_jobs_v34(self, page_url: str, html_body: str) -> list[dict]:
        ...
        site_ids = self._oracle_site_ids_v34(page_url, html_body)
        if not site_ids:
            return []
                ...
                for site_id in site_ids[:10]:
                    ...
                    for offset in range(0, 264, 24):
                        ...
                        for finder in finder_variants:
                            ...
                            batch = self._extract_oracle_items_v34(data, page_url, site_id)
                            if not batch:
                                continue
                    ...
                    deduped = self._dedupe_jobs_v34(site_jobs, page_url)
                    if deduped:
                        site_candidates.append((site_id, deduped))
        except Exception:
            logger.debug("v3.4 oracle probing failed for %s", page_url)
    ...
    def _extract_jobs_from_probe_payload_v34(self, body: str, response_url: str, page_url: str) -> list[dict]:
        ...
        if payload.startswith("{") or payload.startswith("["):
            try:
                parsed = json.loads(payload)
                jobs.extend(self._extract_jobs_from_json_obj_v34(parsed, response_url, method="tier0_api_json_v34"))
            except Exception:
                pass

        root = _parse_html(payload)
        if root is not None:
            jobs.extend(self._extract_job_links_v34(root, response_url))
            jobs.extend(self._extract_elementor_cards_v34(root, response_url))
            jobs.extend(self._extract_heading_rows_v34(root, response_url))
            jobs.extend(self._extract_repeating_row_groups_v34(root, response_url))
            ...
            for job in jobs:
                ...
                if method.startswith("tier2_"):
                    job["extraction_method"] = f"{method}_probe_v34"

        return self._dedupe_jobs_v34(jobs, page_url)

    def _extract_jobs_from_json_obj_v34(
        self,
        data: object,
        page_url: str,
        method: str,
        ...
        while queue and visited < 7000:
            ...
            job = self._job_from_json_dict_v34(node, page_url, method, require_job_type=require_job_type)
            if job:
                jobs.append(job)
                if len(jobs) >= MAX_JOBS_PER_PAGE:
                    break
    ...
    def _job_from_json_dict_v34(
        ...
        for key in (
            ...
            if isinstance(value, str) and value.strip():
                title = value.strip()
                break
        title = self._normalize_title_v34(title)
        if not self._is_valid_title_v34(title):
            return None
        ...
        has_role = self._title_has_role_signal_v34(title) or self._is_acronym_title_v34(title)
        url_hint = self._is_job_like_url_v34(source_url)
        ...
        description = ""
        for key in ("description", "summary", "shortDescription", "jobDescription", "Description", "ExternalDescriptionStr"):
            value = node.get(key)
            if isinstance(value, str) and value.strip():
                description = value.strip()
                break
        description = self._clean_description_v34(description)

        location = self._extract_location_from_json_v34(node)
        salary = self._extract_salary_from_json_v34(node)
        employment_type = self._extract_employment_from_json_v34(node)
    ...
    def _pick_best_jobset_v34(
        ...
        for label, jobs in candidates:
            deduped = self._dedupe_jobs_v34(jobs, page_url)
            if not deduped:
                continue

            valid = self._passes_jobset_validation_v34(deduped, page_url)
            score = self._jobset_score_v34(deduped, page_url)
            logger.debug("v3.4 candidate %s: jobs=%d valid=%s score=%.2f", label, len(deduped), valid, score)
        ...
        if not scored:
            ...
            largest = max(
                ((label, self._dedupe_jobs_v34(jobs, page_url)) for label, jobs in candidates),
        ...
        for label, jobs, score in scored:
            if label == best_label:
                continue
            overlap = self._title_overlap_ratio_v34(jobs, best_jobs)
        ...
        if parent_jobs and best_label != "parent_v16":
            overlap = self._title_overlap_ratio_v34(best_jobs, parent_jobs)
    ...
    def _passes_jobset_validation_v34(self, jobs: list[dict], page_url: str) -> bool:
        if not jobs:
            return False

        titles = [self._normalize_title_v34(j.get("title", "")) for j in jobs]
        titles = [t for t in titles if self._is_valid_title_v34(t)]
        if not titles:
            return False

        if len(titles) < MIN_JOBS_FOR_SUCCESS:
            return self._passes_small_high_precision_jobset_v34(jobs, page_url, titles)

        unique_ratio = len(set(t.lower() for t in titles)) / max(1, len(titles))
        if len(titles) > 3 and unique_ratio < 0.62:
            return False

        role_hits = sum(1 for t in titles if self._title_has_role_signal_v34(t))
        ...
        url_hits = sum(1 for j in jobs if self._is_job_like_url_v34(j.get("source_url") or page_url))
        ...
        same_page_evidence_hits = sum(1 for j in jobs if self._has_same_page_evidence_v34(j, page_url))
        strong_evidence_hits = sum(1 for j in jobs if self._job_has_strong_evidence_v34(j, page_url))
        same_page_hits = sum(
    ...
    def _jobset_score_v34(self, jobs: list[dict], page_url: str) -> float:
        if not jobs:
            return 0.0
    ...
    def _passes_small_high_precision_jobset_v34(self, jobs: list[dict], page_url: str, titles: list[str]) -> bool:
        ...
        strong_hits = sum(1 for j in jobs if self._job_has_strong_evidence_v34(j, page_url))
        return strong_hits >= len(titles)

    def _prose_block_has_job_evidence_v34(self, text: str) -> bool:
        desc = (text or "").strip()
        if len(desc) < 150:
            return False
        lower = desc.lower()
        ...
        has_rich = bool(_RICH_CONTEXT_PATTERN_V34.search(desc))
        return (has_apply and has_rich) or (has_salary and has_rich) or (has_type and has_rich)

    def _has_same_page_evidence_v34(self, job: dict, page_url: str) -> bool:
        source_url = (job.get("source_url") or page_url).strip() or page_url
        same_page = source_url.rstrip("/") == page_url.rstrip("/")
        if not same_page:
            return False
        ...
        has_rich = len(desc.strip()) >= 180 and bool(_RICH_CONTEXT_PATTERN_V34.search(desc_l))
        return has_apply or (has_structured and has_rich) or len(desc.strip()) >= 380

    def _job_has_strong_evidence_v34(self, job: dict, page_url: str) -> bool:
        source_url = (job.get("source_url") or page_url).strip() or page_url
        if self._is_non_job_url_v34(source_url):
            return False

        same_page = source_url.rstrip("/") == page_url.rstrip("/")
        title = self._normalize_title_v34(job.get("title", ""))
        if _JOB_DETAILISH_URL_PATTERN_V33.search(source_url):
            return True
        if self._is_job_like_url_v34(source_url) and not same_page:
            return True
        if not same_page and title and self._title_has_role_signal_v34(title):
            ...
            if tokens:
                ...
                if leaf not in {"careers", "career", "jobs", "job", "vacancies", "vacancy", "openings"}:
                    ...
                    if slug_words and (slug_words & title_words):
                        return True
        return self._has_same_page_evidence_v34(job, page_url)

    def _title_overlap_ratio_v34(self, a_jobs: list[dict], b_jobs: list[dict]) -> float:
        a_titles = {self._normalize_title_v34(j.get("title", "")).lower() for j in a_jobs if j.get("title")}
        b_titles = {self._normalize_title_v34(j.get("title", "")).lower() for j in b_jobs if j.get("title")}
        if not a_titles or not b_titles:
            return 0.0
        return len(a_titles & b_titles) / max(1, min(len(a_titles), len(b_titles)))

    def _dedupe_jobs_v34(self, jobs: list[dict], page_url: str) -> list[dict]:
        deduped: list[dict] = []
        ...
        for raw in jobs:
            ...
            title = self._normalize_title_v34(raw.get("title", ""))
            if not self._is_title_acceptable_for_url_v34(title, source_url):
                continue
            ...
            cloned["description"] = self._clean_description_v34(str(cloned.get("description") or ""))
    ...
    def _postprocess_jobs_v34(self, jobs: list[dict], page_url: str) -> list[dict]:
        cleaned: list[dict] = []
        for raw in jobs:
            title = self._normalize_title_v34(raw.get("title", ""))
        ...
        return self._dedupe_jobs_v34(cleaned, page_url)

    # ------------------------------------------------------------------
    # Normalization / validation helpers
    # ------------------------------------------------------------------

    def _normalize_title_v34(self, title: str) -> str:
        ...
        t = self._strip_location_suffix_v34(t, raw)
        return t.strip(" \t\r\n-–|:;,.")

    def _is_valid_title_v34(self, title: str) -> bool:
        if not title:
            return False
        t = title.strip()
        if len(t) < 4 or len(t) > 200:
            return False
        if len(t) < 5 and not self._is_acronym_title_v34(t):
            return False
        ...
        if _BOILERPLATE_BOUNDARY_PATTERN_V33.search(lower):
            ...
            if not self._title_has_role_signal_v34(t):
                return False
        if _NOISY_CTA_TITLE_PATTERN_V34.search(lower):
            return False
        if _COMPANY_CAREER_LABEL_PATTERN_V34.match(lower) and not self._title_has_role_signal_v34(t):
            return False
    ...
    def _title_has_role_signal_v34(self, title: str) -> bool:
        if not title:
            return False
        return _title_has_job_noun(title) or bool(_ROLE_HINT_PATTERN_V33.search(title)) or self._is_acronym_title_v34(title)

    def _is_title_acceptable_for_url_v34(self, title: str, source_url: str) -> bool:
        if self._is_valid_title_v34(title):
            return True
        return self._is_acronym_title_v34(title) and self._is_job_like_url_v34(source_url)

    @staticmethod
    def _is_acronym_title_v34(title: str) -> bool:
        t = (title or "").strip()
        if not _ACRONYM_TITLE_PATTERN_V33.match(t):
            return False
        return t.lower() not in {"faq", "home", "menu", "login", "logout"}

    def _strip_location_suffix_v34(self, normalized: str, raw: str) -> str:
        ...
        if re.search(r"\s+OR\s+", text, re.IGNORECASE):
            ...
            if before_or and self._title_has_role_signal_v34(before_or):
                text = re.sub(r"([a-z])([A-Z][a-z]{2,})$", r"\1 \2", before_or)
                had_or_segment = True

        words = text.split()
        if len(words) < 2 or not self._title_has_role_signal_v34(text):
            return text

        if had_or_segment:
            tail = words[-1]
            if not self._title_has_role_signal_v34(tail) and len(tail) >= 3:
                candidate = " ".join(words[:-1]).strip()
                if candidate and self._title_has_role_signal_v34(candidate):
                    return candidate
        ...
        for cut in range(len(words) - 1, 0, -1):
            ...
            if not head or not tail:
                continue
            if not self._title_has_role_signal_v34(head):
                continue
            if self._title_has_role_signal_v34(tail):
                continue
            if len(tail.split()) > 5:
                continue
    ...
    def _is_job_like_url_v34(self, url: str) -> bool:
        value = (url or "").strip()
        if not value:
            return False
    ...
    def _is_non_job_url_v34(self, url: str) -> bool:
    ...
    def _clean_description_v34(self, description: str) -> Optional[str]:
    ...
    def _extract_location_v34(self, row: etree._Element, title: str) -> Optional[str]:
        for el in row.iter():
            if not isinstance(el.tag, str):
                continue
    ...
    @staticmethod
    def _extract_salary_v34(text: str) -> Optional[str]:
        if not text:
            return None
    ...
    @staticmethod
    def _extract_employment_type_v34(text: str) -> Optional[str]:
        if not text:
            return None
    ...
    def _extract_location_from_json_v34(self, node: dict[str, Any]) -> Optional[str]:
    ...
    def _extract_salary_from_json_v34(self, node: dict[str, Any]) -> Optional[str]:
    ...
    def _extract_employment_from_json_v34(self, node: dict[str, Any]) -> Optional[str]:
    ...
    def _extract_martian_context_v34(self, html_body: str, page_url: str) -> dict[str, str]:
        result = {
            "client_code": "",
            "recruiter_id": "",
            "job_board_theme_id": "",
    ...
    def _martian_probe_urls_v34(self, page_url: str, context: dict[str, str]) -> list[str]:
        ...
        if next_page:
            ...
            if cleaned_page:
                paths.extend([f"/{cleaned_page}", f"/{cleaned_page}/jobs"])
        ...
        for host in hosts:
            ...
            candidates.extend(self._next_data_candidate_urls_v34(host, page_url, context, client_code))
    ...
    def _next_data_candidate_urls_v34(
        self,
        host: str,
        page_url: str,
    ...
    @staticmethod
    def _martian_paged_variants_v34(endpoint: str) -> list[str]:
        variants = [endpoint]
    ...
    @staticmethod
    def _martian_post_endpoints_v34(endpoints: list[str]) -> list[str]:
        postable: list[str] = []
        seen: set[str] = set()
        for endpoint in endpoints:
            low = endpoint.lower()
    ...
    @staticmethod
    def _martian_post_payloads_v34(context: dict[str, str]) -> list[dict[str, Any]]:
        client_code = (context.get("client_code") or "").strip()
        recruiter_id = (context.get("recruiter_id") or "").strip()
        theme_id = (context.get("job_board_theme_id") or "").strip()
        board_name = (context.get("board_name") or "").strip()
    ...
    def _oracle_site_ids_v34(self, page_url: str, html_body: str) -> list[str]:
        seen: set[str] = set()
        ordered: list[str] = []
    ...
    def _extract_oracle_items_v34(self, data: Any, page_url: str, site_id: str) -> list[dict]:
        ...
        for row in rows:
            title = self._normalize_title_v34(
                str(
                    row.get("Title")
                    or row.get("title")
                    or row.get("JobTitle")
            ...
            if not self._is_valid_title_v34(title):
                continue
            if not (self._title_has_role_signal_v34(title) or self._is_acronym_title_v34(title)):
                continue
            ...
            location = self._extract_location_from_json_v34(row)
            if not location:
            ...
            description = self._clean_description_v34(
                str(
                    row.get("Description")
            ...
            jobs.append(
                {
                    ...
                    "source_url": source_url,
                    "location_raw": location,
                    "salary_raw": self._extract_salary_from_json_v34(row),
                    "employment_type": self._extract_employment_from_json_v34(row),
                    "description": description,
                    "extraction_method": "tier0_oracle_api_v34",
