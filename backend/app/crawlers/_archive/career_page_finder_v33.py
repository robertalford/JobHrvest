"""
Career Page Finder v3.3 — direct from CareerPageFinderV4.

v3.3 keeps the proven v3.0 discovery behavior and adds a targeted Oracle
...
class CareerPageFinderV33(CareerPageFinderV4):
    """v3.3 finder wrapper with Oracle tenant-path tie-break recovery."""
    ...
    async def find(self, domain: str, company_name: str = "") -> dict:
        ...
        improved = await self._oracle_tenant_recovery_v33(delegate, disc)
        return improved or disc

    async def _oracle_tenant_recovery_v33(self, delegate: CareerPageFinderV30, disc: dict) -> dict | None:
        ...
        site_ids = self._oracle_site_ids_v33(current_url, current_html)
        if not site_ids:
            return None
            ...
            async with httpx.AsyncClient(timeout=8, follow_redirects=True, headers=_CLIENT_HEADERS) as client:
                for candidate_url, candidate_site in candidate_urls:
                    ...
                    if should_upgrade:
                        ...
                        best = {
                            "url": str(resp.url),
                            "method": disc.get("method", "") + f"+oracle_tenant_v33:{candidate_site}",
                            "candidates": disc.get("candidates", []) + [candidate_url],
                            "html": body,
                        }
        except Exception:
            logger.debug("v3.3 oracle tenant recovery failed for %s", current_url)

        return best

    @staticmethod
    def _oracle_site_ids_v33(page_url: str, html_body: str) -> list[str]:
        seen: set[str] = set()
        ordered: list[str] = []
