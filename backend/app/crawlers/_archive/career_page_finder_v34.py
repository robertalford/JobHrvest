"""
Career Page Finder v3.4 — direct from CareerPageFinderV4.

v3.4 keeps the proven v3.0 discovery behavior and adds a targeted Oracle
tenant recovery pass to prefer richer tenant-specific CandidateExperience
paths (e.g. CX_1001 requisitions) over generic CX shells.
"""
...
class CareerPageFinderV34(CareerPageFinderV4):
    """v3.4 finder wrapper with Oracle tenant-path tie-break recovery."""
    ...
    async def find(self, domain: str, company_name: str = "") -> dict:
        ...
        canonical_upgrade = await self._canonical_listing_upgrade_v34(delegate, disc)
        ...
        improved = await self._oracle_tenant_recovery_v34(delegate, disc)
        return improved or disc

    async def _canonical_listing_upgrade_v34(self, delegate: CareerPageFinderV30, disc: dict) -> dict | None:
        ...
        return {
            "url": str(resp.url),
            "method": disc.get("method", "") + "+canonical_v34",
            "candidates": disc.get("candidates", []) + [canonical_url],
            "html": body,
        }

    async def _oracle_tenant_recovery_v34(self, delegate: CareerPageFinderV30, disc: dict) -> dict | None:
        current_url = disc.get("url", "")
        current_html = disc.get("html") or ""
        ...
        site_ids = self._oracle_site_ids_v34(current_url, current_html)
        if not site_ids:
            return None
            ...
            async with httpx.AsyncClient(timeout=8, follow_redirects=True, headers=_CLIENT_HEADERS) as client:
                for candidate_url, candidate_site in candidate_urls:
                    ...
                    if should_upgrade:
                        ...
                        best = {
                            "url": str(resp.url),
                            "method": disc.get("method", "") + f"+oracle_tenant_v34:{candidate_site}",
                            "candidates": disc.get("candidates", []) + [candidate_url],
                            "html": body,
                        }
        except Exception:
            logger.debug("v3.4 oracle tenant recovery failed for %s", current_url)

        return best

    @staticmethod
    def _oracle_site_ids_v34(page_url: str, html_body: str) -> list[str]:
        seen: set[str] = set()
        ordered: list[str] = []
