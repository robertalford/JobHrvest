"""
Tiered Extraction Engine v4.7 — direct from v1.6.
...
_ROLE_HINT_PATTERN_V46 = re.compile(
    ...
    r"fashion|full\s*stack|fullstack|devops|owner|customer\s+service|"
    r"crew|yardman|activator)\b",
    re.IGNORECASE,
)
...
_HEADING_BLOCK_STOP_PATTERN_V47 = re.compile(r"^h[1-4]$", re.IGNORECASE)

_HEADING_TITLE_REJECT_PATTERN_V47 = re.compile(
    r"^(?:internship\s+details|job\s+description|position\s+description|role\s+description)$",
    re.IGNORECASE,
)

_DOCUMENT_VACANCY_TEXT_PATTERN_V47 = re.compile(
    r"(?:job\s+description|position\s+description|role\s+description|"
    r"candidate\s+information|vacancy)",
    re.IGNORECASE,
)

_MARTIAN_HOST_HINT_PATTERN_V47 = re.compile(
    r"https?://([a-z0-9.-]*(?:martianlogic|myrecruitmentplus)\.[a-z.]{2,})(?:/|[\"'])",
    re.IGNORECASE,
)


class TieredExtractorV47(TieredExtractorV16):
    """v4.7 extractor with v1.6-first fallback arbitration and focused shell recovery."""
    ...
    async def extract(self, career_page, company, html: str) -> list[dict]:
        ...
        recovered_html = await self._recover_short_html_v47(page_url, working_html)
        if recovered_html:
        ...
        try:
            ...
            logger.warning("v4.7 parent extractor timeout for %s", page_url)
        except Exception:
            logger.exception("v4.7 parent extractor failed for %s", page_url)

        parent_jobs = self._prepare_candidate_jobs_v47(parent_jobs or [], page_url)
        if parent_jobs:
            candidates.append(("parent_v16", parent_jobs))

        local_jobs = self._extract_local_jobs_v47(page_url, working_html)
        if local_jobs:
            candidates.append(("local_v47", local_jobs))

        structured_jobs = self._extract_state_and_jsonld_jobs_v47(working_html, page_url)
        if structured_jobs:
            candidates.append(("structured_v47", structured_jobs))

        best_label, best_jobs, best_score = self._pick_best_candidate_v47(candidates, page_url)

        if self._needs_deep_recovery_v47(page_url, working_html, best_jobs, best_score):
            martian_jobs = await self._extract_martian_jobs_v47(page_url, working_html)
            if martian_jobs:
                candidates.append(("martian_v47", martian_jobs))

            rss_jobs = await self._extract_rss_jobs_v47(page_url, working_html)
            if rss_jobs:
                candidates.append(("rss_v47", rss_jobs))

            subpage_jobs = await self._follow_listing_subpages_v47(page_url, working_html)
            if subpage_jobs:
                candidates.append(("subpage_follow_v47", subpage_jobs))

            iframe_jobs = await self._extract_iframe_jobs_v47(page_url, working_html)
            if iframe_jobs:
                candidates.append(("iframe_v47", iframe_jobs))
        ...
        if self._should_enrich_detail_pages_v47(best_jobs, page_url):
            try:
                best_jobs = await asyncio.wait_for(self._enrich_from_detail_pages(best_jobs), timeout=16.0)
            except asyncio.TimeoutError:
                logger.warning("v4.7 enrichment timeout for %s", page_url)
            except Exception:
                logger.exception("v4.7 enrichment failed for %s", page_url)

        final_jobs = self._prepare_candidate_jobs_v47(best_jobs, page_url)
        return final_jobs[:MAX_JOBS_PER_PAGE]

    async def _recover_short_html_v47(self, page_url: str, html_body: str) -> Optional[str]:
        body = html_body or ""
        shell_like = self._looks_like_shell_v47(page_url, body)
        ...
        best_html = body
        best_score = self._page_listing_score_v47(page_url, body)
        seen: set[str] = set()
        ...
        async with httpx.AsyncClient(
            ...
            for probe in probes:
                ...
                if resp.status_code != 200 or len(text) < 200:
                    continue
                if self._looks_non_html_payload_v47(text):
                    continue

                score = self._page_listing_score_v47(str(resp.url), text)
    ...
    def _extract_local_jobs_v47(self, page_url: str, html_body: str) -> list[dict]:
        ...
        if root is None:
            _label, jobs, _score = self._pick_best_candidate_v47(candidates, page_url)
            return jobs

        utility_rows = self._extract_utility_card_rows_v47(root, page_url)
        if utility_rows:
            candidates.append(("utility_rows_v47", utility_rows))

        elementor_cards = self._extract_elementor_cards_v47(root, page_url)
        if elementor_cards:
            candidates.append(("elementor_cards_v47", elementor_cards))

        cta_cards = self._extract_heading_cta_cards_v47(root, page_url)
        if cta_cards:
            candidates.append(("heading_cta_v47", cta_cards))

        greenhouse_posts = self._extract_greenhouse_posts_v47(root, page_url)
        if greenhouse_posts:
            candidates.append(("greenhouse_posts_v47", greenhouse_posts))

        job_links = self._extract_job_links_v47(root, page_url)
        if job_links:
            candidates.append(("job_links_v47", job_links))

        heading_blocks = self._extract_heading_blocks_v47(root, page_url)
        if heading_blocks:
            candidates.append(("heading_blocks_v47", heading_blocks))

        doc_links = self._extract_document_vacancy_links_v47(root, page_url)
        if doc_links:
            candidates.append(("document_links_v47", doc_links))

        _label, jobs, _score = self._pick_best_candidate_v47(candidates, page_url)
        return jobs

    def _extract_state_and_jsonld_jobs_v47(self, html_body: str, page_url: str) -> list[dict]:
        ...
        for match in _NEXT_DATA_PATTERN_V46.finditer(html_body):
            ...
            try:
                ...
                continue
            state_jobs.extend(self._extract_jobs_from_json_payload_v47(parsed, page_url, "tier0_next_data_v47"))
        ...
        for match in re.finditer(
            ...
            try:
                ...
                continue
            state_jobs.extend(self._extract_jobs_from_json_payload_v47(parsed, page_url, "tier0_state_json_v47"))

        if state_jobs:
            candidates.append(("state_json_v47", state_jobs))
        ...
        for match in re.finditer(
            ...
            for node in self._iter_json_dicts_v47(parsed):
                ...
                title = self._normalize_title_v47(str(node.get("title") or node.get("name") or ""))
                if not self._is_title_acceptable_v47(title, str(node.get("url") or "")):
                    continue
                ...
                if self._is_non_job_url_v47(source_url):
                    continue

                location = self._extract_location_from_json_v47(node)
                description = self._clean_description_v47(
                    str(node.get("description") or node.get("responsibilities") or node.get("qualifications") or "")
                )
                salary_raw = self._extract_salary_v47(json.dumps(node, ensure_ascii=True, default=str))
                employment_type = str(node.get("employmentType") or "").strip() or None

                evidence = (
                    self._is_job_like_url_v47(source_url)
                    or bool(location)
                    or bool(salary_raw)
                ...
                jsonld_jobs.append(
                    {
                        ...
                        "extraction_method": "tier0_jsonld_v47",
                        "extraction_confidence": 0.84 if self._is_job_like_url_v47(source_url) else 0.78,
                    }
                )

        if jsonld_jobs:
            candidates.append(("jsonld_v47", jsonld_jobs))

        _label, best_jobs, _score = self._pick_best_candidate_v47(candidates, page_url)
        return best_jobs

    async def _extract_martian_jobs_v47(self, page_url: str, html_body: str) -> list[dict]:
        if not self._looks_like_martian_shell_v47(html_body):
            return []

        context = self._extract_martian_context_v47(page_url, html_body)
        if not (context.get("client_code") or context.get("recruiter_id") or context.get("build_id")):
            return []

        endpoints = self._martian_probe_urls_v47(page_url, context)
        if not endpoints:
            return []
        endpoint_plan = self._interleave_endpoints_by_host_v47(endpoints, page_url, context)
        ...
        async with httpx.AsyncClient(
            ...
            for endpoint in endpoint_plan[:36]:
                if request_budget <= 0:
                    break
                ...
                request_budget -= 1
                try:
                    resp = await asyncio.wait_for(client.get(endpoint), timeout=2.9)
                except Exception:
                    continue
                if resp.status_code >= 400 or not resp.text:
                    continue

                extracted = self._extract_jobs_from_probe_payload_v47(resp.text, str(resp.url), page_url)
                if extracted:
            ...
            if len(jobs) < MIN_JOBS_FOR_SUCCESS and request_budget > 0:
                post_endpoints = self._martian_post_endpoints_v47(endpoints)
                post_plan = self._interleave_endpoints_by_host_v47(post_endpoints, page_url, context)
                payloads = self._martian_post_payloads_v47(context)
                for endpoint in post_plan[:12]:
                    for payload in payloads[:5]:
                        if request_budget <= 0:
                            break
                        request_budget -= 1
                        try:
                            resp = await asyncio.wait_for(client.post(endpoint, json=payload), timeout=2.9)
                        except Exception:
                            continue
                        if resp.status_code >= 400 or not resp.text:
                            continue
        ...
        return self._prepare_candidate_jobs_v47(jobs, page_url)

    async def _extract_rss_jobs_v47(self, page_url: str, html_body: str) -> list[dict]:
        if not html_body:
            return []
        ...
        async with httpx.AsyncClient(
            ...
            for feed_url in feed_urls[:3]:
                ...
                for row in rows[:MAX_JOBS_PER_PAGE]:
                    title = self._normalize_title_v47(
                        " ".join(row.xpath("./title//text()") or []).strip()
                    )
                    if not self._is_title_acceptable_v47(title, ""):
                        continue
                    ...
                    description = self._clean_description_v47(desc)
                    if not (self._is_job_like_url_v47(source_url) or (description and len(description) >= 120)):
                        continue
                    ...
                    jobs.append(
                        {
                            ...
                            "source_url": source_url,
                            "location_raw": None,
                            "salary_raw": self._extract_salary_v47(desc),
                            "employment_type": self._extract_employment_type_v47(desc),
                            "description": description,
                            "extraction_method": "tier0_rss_v47",
    ...
    async def _follow_listing_subpages_v47(self, page_url: str, html_body: str) -> list[dict]:
        root = _parse_html(html_body)
        if root is None:
            return []
        ...
        for subpage_url, _score in candidates[:10]:
            if fetched >= 4:
                break
            sub_html = await self._fetch_html_v47(subpage_url)
            if not sub_html:
                continue
            fetched += 1

            local_jobs = self._extract_local_jobs_v47(subpage_url, sub_html)
            if local_jobs:
                jobsets.append((f"sub_local_{fetched}", local_jobs))

            structured = self._extract_state_and_jsonld_jobs_v47(sub_html, subpage_url)
            if structured:
                jobsets.append((f"sub_structured_{fetched}", structured))

        _label, best_jobs, _score = self._pick_best_candidate_v47(jobsets, page_url)
        return best_jobs

    async def _extract_iframe_jobs_v47(self, page_url: str, html_body: str) -> list[dict]:
        root = _parse_html(html_body)
        if root is None:
            return []
        ...
        for iframe_url in iframe_urls[:4]:
            iframe_html = await self._fetch_html_v47(iframe_url)
            if not iframe_html:
                continue
            fetched += 1

            local_jobs = self._extract_local_jobs_v47(iframe_url, iframe_html)
            if local_jobs:
                jobsets.append((f"iframe_local_{fetched}", local_jobs))

            structured = self._extract_state_and_jsonld_jobs_v47(iframe_html, iframe_url)
            if structured:
    ...
    async def _fetch_html_v47(self, url: str) -> Optional[str]:
        try:
            async with httpx.AsyncClient(
                timeout=7,
                follow_redirects=True,
        ...
        body = resp.text or ""
        if resp.status_code != 200 or len(body) < 200 or self._looks_non_html_payload_v47(body):
            return None
        return body

    def _extract_jobs_from_probe_payload_v47(self, body: str, response_url: str, page_url: str) -> list[dict]:
        ...
        if payload.startswith("{") or payload.startswith("["):
            ...
            if parsed is not None:
                jobs.extend(self._extract_jobs_from_json_payload_v47(parsed, response_url, "tier0_probe_json_v47"))
        ...
        if root is not None:
            ...
            if anchor_count >= 2 or marker_hit:
                jobs.extend(self._extract_local_jobs_v47(response_url, payload))

        return self._prepare_candidate_jobs_v47(jobs, page_url)

    def _extract_jobs_from_json_payload_v47(self, payload: Any, page_url: str, method: str) -> list[dict]:
        jobs: list[dict] = []

        for row in self._iter_json_dicts_v47(payload):
            title = self._normalize_title_v47(
                str(
                    row.get("title")
                    or row.get("Title")
            ...
            if not self._is_valid_title_v47(title):
                continue
            if not self._title_has_role_signal_v47(title):
                continue
            ...
            location = self._extract_location_from_json_v47(row)
            description = self._clean_description_v47(
                str(
                    row.get("description")
            ...
            salary_raw = self._extract_salary_v47(json.dumps(row, ensure_ascii=True, default=str))
            ...
            jobs.append(
                {
                    ...
                    "extraction_confidence": 0.82 if self._is_job_like_url_v47(source_url) else 0.74,
                }
            )
            if len(jobs) >= MAX_JOBS_PER_PAGE:
                break
    ...
    def _extract_martian_context_v47(self, page_url: str, html_body: str) -> dict[str, str]:
        context = {
            "client_code": "",
            "recruiter_id": "",
            "job_board_theme_id": "",
        ...
        host_hints: list[str] = []
        for match in _MARTIAN_HOST_HINT_PATTERN_V47.finditer(html_body or ""):
    ...
    def _martian_probe_urls_v47(self, page_url: str, context: dict[str, str]) -> list[str]:
        ...
        if hint_hosts:
            ats_hosts = [page_host, *hint_hosts, *ats_hosts[1:]]
        if client_code:
            ats_hosts.extend([
        ...
        for host in hosts:
            ...
            probe_urls.extend(self._next_data_probe_urls_v47(host, page_url, context))

        prioritized = sorted(
            {u.rstrip("/"): u for u in probe_urls if u}.values(),
            key=lambda u: self._martian_endpoint_priority_v47(u, parsed.netloc.lower(), path),
            reverse=True,
        )
        return prioritized[:68]

    def _next_data_probe_urls_v47(self, host: str, page_url: str, context: dict[str, str]) -> list[str]:
    ...
    def _martian_endpoint_priority_v47(self, url: str, page_host: str, page_path: str) -> int:
        low = (url or "").lower()
        parsed = urlparse(url)
    ...
    def _interleave_endpoints_by_host_v47(
        self,
        endpoints: list[str],
        page_url: str,
        ...
        for endpoint in endpoints:
            ...
            if not host:
                continue
            grouped[host].append(endpoint)

        host_order = sorted(
            grouped.keys(),
            key=lambda host: (
                -self._martian_host_rank_v47(host, page_host, hint_hosts),
                -len(grouped[host]),
                host,
            ),
        )
        ...
        # Keep each host list internally sorted by endpoint confidence.
        for host in host_order:
            grouped[host].sort(
                key=lambda url: self._martian_endpoint_priority_v47(url, page_host, urlparse(page_url).path.rstrip("/")),
                reverse=True,
            )

        interleaved: list[str] = []
        depth = max(len(grouped[host]) for host in host_order)
        for idx in range(depth):
            for host in host_order:
                if idx < len(grouped[host]):
                    interleaved.append(grouped[host][idx])
        return interleaved

    @staticmethod
    def _martian_host_rank_v47(host: str, page_host: str, hint_hosts: set[str]) -> int:
        rank = 0
        if host == page_host:
            rank += 7
        if host in hint_hosts:
            rank += 6
        if "web.martianlogic.com" in host:
            rank += 5
        if "myrecruitmentplus" in host:
            rank += 4
        if "martianlogic" in host:
            rank += 3
        return rank

    @staticmethod
    def _martian_post_endpoints_v47(endpoints: list[str]) -> list[str]:
        out: list[str] = []
        seen: set[str] = set()
        for endpoint in endpoints:
            low = endpoint.lower()
    ...
    @staticmethod
    def _martian_post_payloads_v47(context: dict[str, str]) -> list[dict[str, Any]]:
        client_code = (context.get("client_code") or "").strip()
        recruiter_id = (context.get("recruiter_id") or "").strip()
        theme_id = (context.get("job_board_theme_id") or "").strip()
    ...
    def _extract_utility_card_rows_v47(self, root: etree._Element, page_url: str) -> list[dict]:
        rows = root.xpath(
        ...
        for row in rows[:320]:
            ...
            title = self._normalize_title_v47(title_raw)
            if not self._is_valid_title_v47(title):
                continue
            if not self._title_has_role_signal_v47(title):
                continue
            ...
            for a_el in links:
                ...
                if not href or self._is_non_job_url_v47(href):
                    continue
                source_url = href
                if self._is_job_like_url_v47(href):
                    break

            row_text = _text(row)
            has_apply = bool(_APPLY_EVIDENCE_PATTERN_V46.search(row_text))
            if not (self._is_job_like_url_v47(source_url) or has_apply or len(row_text) >= 120):
                continue
            ...
            jobs.append(
                {
                    "title": title,
                    "source_url": source_url,
                    "location_raw": self._extract_location_v47(row, title),
                    "salary_raw": self._extract_salary_v47(row_text),
                    "employment_type": self._extract_employment_type_v47(row_text),
                    "description": self._clean_description_v47(row_text),
                    "extraction_method": "tier2_utility_cards_v47",
                    "extraction_confidence": 0.79 if self._is_job_like_url_v47(source_url) else 0.73,
                }
            )

        prepared = self._prepare_candidate_jobs_v47(jobs, page_url)
        return prepared if len(prepared) >= 2 else []

    def _extract_elementor_cards_v47(self, root: etree._Element, page_url: str) -> list[dict]:
        cards = root.xpath(
        ...
        for card in cards[:360]:
            ...
            if not heading_nodes:
                continue
            title = self._normalize_title_v47(_text(heading_nodes[0]))
            if not self._is_valid_title_v47(title):
                continue
            if not self._title_has_role_signal_v47(title):
                continue
            ...
            jobs.append(
                {
                    "title": title,
                    "source_url": source_url,
                    "location_raw": self._extract_location_v47(card, title),
                    "salary_raw": self._extract_salary_v47(card_text),
                    "employment_type": self._extract_employment_type_v47(card_text),
                    "description": self._clean_description_v47(card_text),
                    "extraction_method": "tier2_elementor_cards_v47",
                    "extraction_confidence": 0.76,
                }
            )
    ...
    def _extract_heading_cta_cards_v47(self, root: etree._Element, page_url: str) -> list[dict]:
        jobs: list[dict] = []
        for a_el in root.xpath("//a[@href]")[:5000]:
            ...
            if not source_url or self._is_non_job_url_v47(source_url):
                continue
            ...
            has_apply = bool(_APPLY_EVIDENCE_PATTERN_V46.search(row_text))
            if not (has_apply or self._is_job_like_url_v47(source_url) or len(row_text) >= 95):
                continue
            ...
            jobs.append(
                {
                    "title": title,
                    "source_url": source_url,
                    "location_raw": self._extract_location_v47(best_row, title),
                    "salary_raw": self._extract_salary_v47(row_text),
                    "employment_type": self._extract_employment_type_v47(row_text),
                    "description": self._clean_description_v47(row_text),
                    "extraction_method": "tier2_heading_cta_cards_v47",
                    "extraction_confidence": 0.77 if self._is_job_like_url_v47(source_url) else 0.72,
                }
            )
        ...
        if len(prepared) < 2:
            return []
        role_hits = sum(1 for j in prepared if self._title_has_role_signal_v47(j.get("title", "")))
        if role_hits < max(2, int(len(prepared) * 0.65)):
            return []
        return prepared

    def _extract_greenhouse_posts_v47(self, root: etree._Element, page_url: str) -> list[dict]:
        jobs: list[dict] = []
        for a_el in root.xpath("//a[@href]")[:5000]:
            href = (a_el.get("href") or "").strip()
            ...
            title = self._normalize_title_v47(_text(title_nodes[0])) if title_nodes else self._normalize_title_v47(_text(a_el))
            if not self._is_title_acceptable_v47(title, source_url):
                continue
            ...
            jobs.append(
                {
                    ...
                    "employment_type": self._extract_employment_type_v47(row_text),
                    "description": self._clean_description_v47(row_text),
                    "extraction_method": "tier2_greenhouse_posts_v47",
    ...
    def _extract_job_links_v47(self, root: etree._Element, page_url: str) -> list[dict]:
        jobs: list[dict] = []
        for a_el in root.xpath("//a[@href]")[:5000]:
            href = (a_el.get("href") or "").strip()
            if not href or href.startswith("#") or href.startswith("javascript:"):
            ...
            title = self._normalize_title_v47(raw_title)
            if not self._is_valid_title_v47(title):
                continue
            if not self._title_has_role_signal_v47(title):
                continue
            ...
            if len(title.split()) > 10 and not self._is_job_like_url_v47(source_url):
                continue
            if _LISTING_LINK_TEXT_PATTERN_V46.search(title) and not self._is_job_like_url_v47(source_url):
                continue
            if not (
                self._is_job_like_url_v47(source_url)
            ...
            jobs.append(
                {
                    "title": title,
                    "source_url": source_url,
                    "location_raw": self._extract_location_v47(context, title),
                    "salary_raw": self._extract_salary_v47(context_text),
                    "employment_type": self._extract_employment_type_v47(context_text),
                    "description": self._clean_description_v47(context_text),
                    "extraction_method": "tier2_job_links_v47",
                    "extraction_confidence": 0.73 if self._is_job_like_url_v47(source_url) else 0.67,
                }
            )
            if len(jobs) >= MAX_JOBS_PER_PAGE:
                break
    ...
    def _extract_heading_blocks_v47(self, root: etree._Element, page_url: str) -> list[dict]:
        jobs: list[dict] = []
        ...
        for heading in headings[:700]:
            title = self._normalize_title_v47(_text(heading))
            if not self._is_valid_title_v47(title):
                continue
            if not self._title_has_role_signal_v47(title):
                continue
            if _HEADING_TITLE_REJECT_PATTERN_V47.match(title):
                continue
            ...
            while sibling is not None and sibling_steps < 10:
                sibling_steps += 1
                if isinstance(sibling.tag, str) and _HEADING_BLOCK_STOP_PATTERN_V47.match(sibling.tag):
                    break
            ...
            description = self._clean_description_v47(detail_text)
            ...
            for a_el in link_els:
                ...
                if href.startswith("mailto:"):
                    ...
                    continue
                if self._is_job_document_url_v47(href, title, _text(a_el)):
                    has_doc_link = True
                    if source_url == page_url:
                        source_url = href
                    continue
                if self._is_non_job_url_v47(href):
                    continue
                if self._is_job_like_url_v47(href):
                    source_url = href
                    break
            ...
            evidence = (
                ...
                or has_doc_link
                or has_mailto_apply
                or bool(_APPLY_EVIDENCE_PATTERN_V46.search(detail_text))
                or (description is not None and len(description) >= 160)
                or bool(self._extract_salary_v47(detail_text))
                or bool(self._extract_employment_type_v47(detail_text))
            )
            if not evidence:
                continue
            ...
            jobs.append(
                {
                    "title": title,
                    "source_url": source_url,
                    "location_raw": self._extract_location_v47(heading.getparent() if heading.getparent() is not None else heading, title),
                    "salary_raw": self._extract_salary_v47(detail_text),
                    "employment_type": self._extract_employment_type_v47(detail_text),
                    "description": description,
                    "extraction_method": "tier2_heading_blocks_v47",
                    "extraction_confidence": 0.74 if (has_doc_link or has_mailto_apply) else 0.71,
                }
            )
    ...
    def _extract_document_vacancy_links_v47(self, root: etree._Element, page_url: str) -> list[dict]:
        jobs: list[dict] = []
        for a_el in root.xpath("//a[@href]")[:5000]:
            href = _resolve_url(a_el.get("href"), page_url)
            if not href:
            ...
            anchor_text = self._normalize_title_v47(_text(a_el))
            if not self._is_job_document_url_v47(href, anchor_text, anchor_text):
                continue

            title = anchor_text
            if "|" in title:
                title = self._normalize_title_v47(title.split("|", 1)[-1])
            title = re.sub(
                r"^(?:job|position|role)\s+description\s*[:\-]?\s*",
                "",
                title,
                flags=re.IGNORECASE,
            ).strip()
            title = self._normalize_title_v47(title)
            if not self._is_valid_title_v47(title):
                continue
            if not self._title_has_role_signal_v47(title):
                continue
            ...
            context_text = _text(row)
            if not (
                _DOCUMENT_VACANCY_TEXT_PATTERN_V47.search(anchor_text)
                or _APPLY_EVIDENCE_PATTERN_V46.search(context_text)
                or len(context_text) >= 160
            ):
                continue
            ...
            jobs.append(
                {
                    ...
                    "employment_type": self._extract_employment_type_v47(context_text),
                    "description": self._clean_description_v47(context_text),
                    "extraction_method": "tier2_document_links_v47",
    ...
    def _pick_best_candidate_v47(
        ...
        for label, jobs in candidates:
            ...
            if not prepared:
                continue
            score = self._candidate_score_v47(prepared, page_url)
        ...
        for label, jobs, score in scored:
            if label == best_label:
                continue
            overlap = self._title_overlap_ratio_v47(jobs, best_jobs)
    ...
    def _prepare_candidate_jobs_v47(self, jobs: list[dict], page_url: str) -> list[dict]:
        cleaned: list[dict] = []

        for idx, raw in enumerate(jobs):
            title = self._normalize_title_v47(raw.get("title", ""))
            source_url = _resolve_url(raw.get("source_url"), page_url) or page_url
            if "#" in source_url:
                source_url = source_url.split("#", 1)[0]

            desc = self._clean_description_v47(str(raw.get("description") or ""))
            is_doc_job = self._is_job_document_url_v47(source_url, title, desc or "")
            if self._is_non_job_url_v47(source_url) and not is_doc_job:
                continue
            if not self._is_title_acceptable_v47(title, source_url, allow_document=is_doc_job):
                continue
            ...
            cleaned.append(
                {
                    ...
                    "employment_type": raw.get("employment_type") or self._extract_employment_type_v47(desc or ""),
                    "description": desc,
                    "extraction_method": raw.get("extraction_method") or "tier2_v47",
                    "extraction_confidence": raw.get("extraction_confidence", 0.65),
                    "_order": idx,
                }
            )

        deduped = self._dedupe_jobs_v47(cleaned, page_url)
        if not self._is_valid_jobset_v47(deduped, page_url):
            return []
        return deduped

    def _dedupe_jobs_v47(self, jobs: list[dict], page_url: str) -> list[dict]:
        by_key: dict[tuple[str, str], dict] = {}
        by_url: dict[str, dict] = {}
        page_norm = (page_url or "").rstrip("/").lower()

        for job in jobs:
            title = self._normalize_title_v47(job.get("title", ""))
            url = _resolve_url(job.get("source_url"), page_url) or page_url
            key = (title.lower(), url.lower())
            existing = by_key.get(key)
            if existing is None or self._title_quality_score_v47(title) > self._title_quality_score_v47(existing.get("title", "")):
                by_key[key] = job

        for job in by_key.values():
            norm_url = (_resolve_url(job.get("source_url"), page_url) or page_url).rstrip("/").lower()
            if norm_url == page_norm:
                # Same-page vacancy sections can contain many real jobs with identical URL.
                same_page_key = f"{norm_url}#title:{self._normalize_title_v47(job.get('title', '')).lower()}"
            ...
            if current is None:
                by_url[norm_url] = job
                continue
            if self._title_quality_score_v47(job.get("title", "")) > self._title_quality_score_v47(current.get("title", "")):
                by_url[norm_url] = job
    ...
    def _is_valid_jobset_v47(self, jobs: list[dict], page_url: str) -> bool:
        if not jobs:
            return False

        titles = [self._normalize_title_v47(j.get("title", "")) for j in jobs]
        titles = [t for t in titles if t]
        if not titles:
            return False

        role_hits = sum(1 for t in titles if self._title_has_role_signal_v47(t))
        reject_hits = sum(1 for t in titles if _TITLE_REJECT_PATTERN_V46.match(t.lower()))
        evidence_hits = sum(1 for j in jobs if self._job_has_evidence_v47(j, page_url))
        detailish_hits = sum(
            1
            for j in jobs
            if self._is_job_like_url_v47(str(j.get("source_url") or ""))
            or self._is_job_document_url_v47(
                str(j.get("source_url") or ""),
                self._normalize_title_v47(str(j.get("title") or "")),
        ...
        if len(titles) <= 2:
            if role_hits == len(titles) and evidence_hits >= 1:
                return True
        ...
        if len(titles) <= 4:
            if role_hits >= max(2, int(len(titles) * 0.7)) and evidence_hits >= max(1, int(len(titles) * 0.5)):
                return True
            return role_hits >= 1 and detailish_hits >= max(2, len(titles) - 1) and evidence_hits >= max(1, int(len(titles) * 0.5))

        if role_hits < max(1, int(len(titles) * 0.6)):
            return False
        return evidence_hits >= max(1, int(len(titles) * 0.3))

    def _candidate_score_v47(self, jobs: list[dict], page_url: str) -> float:
        if not jobs:
            return 0.0
    ...
    def _job_has_evidence_v47(self, job: dict, page_url: str) -> bool:
        source_url = (job.get("source_url") or page_url).strip() or page_url
        desc = str(job.get("description") or "")

        if self._is_job_document_url_v47(
            source_url,
            self._normalize_title_v47(str(job.get("title") or "")),
            desc,
        ):
            return True

        if self._is_job_like_url_v47(source_url):
            return True

        if source_url.rstrip("/") != page_url.rstrip("/") and not self._is_non_job_url_v47(source_url):
    ...
    def _normalize_title_v47(self, title: str) -> str:
    ...
    def _is_valid_title_v47(self, title: str) -> bool:
        if not title:
            return False
    ...
    def _is_title_acceptable_v47(self, title: str, source_url: str, allow_document: bool = False) -> bool:
        if not self._is_valid_title_v47(title):
            if allow_document:
                return self._title_has_role_signal_v47(title)
            return self._is_acronym_title_v47(title) and self._is_job_like_url_v47(source_url)

        if self._title_has_role_signal_v47(title):
            return True
    ...
    def _title_has_role_signal_v47(self, title: str) -> bool:
        if not title:
            return False
        return bool(_ROLE_HINT_PATTERN_V46.search(title) or _title_has_job_noun(title) or self._is_acronym_title_v47(title))

    @staticmethod
    def _is_acronym_title_v47(title: str) -> bool:
        t = (title or "").strip()
        if not re.fullmatch(r"[A-Z][A-Z0-9&/\-\+]{1,10}", t):
            return False
        return t.lower() not in {"home", "menu", "faq", "apply"}

    def _is_job_like_url_v47(self, url: str) -> bool:
        value = (url or "").strip()
        if not value or self._is_non_job_url_v47(value):
            return False
    ...
    @staticmethod
    def _is_non_job_url_v47(url: str) -> bool:
    ...
    def _is_job_document_url_v47(self, url: str, title: str, context_text: str) -> bool:
        value = (url or "").strip().lower()
        if not value:
            return False
        ...
        if _DOCUMENT_VACANCY_TEXT_PATTERN_V47.search(context):
            return True
        if self._title_has_role_signal_v47(title):
            return True
        return False
    ...
    @staticmethod
    def _iter_json_dicts_v47(payload: Any) -> list[dict[str, Any]]:
    ...
    def _extract_location_from_json_v47(self, node: dict[str, Any]) -> Optional[str]:
        value = (
            node.get("location")
        ...
        if isinstance(value, list):
            for item in value:
                ...
                if isinstance(item, dict):
                    nested = self._extract_location_from_json_v47(item)
                    if nested:
                        return nested
        return None
    ...
    async def _enrich_from_detail_pages(self, jobs: list[dict]) -> list[dict]:
        jobs_to_enrich = [
            ...
            if self._needs_detail_refresh_v47(j)
            and j.get("source_url")
            and str(j.get("source_url")).startswith("http")
            and not self._is_non_job_url_v47(str(j.get("source_url") or ""))
        ]
        ...
        async with httpx.AsyncClient(
            ...
            async def _enrich_one(idx: int, job: dict) -> tuple[int, dict]:
                ...
                description = self._extract_detail_description_v47(root)
                if description:
                    enriched["description"] = description

                location = self._extract_detail_location_v47(root)
                if location:
                    enriched["location_raw"] = location

                employment_type = self._extract_detail_employment_type_v47(root)
                if employment_type:
                    enriched["employment_type"] = employment_type

                salary = self._extract_salary_v47(_text(root))
                if salary:
    ...
    def _extract_detail_description_v47(self, root: etree._Element) -> Optional[str]:
        for selector in _DETAIL_DESC_SELECTORS_V46:
            try:
                ...
                nodes = []
            for node in nodes[:2]:
                text = self._clean_description_v47(_text(node))
    ...
    def _extract_detail_location_v47(self, root: etree._Element) -> Optional[str]:
        icon_xpaths = [
    ...
    def _extract_detail_employment_type_v47(self, root: etree._Element) -> Optional[str]:
        xpaths = [
    ...
    def _should_enrich_detail_pages_v47(self, jobs: list[dict], page_url: str) -> bool:
        off_page = [
            j
            for j in jobs
            if (j.get("source_url") or "").startswith("http")
            and (j.get("source_url") or page_url).rstrip("/") != page_url.rstrip("/")
            and not self._is_non_job_url_v47(j.get("source_url") or "")
        ]
        if not off_page:
            return False

        detail_hits = sum(1 for j in off_page if self._is_job_like_url_v47(j.get("source_url") or ""))
        refresh_hits = sum(1 for j in off_page if self._needs_detail_refresh_v47(j))
    ...
    def _needs_detail_refresh_v47(self, job: dict) -> bool:
    ...
    def _needs_deep_recovery_v47(
        ...
        if best_score < 11.0:
            return True
        if self._looks_like_shell_v47(page_url, html_body):
            return True
        role_hits = sum(1 for j in best_jobs if self._title_has_role_signal_v47(j.get("title", "")))
        return role_hits < max(2, int(len(best_jobs) * 0.6))

    def _looks_like_shell_v47(self, page_url: str, html_body: str) -> bool:
    ...
    @staticmethod
    def _looks_like_martian_shell_v47(html_body: str) -> bool:
        return bool(_MARTIAN_SHELL_PATTERN_V46.search(html_body or ""))

    def _page_listing_score_v47(self, page_url: str, html_body: str) -> float:
        if not html_body or len(html_body) < 200:
            return -20.0
    ...
    @staticmethod
    def _safe_text_v47(el: etree._Element) -> str:
        try:
            txt = el.text_content()
            if txt:
    ...
    def _clean_description_v47(self, description: str) -> Optional[str]:
    ...
    def _extract_location_v47(self, row: etree._Element, title: str) -> Optional[str]:
        for el in row.iter():
    ...
    @staticmethod
    def _extract_salary_v47(text: str) -> Optional[str]:
        if not text:
            return None
    ...
    @staticmethod
    def _extract_employment_type_v47(text: str) -> Optional[str]:
        if not text:
            return None
    ...
    def _title_quality_score_v47(self, title: str) -> float:
        t = self._normalize_title_v47(title)
        if not t:
            return -10.0
        score = 0.0
        score += 2.0 if self._title_has_role_signal_v47(t) else 0.0
        score += 1.0 if self._is_valid_title_v47(t) else 0.0
        score -= max(0.0, (len(t) - 90) / 80.0)
        return score

    def _title_overlap_ratio_v47(self, a_jobs: list[dict], b_jobs: list[dict]) -> float:
        a_titles = {self._normalize_title_v47(j.get("title", "")).lower() for j in a_jobs if j.get("title")}
        b_titles = {self._normalize_title_v47(j.get("title", "")).lower() for j in b_jobs if j.get("title")}
        if not a_titles or not b_titles:
            return 0.0
        return len(a_titles & b_titles) / max(1, min(len(a_titles), len(b_titles)))

    @staticmethod
    def _looks_non_html_payload_v47(body: Optional[str]) -> bool:
        if not body:
            return True
