"""
Tiered Extraction Engine v4.3 — direct from v1.6.

High-impact improvements:
1. Adds utility-card row extraction for generic `p-4`/`mx-4` ATS HTML cards.
2. Expands Martian/MyRecruitmentPlus probing with root query variants and sorted feeds.
3. Adds title cleanup for attached status tails (e.g. "Just posted", "2 days ago").
4. Expands role-signal vocabulary for liaison-style titles.
5. Adds richer detail-page refresh for low-signal descriptions and metadata recovery.
"""
...
class TieredExtractorV43(TieredExtractorV16):
    """v4.3 extractor with stronger martian recovery and quality-focused enrichment."""
    ...
    async def extract(self, career_page, company, html: str) -> list[dict]:
        ...
        try:
            ...
            logger.warning("v4.3 parent extractor timeout for %s", page_url)
        except Exception:
            logger.exception("v4.3 parent extractor failed for %s", page_url)
        ...
        if best_label != "parent_v16" and self._should_enrich_detail_pages_v43(best_jobs, page_url):
            try:
                best_jobs = await asyncio.wait_for(self._enrich_from_detail_pages(best_jobs), timeout=16.0)
            except asyncio.TimeoutError:
                logger.warning("v4.3 enrichment timeout for %s", page_url)
            except Exception:
                logger.exception("v4.3 enrichment failed for %s", page_url)

        final_jobs = self._prepare_candidate_jobs_v38(best_jobs, page_url)
        return final_jobs[:MAX_JOBS_PER_PAGE]

    def _should_enrich_detail_pages_v43(self, jobs: list[dict], page_url: str) -> bool:
        if not jobs:
            return False
        ...
        refresh_hits = sum(1 for j in off_page if self._needs_detail_refresh_v43(j))
        if detailish_hits >= 1:
            return True
        if refresh_hits >= 2:
            return True
    ...
    def _needs_detail_refresh_v43(self, job: dict) -> bool:
    ...
    async def _enrich_from_detail_pages(self, jobs: list[dict]) -> list[dict]:
        jobs_to_enrich = [
            ...
            if self._needs_detail_refresh_v43(job)
        ][:20]
                ...
                for idx, enriched in results:
                    ...
                    for key, value in enriched.items():
                        ...
                        if key == "description":
                            ...
                            if self._needs_detail_refresh_v43(current) or len(value) > len(current_desc) + 120:
                                current[key] = value
    ...
    async def _extract_oracle_jobs_v39(self, page_url: str, html_body: str) -> list[dict]:
        ...
        try:
            ...
            logger.debug("v4.3 oracle probing failed for %s", page_url)
    ...
    async def _extract_martian_jobs_v39(self, page_url: str, html_body: str) -> list[dict]:
        ...
        try:
            ...
            logger.debug("v4.3 martian probing failed for %s", page_url)
