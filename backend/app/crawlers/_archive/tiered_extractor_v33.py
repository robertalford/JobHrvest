"""
Tiered Extraction Engine v3.3 — direct from v1.6 with simplified high-impact recovery.

High-impact improvements:
1. Heading+detail extraction for same-page role blocks with long sibling content and
   apply/mailto evidence (captures internship/collapsible layouts).
2. Stronger non-job suppression for ecommerce/support labels (Size & Fit, Shipping,
   Cart, etc.) plus stricter context checks.
3. Martian/MyRecruitmentPlus fallback expanded with host-hint parsing, _next/data
   variants, and POST search probes for config-only Next.js shells.
4. Oracle CandidateExperience API sweep hardened with tenant-priority site IDs and
   nested requisition flattening.
5. Title quality cleanup: split attached location suffixes (Greenhouse-style
   `EngineerVijayawada`) and keep short acronym roles only with strong evidence.
"""
...
class TieredExtractorV33(TieredExtractorV16):
    """v3.3 extractor with coverage-first fallbacks and stricter validation."""
    ...
    async def extract(self, career_page, company, html: str) -> list[dict]:
        ...
        try:
            ...
            logger.warning("v3.3 parent extractor timeout for %s", url)
        except Exception:
            logger.exception("v3.3 parent extractor failed for %s", url)

        parent_jobs = self._dedupe_jobs_v33(parent_jobs or [], url)
        if parent_jobs:
            candidates.append(("parent_v16", parent_jobs))

        jsonld_jobs = self._extract_jsonld_jobs_v33(working_html, url)
        if jsonld_jobs:
            candidates.append(("jsonld_v33", jsonld_jobs))

        root = _parse_html(working_html)
        if root is not None:
            elementor_jobs = self._extract_elementor_cards_v33(root, url)
            if elementor_jobs:
                candidates.append(("elementor_cards_v33", elementor_jobs))

            heading_jobs = self._extract_heading_rows_v33(root, url)
            if heading_jobs:
                candidates.append(("heading_rows_v33", heading_jobs))

            link_jobs = self._extract_job_links_v33(root, url)
            if link_jobs:
                candidates.append(("job_links_v33", link_jobs))

            grouped_rows = self._extract_repeating_row_groups_v33(root, url)
            if grouped_rows:
                candidates.append(("row_groups_v33", grouped_rows))

        martian_jobs = await self._extract_martian_jobs_v33(url, working_html)
        if martian_jobs:
            candidates.append(("martian_api_v33", martian_jobs))

        oracle_jobs = await self._extract_oracle_jobs_v33(url, working_html)
        if oracle_jobs:
            candidates.append(("oracle_api_v33", oracle_jobs))

        best_label, best_jobs = self._pick_best_jobset_v33(candidates, url)
        if not best_jobs:
            return []

        if best_label != "parent_v16" and any(self._is_job_like_url_v33(j.get("source_url") or "") for j in best_jobs):
            try:
                best_jobs = await asyncio.wait_for(self._enrich_from_detail_pages(best_jobs), timeout=18.0)
            except asyncio.TimeoutError:
                logger.warning("v3.3 enrichment timeout for %s", url)
            except Exception:
                logger.exception("v3.3 enrichment failed for %s", url)
            best_jobs = self._dedupe_jobs_v33(best_jobs, url)
    ...
    def _extract_heading_rows_v33(self, root: etree._Element, page_url: str) -> list[dict]:
        ...
        for container in containers[:280]:
            ...
            for row in row_candidates[:80]:
                ...
                if not heading_nodes:
                    continue
                title = self._normalize_title_v33(_text(heading_nodes[0]))
                if not self._is_valid_title_v33(title):
                    continue
                ...
                if self._is_non_job_url_v33(source_url):
                    source_url = page_url
                ...
                job_url_hint = self._is_job_like_url_v33(source_url) or bool(_JOB_DETAILISH_URL_PATTERN_V33.search(source_url))
                ...
                has_role = self._title_has_role_signal_v33(title)
                if not has_role and not (
                    (job_url_hint and len(title.split()) >= 2)
                    or (job_url_hint and self._is_acronym_title_v33(title))
                ):
                    continue

                location = self._extract_location_v33(row, title)
                description = self._clean_description_v33(row_text)
                ...
                local_jobs.append(
                    {
                        ...
                        "salary_raw": self._extract_salary_v33(row_text),
                        "employment_type": self._extract_employment_type_v33(row_text),
                        "description": description,
                        "extraction_method": "tier2_heading_rows_v33",
        ...
        jobs = self._dedupe_jobs_v33(jobs, page_url)
        if not self._passes_jobset_validation_v33(jobs, page_url):
            return []
        return jobs

    def _extract_job_links_v33(self, root: etree._Element, page_url: str) -> list[dict]:
        jobs: list[dict] = []
        ...
        for a_el in anchors[:6000]:
            ...
            if not source_url or self._is_non_job_url_v33(source_url):
                continue
            ...
            title = self._normalize_title_v33(title_raw)
            if not self._is_valid_title_v33(title):
                continue
            ...
            job_url_hint = self._is_job_like_url_v33(source_url)
            same_page = source_url.rstrip("/") == page_url.rstrip("/")
            ...
            jobs.append(
                {
                    "title": title,
                    "source_url": source_url,
                    "location_raw": self._extract_location_v33(context_el, title),
                    "salary_raw": self._extract_salary_v33(context_text),
                    "employment_type": self._extract_employment_type_v33(context_text),
                    "description": self._clean_description_v33(context_text),
                    "extraction_method": "tier2_job_links_v33",
    ...
    def _extract_repeating_row_groups_v33(self, root: etree._Element, page_url: str) -> list[dict]:
        ...
        for rows in groups.values():
            ...
            for row in rows[:MAX_JOBS_PER_PAGE]:
                ...
                title = self._normalize_title_v33(job.get("title") or "")
                ...
                if not (job_url_hint or apply_hint or len(row_text) >= 180):
                    continue
                if not has_role and not (apply_hint or (job_url_hint and self._is_acronym_title_v33(title))):
                    continue
                ...
                jobs.append(
                    {
                        "title": title,
                        "source_url": source_url,
                        "location_raw": job.get("location_raw") or self._extract_location_v33(row, title),
                        "salary_raw": job.get("salary_raw") or self._extract_salary_v33(row_text),
                        "employment_type": job.get("employment_type") or self._extract_employment_type_v33(row_text),
                        "description": self._clean_description_v33(row_text),
                        "extraction_method": "tier2_row_groups_v33",
    ...
    def _extract_elementor_cards_v33(self, root: etree._Element, page_url: str) -> list[dict]:
        cards = root.xpath(
        ...
        for card in cards[:320]:
            ...
            if not self._is_valid_title_v33(title):
                continue
            if not self._title_has_role_signal_v33(title):
                continue

            source_url = page_url
            for a_el in card.xpath(".//a[@href]"):
                href = _resolve_url(a_el.get("href"), page_url)
                if not href or self._is_non_job_url_v33(href):
                    continue
                if self._is_job_like_url_v33(href):
                    source_url = href
                    break
            ...
            jobs.append(
                {
                    "title": title,
                    "source_url": source_url,
                    "location_raw": self._extract_location_v33(card, title),
                    "salary_raw": self._extract_salary_v33(card_text),
                    "employment_type": self._extract_employment_type_v33(card_text),
                    "description": self._clean_description_v33(card_text),
                    "extraction_method": "tier2_elementor_cards_v33",
    ...
    def _extract_jsonld_jobs_v33(self, html_body: str, page_url: str) -> list[dict]:
        jobs: list[dict] = []
        for match in re.finditer(
            ...
            try:
                ...
                continue
            jobs.extend(
                self._extract_jobs_from_json_obj_v33(
                    parsed,
                    page_url,
                    method="tier0_jsonld_v33",
    ...
    async def _extract_martian_jobs_v33(self, page_url: str, html_body: str) -> list[dict]:
        ...
        context = self._extract_martian_context_v33(html_body, page_url)
        client_code = context.get("client_code") or ""
        if not client_code:
            return []

        endpoints = self._martian_probe_urls_v33(page_url, context)
        if not endpoints:
            return []
        ...
        try:
            async with httpx.AsyncClient(timeout=7, follow_redirects=True) as client:
                for endpoint in endpoints:
                    for probe_url in self._martian_paged_variants_v33(endpoint):
                        ...
                        extracted = self._extract_jobs_from_probe_payload_v33(resp.text, str(resp.url), page_url)
                        if extracted:
                ...
                if request_count < max_requests and len(jobs) < MIN_JOBS_FOR_SUCCESS:
                    post_endpoints = self._martian_post_endpoints_v33(endpoints)
                    payloads = self._martian_post_payloads_v33(context)
            ...
            logger.debug("v3.3 martian probing failed for %s", page_url)
    ...
    async def _extract_oracle_jobs_v33(self, page_url: str, html_body: str) -> list[dict]:
        ...
        site_ids = self._oracle_site_ids_v33(page_url, html_body)
        if not site_ids:
            return []
                ...
                for site_id in site_ids[:10]:
                    ...
                    for offset in range(0, 264, 24):
                        ...
                        for finder in finder_variants:
                            ...
                            batch = self._extract_oracle_items_v33(data, page_url, site_id)
                            if not batch:
                                continue
                    ...
                    deduped = self._dedupe_jobs_v33(site_jobs, page_url)
                    if deduped:
                        site_candidates.append((site_id, deduped))
        except Exception:
            logger.debug("v3.3 oracle probing failed for %s", page_url)
    ...
    def _extract_jobs_from_probe_payload_v33(self, body: str, response_url: str, page_url: str) -> list[dict]:
        ...
        if payload.startswith("{") or payload.startswith("["):
            try:
                parsed = json.loads(payload)
                jobs.extend(self._extract_jobs_from_json_obj_v33(parsed, response_url, method="tier0_api_json_v33"))
            except Exception:
                pass

        root = _parse_html(payload)
        if root is not None:
            jobs.extend(self._extract_job_links_v33(root, response_url))
            jobs.extend(self._extract_elementor_cards_v33(root, response_url))
            jobs.extend(self._extract_heading_rows_v33(root, response_url))
            jobs.extend(self._extract_repeating_row_groups_v33(root, response_url))
            ...
            for job in jobs:
                ...
                if method.startswith("tier2_"):
                    job["extraction_method"] = f"{method}_probe_v33"

        return self._dedupe_jobs_v33(jobs, page_url)

    def _extract_jobs_from_json_obj_v33(
        self,
        data: object,
        page_url: str,
        method: str,
        ...
        while queue and visited < 7000:
            ...
            job = self._job_from_json_dict_v33(node, page_url, method, require_job_type=require_job_type)
            if job:
                jobs.append(job)
                if len(jobs) >= MAX_JOBS_PER_PAGE:
                    break
    ...
    def _job_from_json_dict_v33(
        ...
        for key in (
            ...
            if isinstance(value, str) and value.strip():
                title = value.strip()
                break
        title = self._normalize_title_v33(title)
        if not self._is_valid_title_v33(title):
            return None
        ...
        has_role = self._title_has_role_signal_v33(title) or self._is_acronym_title_v33(title)
        url_hint = self._is_job_like_url_v33(source_url)
        ...
        for key in ("description", "summary", "shortDescription", "jobDescription", "Description", "ExternalDescriptionStr"):
            ...
            if isinstance(value, str) and value.strip():
                ...
                break
        description = self._clean_description_v33(description)

        location = self._extract_location_from_json_v33(node)
        salary = self._extract_salary_from_json_v33(node)
        employment_type = self._extract_employment_from_json_v33(node)
    ...
    def _pick_best_jobset_v33(
        ...
        for label, jobs in candidates:
            deduped = self._dedupe_jobs_v33(jobs, page_url)
            if not deduped:
                continue

            valid = self._passes_jobset_validation_v33(deduped, page_url)
            score = self._jobset_score_v33(deduped, page_url)
            logger.debug("v3.3 candidate %s: jobs=%d valid=%s score=%.2f", label, len(deduped), valid, score)
        ...
        if not scored:
            ...
            largest = max(
                ((label, self._dedupe_jobs_v33(jobs, page_url)) for label, jobs in candidates),
        ...
        for label, jobs, score in scored:
            if label == best_label:
                continue
            overlap = self._title_overlap_ratio_v33(jobs, best_jobs)
        ...
        if parent_jobs and best_label != "parent_v16":
            overlap = self._title_overlap_ratio_v33(best_jobs, parent_jobs)
    ...
    def _passes_jobset_validation_v33(self, jobs: list[dict], page_url: str) -> bool:
        if not jobs or len(jobs) < MIN_JOBS_FOR_SUCCESS:
            return False

        titles = [self._normalize_title_v33(j.get("title", "")) for j in jobs]
        titles = [t for t in titles if self._is_valid_title_v33(t)]
        ...
        role_hits = sum(1 for t in titles if self._title_has_role_signal_v33(t))
        ...
        url_hits = sum(1 for j in jobs if self._is_job_like_url_v33(j.get("source_url") or page_url))
        detail_hits = sum(1 for j in jobs if _JOB_DETAILISH_URL_PATTERN_V33.search(j.get("source_url") or ""))
        apply_hits = sum(1 for j in jobs if _APPLY_CONTEXT_PATTERN_V33.search((j.get("description") or "")[:1600]))
        same_page_hits = sum(
    ...
    def _jobset_score_v33(self, jobs: list[dict], page_url: str) -> float:
        if not jobs:
            return 0.0
    ...
    def _title_overlap_ratio_v33(self, a_jobs: list[dict], b_jobs: list[dict]) -> float:
        a_titles = {self._normalize_title_v33(j.get("title", "")).lower() for j in a_jobs if j.get("title")}
        b_titles = {self._normalize_title_v33(j.get("title", "")).lower() for j in b_jobs if j.get("title")}
        if not a_titles or not b_titles:
            return 0.0
        return len(a_titles & b_titles) / max(1, min(len(a_titles), len(b_titles)))

    def _dedupe_jobs_v33(self, jobs: list[dict], page_url: str) -> list[dict]:
        deduped: list[dict] = []
        ...
        for raw in jobs:
            ...
            title = self._normalize_title_v33(raw.get("title", ""))
            if not self._is_title_acceptable_for_url_v33(title, source_url):
                continue
            ...
            cloned["description"] = self._clean_description_v33(str(cloned.get("description") or ""))
    ...
    def _normalize_title_v33(self, title: str) -> str:
        ...
        t = self._strip_location_suffix_v33(t, raw)
        return t.strip(" \t\r\n-–|:;,.")

    def _is_valid_title_v33(self, title: str) -> bool:
        if not title:
            return False
        t = title.strip()
        if len(t) < 4 or len(t) > 200:
            return False
        if len(t) < 5 and not self._is_acronym_title_v33(t):
            return False
        ...
        if _BOILERPLATE_BOUNDARY_PATTERN_V33.search(lower):
            ...
            if not self._title_has_role_signal_v33(t):
                return False
    ...
    def _title_has_role_signal_v33(self, title: str) -> bool:
        if not title:
            return False
        return _title_has_job_noun(title) or bool(_ROLE_HINT_PATTERN_V33.search(title)) or self._is_acronym_title_v33(title)

    def _is_title_acceptable_for_url_v33(self, title: str, source_url: str) -> bool:
        if self._is_valid_title_v33(title):
            return True
        return self._is_acronym_title_v33(title) and self._is_job_like_url_v33(source_url)

    @staticmethod
    def _is_acronym_title_v33(title: str) -> bool:
        t = (title or "").strip()
        if not _ACRONYM_TITLE_PATTERN_V33.match(t):
            return False
        return t.lower() not in {"faq", "home", "menu", "login", "logout"}

    def _strip_location_suffix_v33(self, normalized: str, raw: str) -> str:
        ...
        if re.search(r"\s+OR\s+", text, re.IGNORECASE):
            ...
            if before_or and self._title_has_role_signal_v33(before_or):
                text = re.sub(r"([a-z])([A-Z][a-z]{2,})$", r"\1 \2", before_or)
                had_or_segment = True

        words = text.split()
        if len(words) < 2 or not self._title_has_role_signal_v33(text):
            return text

        if had_or_segment:
            tail = words[-1]
            if not self._title_has_role_signal_v33(tail) and len(tail) >= 3:
                candidate = " ".join(words[:-1]).strip()
                if candidate and self._title_has_role_signal_v33(candidate):
                    return candidate
        ...
        for cut in range(len(words) - 1, 0, -1):
            ...
            if not head or not tail:
                continue
            if not self._title_has_role_signal_v33(head):
                continue
            if self._title_has_role_signal_v33(tail):
                continue
            if len(tail.split()) > 5:
                continue
    ...
    def _is_job_like_url_v33(self, url: str) -> bool:
        value = (url or "").strip()
        if not value:
            return False
    ...
    def _is_non_job_url_v33(self, url: str) -> bool:
    ...
    def _clean_description_v33(self, description: str) -> Optional[str]:
    ...
    def _extract_location_v33(self, row: etree._Element, title: str) -> Optional[str]:
        for el in row.iter():
            if not isinstance(el.tag, str):
                continue
    ...
    @staticmethod
    def _extract_salary_v33(text: str) -> Optional[str]:
        if not text:
            return None
    ...
    @staticmethod
    def _extract_employment_type_v33(text: str) -> Optional[str]:
        if not text:
            return None
    ...
    def _extract_location_from_json_v33(self, node: dict[str, Any]) -> Optional[str]:
    ...
    def _extract_salary_from_json_v33(self, node: dict[str, Any]) -> Optional[str]:
    ...
    def _extract_employment_from_json_v33(self, node: dict[str, Any]) -> Optional[str]:
    ...
    def _extract_martian_context_v33(self, html_body: str, page_url: str) -> dict[str, str]:
        result = {
            "client_code": "",
            "recruiter_id": "",
            "job_board_theme_id": "",
    ...
    def _martian_probe_urls_v33(self, page_url: str, context: dict[str, str]) -> list[str]:
        ...
        hosts = [base_host, "https://web.martianlogic.com", "https://form.myrecruitmentplus.com", *host_hints]
    ...
    @staticmethod
    def _martian_paged_variants_v33(endpoint: str) -> list[str]:
        variants = [endpoint]
    ...
    @staticmethod
    def _martian_post_endpoints_v33(endpoints: list[str]) -> list[str]:
        postable: list[str] = []
        seen: set[str] = set()
        for endpoint in endpoints:
            low = endpoint.lower()
    ...
    @staticmethod
    def _martian_post_payloads_v33(context: dict[str, str]) -> list[dict[str, Any]]:
        client_code = (context.get("client_code") or "").strip()
        recruiter_id = (context.get("recruiter_id") or "").strip()
        theme_id = (context.get("job_board_theme_id") or "").strip()
        board_name = (context.get("board_name") or "").strip()
    ...
    def _oracle_site_ids_v33(self, page_url: str, html_body: str) -> list[str]:
        seen: set[str] = set()
        ordered: list[str] = []
    ...
    def _extract_oracle_items_v33(self, data: Any, page_url: str, site_id: str) -> list[dict]:
        ...
        for row in rows:
            title = self._normalize_title_v33(
                str(
                    row.get("Title")
                    or row.get("title")
                    or row.get("JobTitle")
            ...
            if not self._is_valid_title_v33(title):
                continue
            if not (self._title_has_role_signal_v33(title) or self._is_acronym_title_v33(title)):
                continue
            ...
            location = self._extract_location_from_json_v33(row)
            if not location:
            ...
            description = self._clean_description_v33(
                str(
                    row.get("Description")
            ...
            jobs.append(
                {
                    ...
                    "source_url": source_url,
                    "location_raw": location,
                    "salary_raw": self._extract_salary_from_json_v33(row),
                    "employment_type": self._extract_employment_from_json_v33(row),
                    "description": description,
                    "extraction_method": "tier0_oracle_api_v33",
