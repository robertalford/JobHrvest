"""
Tiered Extraction Engine v3.6 — direct from v1.6 with state-first shell recovery.

High-impact improvements:
1. App-shell endpoint harvesting: mine API hints from first-party script chunks.
2. Martian/MyRecruitmentPlus probing upgrades: recruiter-first and camelCase endpoint
   variants with low-latency fast-pass ordering.
3. High-confidence JSON salvage: recover strong job subsets when mixed payloads fail
   whole-set validation.
4. Title normalization hardening: avoid over-trimming comma specialization tails while
   still removing location suffix noise.
"""
...
_SCRIPT_SRC_PATTERN_V36 = re.compile(
    r"<script[^>]+src=['\"]([^\"']+)['\"][^>]*>",
    re.IGNORECASE,
)
_MARTIAN_RECRUITER_ENDPOINT_JS_PATTERN_V36 = re.compile(
    r"/api/recruiter/[0-9]{2,}/(?:jobAds|jobads|job-ads|jobs)"
    r"(?:/[A-Za-z0-9_-]+)?(?:\?[A-Za-z0-9=&_%.\-]+)?",
    re.IGNORECASE,
)
_LOCATION_TAIL_STRICT_PATTERN_V36 = re.compile(
...
class TieredExtractorV36(TieredExtractorV16):
    """v3.6 extractor with coverage-first fallbacks and stricter validation."""
    ...
    async def extract(self, career_page, company, html: str) -> list[dict]:
        ...
        try:
            ...
            logger.warning("v3.6 parent extractor timeout for %s", url)
        except Exception:
            logger.exception("v3.6 parent extractor failed for %s", url)

        parent_jobs = self._dedupe_jobs_v36(parent_jobs or [], url)
        if parent_jobs:
            candidates.append(("parent_v16", parent_jobs))

        jsonld_jobs = self._extract_jsonld_jobs_v36(working_html, url)
        if jsonld_jobs:
            candidates.append(("jsonld_v36", jsonld_jobs))

        root = _parse_html(working_html)
        if root is not None:
            prose_jobs = self._extract_prose_heading_blocks_v36(root, url)
            if prose_jobs:
                candidates.append(("prose_blocks_v36", prose_jobs))

            elementor_jobs = self._extract_elementor_cards_v36(root, url)
            if elementor_jobs:
                candidates.append(("elementor_cards_v36", elementor_jobs))

            heading_jobs = self._extract_heading_rows_v36(root, url)
            if heading_jobs:
                candidates.append(("heading_rows_v36", heading_jobs))

            link_jobs = self._extract_job_links_v36(root, url)
            if link_jobs:
                candidates.append(("job_links_v36", link_jobs))

            grouped_rows = self._extract_repeating_row_groups_v36(root, url)
            if grouped_rows:
                candidates.append(("row_groups_v36", grouped_rows))

        martian_jobs: list[dict] = []
        try:
            martian_jobs = await asyncio.wait_for(self._extract_martian_jobs_v36(url, working_html), timeout=14.0)
        except asyncio.TimeoutError:
            logger.debug("v3.6 martian fallback timeout for %s", url)
        except Exception:
            logger.debug("v3.6 martian fallback failed for %s", url)
        if martian_jobs:
            candidates.append(("martian_api_v36", martian_jobs))

        oracle_jobs: list[dict] = []
        try:
            oracle_jobs = await asyncio.wait_for(self._extract_oracle_jobs_v36(url, working_html), timeout=12.0)
        except asyncio.TimeoutError:
            logger.debug("v3.6 oracle fallback timeout for %s", url)
        except Exception:
            logger.debug("v3.6 oracle fallback failed for %s", url)
        if oracle_jobs:
            candidates.append(("oracle_api_v36", oracle_jobs))
        ...
        if not candidates or max((len(jobs) for _label, jobs in candidates), default=0) < MIN_JOBS_FOR_SUCCESS:
            try:
                query_variant_jobs = await asyncio.wait_for(
                    self._extract_query_variant_jobs_v36(url, working_html), timeout=8.0
                )
            except asyncio.TimeoutError:
                query_variant_jobs = []
            except Exception:
            ...
            if query_variant_jobs:
                candidates.append(("query_variants_v36", query_variant_jobs))

        best_label, best_jobs = self._pick_best_jobset_v36(candidates, url)
        if not best_jobs:
            return []

        if best_label != "parent_v16" and any(self._is_job_like_url_v36(j.get("source_url") or "") for j in best_jobs):
            try:
                best_jobs = await asyncio.wait_for(self._enrich_from_detail_pages(best_jobs), timeout=18.0)
            except asyncio.TimeoutError:
                logger.warning("v3.6 enrichment timeout for %s", url)
            except Exception:
                logger.exception("v3.6 enrichment failed for %s", url)
            best_jobs = self._dedupe_jobs_v36(best_jobs, url)

        best_jobs = self._postprocess_jobs_v36(best_jobs, url)
    ...
    def _extract_heading_rows_v36(self, root: etree._Element, page_url: str) -> list[dict]:
        ...
        for container in containers[:280]:
            ...
            for row in row_candidates[:80]:
                ...
                if not heading_nodes:
                    continue
                title = self._normalize_title_v36(_text(heading_nodes[0]))
                if not self._is_valid_title_v36(title):
                    continue
                ...
                if self._is_non_job_url_v36(source_url):
                    source_url = page_url
                ...
                job_url_hint = self._is_job_like_url_v36(source_url) or bool(_JOB_DETAILISH_URL_PATTERN_V33.search(source_url))
                ...
                has_role = self._title_has_role_signal_v36(title)
                if not has_role and not (
                    (job_url_hint and len(title.split()) >= 2)
                    or (job_url_hint and self._is_acronym_title_v36(title))
                ):
                    continue

                location = self._extract_location_v36(row, title)
                description = self._clean_description_v36(row_text)
                ...
                local_jobs.append(
                    {
                        ...
                        "salary_raw": self._extract_salary_v36(row_text),
                        "employment_type": self._extract_employment_type_v36(row_text),
                        "description": description,
                        "extraction_method": "tier2_heading_rows_v36",
        ...
        jobs = self._dedupe_jobs_v36(jobs, page_url)
        if not self._passes_jobset_validation_v36(jobs, page_url):
            return []
        return jobs

    def _extract_prose_heading_blocks_v36(self, root: etree._Element, page_url: str) -> list[dict]:
        ...
        for container in containers[:160]:
            ...
            for heading in headings:
                title = self._normalize_title_v36(_text(heading))
                if not self._is_valid_title_v36(title):
                    continue
                if not self._title_has_role_signal_v36(title):
                    continue
                ...
                if len(block_text) < 180:
                    continue
                if not self._prose_block_has_job_evidence_v36(block_text):
                    continue
                ...
                jobs.append(
                    {
                        "title": title,
                        "source_url": page_url,
                        "location_raw": self._extract_location_v36(container, title),
                        "salary_raw": self._extract_salary_v36(block_text),
                        "employment_type": self._extract_employment_type_v36(block_text),
                        "description": self._clean_description_v36(block_text),
                        "extraction_method": "tier2_prose_blocks_v36",
    ...
    def _extract_job_links_v36(self, root: etree._Element, page_url: str) -> list[dict]:
        jobs: list[dict] = []
        ...
        for a_el in anchors[:6000]:
            ...
            if not source_url or self._is_non_job_url_v36(source_url):
                continue
            ...
            title = self._normalize_title_v36(title_raw)
            if not self._is_valid_title_v36(title):
                continue
            ...
            job_url_hint = self._is_job_like_url_v36(source_url)
            strong_job_url_hint = self._is_strong_job_url_v36(source_url)
            same_page = source_url.rstrip("/") == page_url.rstrip("/")
            ...
            if not has_role and not (
                ...
                or (strong_job_url_hint and self._is_acronym_title_v36(title))
            ):
                continue
            ...
            jobs.append(
                {
                    "title": title,
                    "source_url": source_url,
                    "location_raw": self._extract_location_v36(context_el, title),
                    "salary_raw": self._extract_salary_v36(context_text),
                    "employment_type": self._extract_employment_type_v36(context_text),
                    "description": self._clean_description_v36(context_text),
                    "extraction_method": "tier2_job_links_v36",
    ...
    def _extract_repeating_row_groups_v36(self, root: etree._Element, page_url: str) -> list[dict]:
        ...
        for rows in groups.values():
            ...
            for row in rows[:MAX_JOBS_PER_PAGE]:
                ...
                title = self._normalize_title_v36(job.get("title") or "")
                ...
                if not (job_url_hint or apply_hint or len(row_text) >= 180):
                    continue
                if not has_role and not (apply_hint or (job_url_hint and self._is_acronym_title_v36(title))):
                    continue
                ...
                jobs.append(
                    {
                        "title": title,
                        "source_url": source_url,
                        "location_raw": job.get("location_raw") or self._extract_location_v36(row, title),
                        "salary_raw": job.get("salary_raw") or self._extract_salary_v36(row_text),
                        "employment_type": job.get("employment_type") or self._extract_employment_type_v36(row_text),
                        "description": self._clean_description_v36(row_text),
                        "extraction_method": "tier2_row_groups_v36",
    ...
    def _extract_elementor_cards_v36(self, root: etree._Element, page_url: str) -> list[dict]:
        cards = root.xpath(
        ...
        for card in cards[:320]:
            ...
            for a_el in card.xpath(".//a[@href]"):
                ...
                if not href or self._is_non_job_url_v36(href):
                    continue
                if self._is_job_like_url_v36(href):
                    source_url = href
                    break
            ...
            jobs.append(
                {
                    "title": title,
                    "source_url": source_url,
                    "location_raw": self._extract_location_v36(card, title),
                    "salary_raw": self._extract_salary_v36(card_text),
                    "employment_type": self._extract_employment_type_v36(card_text),
                    "description": self._clean_description_v36(card_text),
                    "extraction_method": "tier2_elementor_cards_v36",
    ...
    def _extract_jsonld_jobs_v36(self, html_body: str, page_url: str) -> list[dict]:
        jobs: list[dict] = []
        for match in re.finditer(
            ...
            try:
                ...
                continue
            jobs.extend(
                self._extract_jobs_from_json_obj_v36(
                    parsed,
                    page_url,
                    method="tier0_jsonld_v36",
    ...
    async def _extract_martian_jobs_v36(self, page_url: str, html_body: str) -> list[dict]:
        ...
        context = self._extract_martian_context_v36(html_body, page_url)
        ...
        extra_hints = await self._harvest_script_endpoint_hints_v36(page_url, html_body)
        if extra_hints:
        ...
        endpoints = self._martian_probe_urls_v36(page_url, context)
        if not endpoints:
            return []
        ...
        try:
            async with httpx.AsyncClient(
                ...
                for endpoint in endpoints[:4]:
                    for probe_url in self._martian_paged_variants_v36(endpoint):
                        ...
                        extracted = self._extract_jobs_from_probe_payload_v36(resp.text, str(resp.url), page_url)
                        if extracted:
                ...
                if request_count < max_requests and len(jobs) < MIN_JOBS_FOR_SUCCESS:
                    post_endpoints = self._martian_post_endpoints_v36(endpoints)
                    payloads = self._martian_post_payloads_v36(context)
            ...
            logger.debug("v3.6 martian probing failed for %s", page_url)
    ...
    async def _extract_oracle_jobs_v36(self, page_url: str, html_body: str) -> list[dict]:
        ...
        site_ids = self._oracle_site_ids_v36(page_url, html_body)
        if not site_ids:
            return []
                ...
                for site_id in site_ids[:10]:
                    ...
                    for offset in range(0, 264, 24):
                        ...
                        for finder in finder_variants:
                            ...
                            batch = self._extract_oracle_items_v36(data, page_url, site_id)
                            if not batch:
                                continue
                    ...
                    deduped = self._dedupe_jobs_v36(site_jobs, page_url)
                    if deduped:
                        site_candidates.append((site_id, deduped))
        except Exception:
            logger.debug("v3.6 oracle probing failed for %s", page_url)
    ...
    async def _extract_query_variant_jobs_v36(self, page_url: str, html_body: str) -> list[dict]:
        if not self._should_try_query_variants_v36(page_url, html_body):
            return []

        variant_urls = self._query_variant_urls_v36(page_url)
        if not variant_urls:
            return []
        ...
        try:
            async with httpx.AsyncClient(
                ...
                for variant_url in variant_urls[:8]:
                    ...
                    if resp.status_code >= 400 or len(body) < 200:
                        continue
                    extracted = self._extract_jobs_from_probe_payload_v36(body, str(resp.url), page_url)
                    if not extracted:
                        continue
                    jobs.extend(extracted)
                    if len(jobs) >= MAX_JOBS_PER_PAGE:
            ...
            logger.debug("v3.6 query variant recovery failed for %s", page_url)
    ...
    def _should_try_query_variants_v36(self, page_url: str, html_body: str) -> bool:
    ...
    def _query_variant_urls_v36(self, page_url: str) -> list[str]:
        parsed = urlparse(page_url or "")
        if not parsed.netloc or parsed.query:
            return []
    ...
    def _extract_jobs_from_probe_payload_v36(self, body: str, response_url: str, page_url: str) -> list[dict]:
        ...
        if payload.startswith("{") or payload.startswith("["):
            try:
                parsed = json.loads(payload)
                jobs.extend(self._extract_jobs_from_json_obj_v36(parsed, response_url, method="tier0_api_json_v36"))
            except Exception:
                pass

        root = _parse_html(payload)
        if root is not None:
            jobs.extend(self._extract_job_links_v36(root, response_url))
            jobs.extend(self._extract_elementor_cards_v36(root, response_url))
            jobs.extend(self._extract_heading_rows_v36(root, response_url))
            jobs.extend(self._extract_repeating_row_groups_v36(root, response_url))
            ...
            for job in jobs:
                ...
                if method.startswith("tier2_"):
                    job["extraction_method"] = f"{method}_probe_v36"

        jobs.extend(self._extract_jobs_from_js_literals_v36(payload, response_url, page_url))

        deduped = self._dedupe_jobs_v36(jobs, page_url)
        if deduped and not self._passes_jobset_validation_v36(deduped, page_url):
            recovered = self._salvage_high_confidence_jobs_v36(deduped, page_url)
            if recovered:
                return recovered
        return deduped

    def _extract_jobs_from_json_obj_v36(
        self,
        data: object,
        page_url: str,
        method: str,
        ...
        while queue and visited < 7000:
            ...
            job = self._job_from_json_dict_v36(node, page_url, method, require_job_type=require_job_type)
            if job:
                jobs.append(job)
                if len(jobs) >= MAX_JOBS_PER_PAGE:
                    break
    ...
    def _extract_jobs_from_js_literals_v36(self, payload: str, response_url: str, page_url: str) -> list[dict]:
        jobs: list[dict] = []
        if not payload:
            return jobs
        ...
        for match in pattern.finditer(payload):
            title = self._normalize_title_v36(match.group(1) or "")
            if not self._is_valid_title_v36(title):
                continue
            if not self._title_has_role_signal_v36(title):
                continue
            ...
            jobs.append(
                {
                    ...
                    "description": None,
                    "extraction_method": "tier0_js_literal_v36",
    ...
    def _salvage_high_confidence_jobs_v36(self, jobs: list[dict], page_url: str) -> list[dict]:
        candidates = self._dedupe_jobs_v36(jobs, page_url)
        if not candidates:
            return []
        ...
        filtered = [
            ...
            if self._title_has_role_signal_v36(self._normalize_title_v36(job.get("title", "")))
            and self._job_has_strong_evidence_v36(job, page_url)
        ]
        filtered = self._dedupe_jobs_v36(filtered, page_url)
        if not filtered or len(filtered) < MIN_JOBS_FOR_SUCCESS:
            return []

        if self._passes_jobset_validation_v36(filtered, page_url):
            return filtered

        role_ratio = sum(
            1
            for j in filtered
            if self._title_has_role_signal_v36(self._normalize_title_v36(j.get("title", "")))
        ) / max(1, len(filtered))
        strong_ratio = sum(1 for j in filtered if self._job_has_strong_evidence_v36(j, page_url)) / max(1, len(filtered))
        if role_ratio >= 0.8 and strong_ratio >= 0.8:
            return filtered
        return []

    def _job_from_json_dict_v36(
        self,
        node: dict[str, Any],
        page_url: str,
        method: str,
        ...
        for key in (
            ...
            if isinstance(value, str) and value.strip():
                title = value.strip()
                break
        title = self._normalize_title_v36(title)
        if not self._is_valid_title_v36(title):
            return None
        ...
        has_role = self._title_has_role_signal_v36(title) or self._is_acronym_title_v36(title)
        url_hint = self._is_job_like_url_v36(source_url)
        ...
        for key in (
            ...
            if isinstance(value, str) and value.strip():
                ...
                break
        description = self._clean_description_v36(description)

        location = self._extract_location_from_json_v36(node)
        salary = self._extract_salary_from_json_v36(node)
        employment_type = self._extract_employment_from_json_v36(node)
    ...
    def _pick_best_jobset_v36(
        ...
        for label, jobs in candidates:
            ...
            valid = self._passes_jobset_validation_v36(deduped, page_url)
            score = self._jobset_score_v36(deduped, page_url)
            logger.debug("v3.6 candidate %s: jobs=%d valid=%s score=%.2f", label, len(deduped), valid, score)
        ...
        if not scored:
            ...
            largest = max(
                ((label, self._dedupe_jobs_v36(jobs, page_url)) for label, jobs in candidates),
        ...
        for label, jobs, score in scored:
            if label == best_label:
                continue
            overlap = self._title_overlap_ratio_v36(jobs, best_jobs)
        ...
        for label, jobs, score in scored:
            ...
            if abs(len(jobs) - len(best_jobs)) > 1 or score < best_score - 1.0:
                continue
            if self._jobset_quality_tuple_v36(jobs, page_url) > self._jobset_quality_tuple_v36(best_jobs, page_url):
                best_label, best_jobs, best_score = label, jobs, score

        if parent_jobs and best_label != "parent_v16":
            overlap = self._title_overlap_ratio_v36(best_jobs, parent_jobs)
    ...
    def _passes_jobset_validation_v36(self, jobs: list[dict], page_url: str) -> bool:
        if not jobs:
            return False

        titles = [self._normalize_title_v36(j.get("title", "")) for j in jobs]
        titles = [t for t in titles if self._is_valid_title_v36(t)]
        if not titles:
            return False

        if len(titles) < MIN_JOBS_FOR_SUCCESS:
            return self._passes_small_high_precision_jobset_v36(jobs, page_url, titles)

        unique_ratio = len(set(t.lower() for t in titles)) / max(1, len(titles))
        if len(titles) > 3 and unique_ratio < 0.62:
            return False

        role_hits = sum(1 for t in titles if self._title_has_role_signal_v36(t))
        ...
        url_hits = sum(1 for j in jobs if self._is_strong_job_url_v36(j.get("source_url") or page_url))
        ...
        same_page_evidence_hits = sum(1 for j in jobs if self._has_same_page_evidence_v36(j, page_url))
        strong_evidence_hits = sum(1 for j in jobs if self._job_has_strong_evidence_v36(j, page_url))
        same_page_hits = sum(
            1 for j in jobs if (j.get("source_url") or page_url).rstrip("/") == page_url.rstrip("/")
        )
        metadata_tail_hits = sum(1 for t in titles if self._title_looks_metadata_heavy_v36(t))
    ...
    def _jobset_score_v36(self, jobs: list[dict], page_url: str) -> float:
        if not jobs:
            return 0.0
        ...
        clean_title_hits = sum(1 for t in titles if not self._title_looks_metadata_heavy_v36(t) and len(t) <= 90)
    ...
    def _jobset_quality_tuple_v36(self, jobs: list[dict], page_url: str) -> tuple[float, float, float, float]:
        if not jobs:
        ...
        clean_ratio = (
            sum(1 for t in titles if not self._title_looks_metadata_heavy_v36(t) and len(t) <= 90)
            / max(1, len(titles))
        )
        metadata_ratio = sum(1 for t in titles if self._title_looks_metadata_heavy_v36(t)) / max(1, len(titles))
        avg_title_len = sum(len(t) for t in titles) / max(1, len(titles))
        return (detail_ratio, clean_ratio, -metadata_ratio, -avg_title_len)

    def _passes_small_high_precision_jobset_v36(self, jobs: list[dict], page_url: str, titles: list[str]) -> bool:
        ...
        strong_hits = sum(1 for j in jobs if self._job_has_strong_evidence_v36(j, page_url))
        return strong_hits >= len(titles)

    def _prose_block_has_job_evidence_v36(self, text: str) -> bool:
        desc = (text or "").strip()
        if len(desc) < 150:
            return False
        lower = desc.lower()
    ...
    def _has_same_page_evidence_v36(self, job: dict, page_url: str) -> bool:
        source_url = (job.get("source_url") or page_url).strip() or page_url
        same_page = source_url.rstrip("/") == page_url.rstrip("/")
        if not same_page:
            return False
    ...
    def _job_has_strong_evidence_v36(self, job: dict, page_url: str) -> bool:
        source_url = (job.get("source_url") or page_url).strip() or page_url
        if self._is_non_job_url_v36(source_url):
            return False

        same_page = source_url.rstrip("/") == page_url.rstrip("/")
        title = self._normalize_title_v36(job.get("title", ""))
        if _JOB_DETAILISH_URL_PATTERN_V33.search(source_url):
            return True
        if self._is_strong_job_url_v36(source_url) and not same_page:
            return True
        if not same_page and title and self._title_has_role_signal_v36(title):
            ...
            if tokens:
                ...
                if leaf not in {"careers", "career", "jobs", "job", "vacancies", "vacancy", "openings"}:
                    ...
                    if slug_words and (slug_words & title_words):
                        return True
        return self._has_same_page_evidence_v36(job, page_url)

    def _title_overlap_ratio_v36(self, a_jobs: list[dict], b_jobs: list[dict]) -> float:
        a_titles = {self._normalize_title_v36(j.get("title", "")).lower() for j in a_jobs if j.get("title")}
        b_titles = {self._normalize_title_v36(j.get("title", "")).lower() for j in b_jobs if j.get("title")}
        if not a_titles or not b_titles:
            return 0.0
        return len(a_titles & b_titles) / max(1, min(len(a_titles), len(b_titles)))

    def _dedupe_jobs_v36(self, jobs: list[dict], page_url: str) -> list[dict]:
        prelim: list[dict] = []
        ...
        for idx, raw in enumerate(jobs):
            ...
            title = self._normalize_title_v36(raw.get("title", ""))
            if not self._is_title_acceptable_for_url_v36(title, source_url):
                continue

            cloned = dict(raw)
            cloned["title"] = title
            cloned["source_url"] = source_url
            cloned["description"] = self._clean_description_v36(str(cloned.get("description") or ""))
            cloned["_order"] = idx
        ...
        for job in prelim:
            ...
            if current is None:
                ...
                continue
            if self._title_quality_score_v36(job.get("title", "")) > self._title_quality_score_v36(current.get("title", "")):
                by_title_url[key] = job
    ...
    def _postprocess_jobs_v36(self, jobs: list[dict], page_url: str) -> list[dict]:
        cleaned: list[dict] = []
        for raw in jobs:
            title = self._normalize_title_v36(raw.get("title", ""))
        ...
        return self._dedupe_jobs_v36(cleaned, page_url)

    # ------------------------------------------------------------------
    # Normalization / validation helpers
    # ------------------------------------------------------------------

    def _normalize_title_v36(self, title: str) -> str:
        ...
        t = self._strip_location_suffix_v36(t, raw)
        t = self._strip_comma_location_tail_v36(t)
        t = _TITLE_META_TAIL_PATTERN_V35.sub("", t).strip()
        return t.strip(" \t\r\n-–|:;,.")

    def _strip_comma_location_tail_v36(self, text: str) -> str:
        value = (text or "").strip()
        if "," not in value:
            return value
        ...
        head = head.strip()
        tail = tail.strip()
        if not head or not tail or not self._title_has_role_signal_v36(head):
            return value
        ...
        if last_word_is_location and len(head_words) > 2:
            ...
            candidate = head
        if candidate and self._title_has_role_signal_v36(candidate):
            return candidate
        return value

    def _title_looks_metadata_heavy_v36(self, title: str) -> bool:
    ...
    def _title_quality_score_v36(self, title: str) -> float:
        t = self._normalize_title_v36(title)
        if not t:
            return -10.0
        score = 0.0
        score += 2.0 if self._title_has_role_signal_v36(t) else 0.0
        score += 1.0 if self._is_valid_title_v36(t) else 0.0
        score -= 2.0 if self._title_looks_metadata_heavy_v36(t) else 0.0
        score -= max(0.0, (len(t) - 80) / 60.0)
        return score

    def _is_valid_title_v36(self, title: str) -> bool:
        if not title:
            return False
        t = title.strip()
        if len(t) < 4 or len(t) > 200:
            return False
        if len(t) < 5 and not self._is_acronym_title_v36(t):
            return False
        ...
        if _BOILERPLATE_BOUNDARY_PATTERN_V33.search(lower):
            ...
            if not self._title_has_role_signal_v36(t):
                return False
        if _NOISY_CTA_TITLE_PATTERN_V35.search(lower):
            return False
        if _COMPANY_CAREER_LABEL_PATTERN_V35.match(lower) and not self._title_has_role_signal_v36(t):
            return False
    ...
    def _title_has_role_signal_v36(self, title: str) -> bool:
        if not title:
            return False
        return _title_has_job_noun(title) or bool(_ROLE_HINT_PATTERN_V33.search(title)) or self._is_acronym_title_v36(title)

    def _is_title_acceptable_for_url_v36(self, title: str, source_url: str) -> bool:
        if self._is_valid_title_v36(title):
            return True
        return self._is_acronym_title_v36(title) and self._is_job_like_url_v36(source_url)

    @staticmethod
    def _is_acronym_title_v36(title: str) -> bool:
        t = (title or "").strip()
        if not _ACRONYM_TITLE_PATTERN_V33.match(t):
            return False
        return t.lower() not in {"faq", "home", "menu", "login", "logout"}

    def _strip_location_suffix_v36(self, normalized: str, raw: str) -> str:
        ...
        if re.search(r"\s+OR\s+", text, re.IGNORECASE):
            ...
            if before_or and self._title_has_role_signal_v36(before_or):
                text = re.sub(r"([a-z])([A-Z][a-z]{2,})$", r"\1 \2", before_or)
                had_or_segment = True

        words = text.split()
        if len(words) < 2 or not self._title_has_role_signal_v36(text):
            return text

        if had_or_segment:
            tail = words[-1]
            if not self._title_has_role_signal_v36(tail) and len(tail) >= 3:
        ...
        for cut in range(len(words) - 1, 0, -1):
            ...
            if not head or not tail:
                continue
            if not self._title_has_role_signal_v36(head):
                continue
            if self._title_has_role_signal_v36(tail):
                continue
            if len(tail.split()) > 5:
                continue
            if re.search(r"\b(project|programme|program)\b", tail, re.IGNORECASE):
                continue
            if self._looks_like_location_tail_v36(tail):
                return head
        return text

    def _looks_like_location_tail_v36(self, tail: str) -> bool:
        value = (tail or "").strip()
        if not value:
            return False
        if _LOCATION_SUFFIX_HINT_PATTERN_V33.search(value):
            return True
        if _LOCATION_TAIL_STRICT_PATTERN_V36.search(value):
            return True
    ...
    def _is_job_like_url_v36(self, url: str) -> bool:
        value = (url or "").strip()
        if not value:
            return False
    ...
    def _is_strong_job_url_v36(self, url: str) -> bool:
        value = (url or "").strip()
        if not value or self._is_non_job_url_v36(value):
            return False
    ...
    def _is_non_job_url_v36(self, url: str) -> bool:
    ...
    def _clean_description_v36(self, description: str) -> Optional[str]:
    ...
    def _extract_location_v36(self, row: etree._Element, title: str) -> Optional[str]:
        for el in row.iter():
            if not isinstance(el.tag, str):
                continue
    ...
    @staticmethod
    def _extract_salary_v36(text: str) -> Optional[str]:
        if not text:
            return None
    ...
    @staticmethod
    def _extract_employment_type_v36(text: str) -> Optional[str]:
        if not text:
            return None
    ...
    def _extract_location_from_json_v36(self, node: dict[str, Any]) -> Optional[str]:
    ...
    def _extract_salary_from_json_v36(self, node: dict[str, Any]) -> Optional[str]:
    ...
    def _extract_employment_from_json_v36(self, node: dict[str, Any]) -> Optional[str]:
    ...
    async def _harvest_script_endpoint_hints_v36(self, page_url: str, html_body: str) -> list[str]:
        script_urls = self._script_src_urls_v36(page_url, html_body)
        if not script_urls:
            return []
        ...
        try:
            async with httpx.AsyncClient(
                ...
                for script_url in script_urls[:2]:
                    try:
                    ...
                    for match in _MARTIAN_RECRUITER_ENDPOINT_JS_PATTERN_V36.finditer(payload):
    ...
    def _script_src_urls_v36(self, page_url: str, html_body: str) -> list[str]:
        urls: list[str] = []
        seen: set[str] = set()
        page_host = urlparse(page_url or "").netloc.lower()

        for match in _SCRIPT_SRC_PATTERN_V36.finditer(html_body or ""):
    ...
    def _extract_martian_context_v36(self, html_body: str, page_url: str) -> dict[str, str]:
        result = {
            "client_code": "",
            "recruiter_id": "",
            "job_board_theme_id": "",
    ...
    def _martian_probe_urls_v36(self, page_url: str, context: dict[str, str]) -> list[str]:
        parsed = urlparse(page_url or "")
        ...
        next_page = (context.get("next_page") or "").strip()
        ...
        query_templates = [
            ...
            "page=1&perPage=50&isActive=true",
            "offset=0&limit=50",
            "page=1&pageSize=50",
            "limit=50&page=1",
        ]
        if client_code:
            query_templates.extend(
                [
                    ...
                    f"client={client_code}&offset=0&limit=50",
                    f"clientCode={client_code}&offset=0&limit=50",
                    f"client={client_code}&page=1&pageSize=50",
                    f"clientCode={client_code}&page=1&pageSize=50",
                ]
            )
        if recruiter_id:
            query_templates.append(f"recruiterId={recruiter_id}&pageNumber=1&pageSize=50&isActive=true")
            query_templates.append(f"recruiterId={recruiter_id}&page=1&pageSize=50")
            if client_code:
                query_templates.extend(
                    [
        ...
        if client_code:
            paths.extend(
                [
                    ...
                    f"/{client_code}/vacancies",
                    f"/{client_code}/openings",
                    f"/{client_code}/positions",
                ]
            )
        if board_slug and client_code:
            paths.extend(
                [
        ...
        if next_page:
            cleaned_page = next_page.replace("[", "").replace("]", "").strip("/")
            if cleaned_page:
                paths.extend(
                    [
                        f"/{cleaned_page}",
                        f"/{cleaned_page}/jobs",
                        f"/{cleaned_page}/search",
                        f"/{cleaned_page}/vacancies",
                    ]
                )
        ...
        for host in hosts:
            ...
            for query in query_templates:
                candidates.append(f"{host}/?{query}")
            if recruiter_id:
                candidates.extend(
                    [
                        f"{host}/api/recruiter/{recruiter_id}/jobs?pageNumber=1&pageSize=50",
                        f"{host}/api/recruiter/{recruiter_id}/jobs/search?pageNumber=1&pageSize=50",
                        ...
                        f"{host}/api/recruiter/{recruiter_id}/jobAds?pageNumber=1&pageSize=50",
                        f"{host}/api/recruiter/{recruiter_id}/jobs?page=1&perPage=50",
                        f"{host}/api/recruiter/{recruiter_id}/jobs?page=1&pageSize=50",
                    ]
                )
            candidates.extend(self._next_data_candidate_urls_v36(host, page_url, context, client_code))
        ...
        if page_query.get("jobAdId"):
            if client_code:
                candidates.append(
                    f"{base_host}/?client={client_code}&jobAdId={page_query['jobAdId']}&pageNumber=1&pageSize=50&isActive=true"
                )
            candidates.append(
                f"{base_host}/?jobAdId={page_query['jobAdId']}&pageNumber=1&pageSize=50&isActive=true"
            )
        ...
        def _priority(candidate_url: str) -> int:
            ...
            if "search" in low:
                score += 7
            if "embed-jobs" in low or "jobads" in low or "job-ads" in low or "job-board" in low:
                score += 6
    ...
    def _next_data_candidate_urls_v36(
        self,
        host: str,
        page_url: str,
    ...
    @staticmethod
    def _martian_paged_variants_v36(endpoint: str) -> list[str]:
        variants = [endpoint]
    ...
    @staticmethod
    def _martian_post_endpoints_v36(endpoints: list[str]) -> list[str]:
        postable: list[str] = []
        seen: set[str] = set()
    ...
    @staticmethod
    def _martian_post_payloads_v36(context: dict[str, str]) -> list[dict[str, Any]]:
        client_code = (context.get("client_code") or "").strip()
        recruiter_id = (context.get("recruiter_id") or "").strip()
        theme_id = (context.get("job_board_theme_id") or "").strip()
        board_name = (context.get("board_name") or "").strip()
    ...
    def _oracle_site_ids_v36(self, page_url: str, html_body: str) -> list[str]:
        seen: set[str] = set()
        ordered: list[str] = []
    ...
    def _extract_oracle_items_v36(self, data: Any, page_url: str, site_id: str) -> list[dict]:
        ...
        for row in rows:
            title = self._normalize_title_v36(
                str(
                    row.get("Title")
                    or row.get("title")
                    or row.get("JobTitle")
            ...
            if not self._is_valid_title_v36(title):
                continue
            if not (self._title_has_role_signal_v36(title) or self._is_acronym_title_v36(title)):
                continue
            ...
            location = self._extract_location_from_json_v36(row)
            if not location:
            ...
            description = self._clean_description_v36(
                str(
                    row.get("Description")
            ...
            jobs.append(
                {
                    ...
                    "source_url": source_url,
                    "location_raw": location,
                    "salary_raw": self._extract_salary_from_json_v36(row),
                    "employment_type": self._extract_employment_from_json_v36(row),
                    "description": description,
                    "extraction_method": "tier0_oracle_api_v36",
