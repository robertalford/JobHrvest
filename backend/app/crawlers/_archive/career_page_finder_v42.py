"""
Career Page Finder v4.2 — direct from CareerPageFinderV4.

High-impact improvements:
1. Preserves v4.1 listing-hub + Greenhouse slug recovery.
2. Adds host-affinity recovery to prevent drift from ATS career hosts to external marketing pages.
3. Adds same-host Salesforce hard probes (`fRecruit__ApplyJobList`) when discovery confidence is weak.
4. Keeps payload-safe recovery for PDF/non-HTML responses.
"""
...
class CareerPageFinderV42(CareerPageFinderV4):
    """v4.2 finder with listing-hub promotion, slug recovery, and host-affinity guardrails."""
    ...
    async def find(self, domain: str, company_name: str = "") -> dict:
        ...
        async with httpx.AsyncClient(timeout=8, follow_redirects=True, headers=_CLIENT_HEADERS) as client:
            ...
            affinity_upgrade = await self._host_affinity_upgrade_v42(client, current, domain, company_name)
            if affinity_upgrade:
                score = self._listing_page_score_v38(affinity_upgrade.get("url", ""), affinity_upgrade.get("html") or "")
                current_host_match = self._is_host_aligned_v42(current.get("url", ""), domain)
                affinity_host_match = self._is_host_aligned_v42(affinity_upgrade.get("url", ""), domain)
                if (
                    (affinity_host_match and not current_host_match and score >= current_score - 0.6)
                    or score > current_score + 0.2
                ):
                    current = affinity_upgrade
    ...
    async def _probe_candidates_v38(
        ...
        for candidate_url, reason, weight in deduped[:limit]:
            ...
            score += self._candidate_host_bonus_v42(current_url, str(resp.url))
    ...
    @staticmethod
    def _candidate_host_bonus_v42(parent_url: str, candidate_url: str) -> float:
        parent_host = (urlparse(parent_url or "").hostname or "").lower()
        candidate_host = (urlparse(candidate_url or "").hostname or "").lower()
        if not parent_host or not candidate_host:
            return 0.0
        if parent_host == candidate_host:
            return 1.6
        ...
        parent_base = _base(parent_host)
        candidate_base = _base(candidate_host)
        ...
        if any(token in candidate_host for token in known_ats):
            return -0.5
        if "salesforce-sites.com" in parent_host:
            return -7.0
        return -3.8
    ...
    async def _host_affinity_upgrade_v42(
        self,
        client: httpx.AsyncClient,
        current: dict,
        domain: str,
        ...
        target_host = (domain or "").strip().lower()
        current_host = (urlparse(current_url).hostname or "").lower()
        target_is_salesforce = "salesforce-sites.com" in target_host
        drifted = not self._is_host_aligned_v42(current_url, domain)
        weak_score = self._listing_page_score_v38(current_url, current_html) < 9.0

        if not (drifted or weak_score or target_is_salesforce):
            return None
        ...
        for path in (
            ...
            weight = 7.0
            if "fRecruit__ApplyJobList" in path:
                weight += 4.2
            candidates.append((base + path, f"host_affinity:{path}", weight))

        seed_disc = {
            "url": current_url,
            "method": current.get("method", "") + "+host_affinity_v42",
            "candidates": current.get("candidates", []),
            "html": current_html,
        }
        upgraded = await self._probe_candidates_v38(client, seed_disc, candidates, limit=20)
        if upgraded and self._is_host_aligned_v42(upgraded.get("url", ""), domain):
            return upgraded

        if target_is_salesforce and current_host != target_host:
            fallback_disc = {
                "url": base,
                "method": current.get("method", "") + "+host_affinity_seed_v42",
                "candidates": current.get("candidates", []) + [base],
                "html": "",
            }
            seeded_candidates = self._build_candidate_urls_v38(fallback_disc, domain, company_name)
            seeded = await self._probe_candidates_v38(client, fallback_disc, seeded_candidates, limit=28)
            if seeded and self._is_host_aligned_v42(seeded.get("url", ""), domain):
                return seeded

        return upgraded

    @staticmethod
    def _is_host_aligned_v42(url: str, domain: str) -> bool:
        url_host = (urlparse(url or "").hostname or "").lower()
        seed_host = (domain or "").strip().lower()
        if not url_host or not seed_host:
            return False
        if url_host == seed_host:
            return True
