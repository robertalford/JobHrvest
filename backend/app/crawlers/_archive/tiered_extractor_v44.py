"""
Tiered Extraction Engine v4.4 — direct from v1.6.

High-impact improvements:
1. Restores Martian/MyRecruitmentPlus script-chunk endpoint hint harvesting for
   config-only Next.js shells.
2. Expands Martian probe planning/variants with search-style query triggers and
   per-host `_next/data` coverage.
3. Adds strict high-confidence JSON subset salvage for mixed/noisy API payloads.
4. Improves HTML-fragment probe parsing (job card fragments and partial payloads).
5. Improves detail-page enrichment triggering for teaser-heavy card grids.
6. Adds JSON-LD JobPosting extraction candidate for structured pages.
"""
...
class TieredExtractorV44(TieredExtractorV16):
    """v4.4 extractor with stronger config-shell recovery and enrichment depth."""
    ...
    async def extract(self, career_page, company, html: str) -> list[dict]:
        ...
        try:
            ...
            logger.warning("v4.4 parent extractor timeout for %s", page_url)
        except Exception:
            logger.exception("v4.4 parent extractor failed for %s", page_url)
        ...
        if best_label != "parent_v16" and self._should_enrich_detail_pages_v44(best_jobs, page_url):
            try:
                ...
                logger.warning("v4.4 enrichment timeout for %s", page_url)
            except Exception:
                logger.exception("v4.4 enrichment failed for %s", page_url)
    ...
    async def _extract_oracle_jobs_v39(self, page_url: str, html_body: str) -> list[dict]:
        ...
        try:
            ...
            logger.debug("v4.4 oracle probing failed for %s", page_url)
    ...
    async def _extract_martian_jobs_v39(self, page_url: str, html_body: str) -> list[dict]:
        ...
        jobs: list[dict] = []
        request_budget = 26 if shell_like else 16
        try:
            async with httpx.AsyncClient(
                timeout=3.2,
                follow_redirects=True,
                ...
                for endpoint in fast_pass:
                    ...
                    if request_budget <= 0 or len(jobs) >= MAX_JOBS_PER_PAGE:
                        break
                    if len(jobs) >= MIN_JOBS_FOR_SUCCESS and request_budget <= 9:
                        break
                ...
                for endpoint in probe_plan:
                    ...
                    if request_budget <= 0 or len(jobs) >= MAX_JOBS_PER_PAGE:
                        break
                    if len(jobs) >= MIN_JOBS_FOR_SUCCESS and request_budget <= 6:
                        break
            ...
            logger.debug("v4.4 martian probing failed for %s", page_url)
        ...
        try:
            async with httpx.AsyncClient(
                ...
                for script_url in script_urls[:6]:
                    try:
                        resp = await asyncio.wait_for(client.get(script_url), timeout=1.9)
                    except Exception:
                        continue
