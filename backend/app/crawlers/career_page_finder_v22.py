"""
Career Page Finder v2.2 — direct from CareerPageFinderV4 with stronger target recovery.
...
_CAREER_HREF_PATTERN_V22 = re.compile(
...
_CAREER_TEXT_PATTERN_V22 = re.compile(
...
_REJECT_LINK_TEXT_PATTERN_V22 = re.compile(
...
_REJECT_LINK_HREF_PATTERN_V22 = re.compile(
...
_LISTING_URL_HINT_PATTERN_V22 = re.compile(
...
_JOB_DETAIL_URL_PATTERN_V22 = re.compile(
...
_ERROR_PAGE_PATTERN_V22 = re.compile(
...
_LOGIN_PAGE_PATTERN_V22 = re.compile(
...
_BASE_HREF_SITE_PATTERN_V22 = re.compile(
    r"<base[^>]+href=[\"']([^\"']+/hcmUI/CandidateExperience/[^\"']+)[\"']",
    re.IGNORECASE,
)

_CANDIDATE_PORTAL_BASE_PATTERN_V22 = re.compile(
...
_ATS_HOST_HINT_PATTERN_V22 = re.compile(
...
class CareerPageFinderV22(CareerPageFinderV4):
    """v2.2 finder with stronger recovery from wrong discovery targets."""
    ...
    async def find(self, domain: str, company_name: str = "") -> dict:
        ...
        async with httpx.AsyncClient(
            ...
            script_candidate = await self._probe_script_and_link_targets_v22(client, current)
            if script_candidate:
            ...
            ats_candidate = await self._probe_ats_paths_v22(client, domain, company_name, current)
            if ats_candidate:
            ...
            sub_candidate = await self._try_subpage_discovery_v22(client, current)
            if sub_candidate:
            ...
            if current_bad:
                homepage_candidate = await self._homepage_recovery_v22(client, domain)
            ...
            if current_bad or current_score < 8:
                domain_probe = await self._probe_domain_paths_v22(client, domain, company_name, current)
                if domain_probe:
    ...
    async def _try_subpage_discovery_v22(
        self,
        ...
        for a_el in root.iter("a"):
            ...
            if self._is_rejected_link_v22(href, text):
                continue
            ...
            if _CAREER_HREF_PATTERN_V22.search(href):
                score += 6.0
                reason_parts.append("href")
            if text and _CAREER_TEXT_PATTERN_V22.search(text):
                score += 6.0
                reason_parts.append("text")
            if _LISTING_URL_HINT_PATTERN_V22.search(full_url):
                score += 3.0
            ...
            if _JOB_DETAIL_URL_PATTERN_V22.search(full_url):
                score -= 8.0
        ...
        for candidate_url, candidate_weight, reason in candidates[:10]:
            ...
            if total_score > best_score or (
                ...
                best = {
                    "url": str(resp.url),
                    "method": disc.get("method", "") + f"+subpage_v22:{reason}",
    ...
    async def _probe_ats_paths_v22(
        self,
        client: httpx.AsyncClient,
        domain: str,
        company_name: str,
        ...
        hosts = self._candidate_hosts_v22(domain, current_url)
        ...
        for host in hosts:
            ...
            generic_paths = (
                ...
                "/job-openings", "/job-openings/", "/vacancies", "/join-us",
                "/lowongan", "/kerjaya", "/ms/kerjaya", "/embed-jobs",
        ...
        for found in self._extract_script_urls_v22(current_url, html_body):
            weight = 4.5 if _LISTING_URL_HINT_PATTERN_V22.search(found) else 3.0
            candidate_urls.append((found, "script_url", weight))

        if not candidate_urls:
            return None

        return await self._probe_url_candidates_v22(
            client=client,
            current=current,
    ...
    async def _probe_script_and_link_targets_v22(
        self,
        client: httpx.AsyncClient,
        current: dict,
    ) -> Optional[dict]:
        ...
        for u in self._extract_script_urls_v22(current_url, html_body):
            weight = 4.5 if _LISTING_URL_HINT_PATTERN_V22.search(u) else 3.0
        ...
        if root is not None:
            for a_el in root.iter("a"):
                ...
                if not self._is_related_host(current_url, full):
                    continue
                if not _LISTING_URL_HINT_PATTERN_V22.search(full):
                    continue
                weight = 3.5
        ...
        if not candidates:
            return None
        return await self._probe_url_candidates_v22(client, current, candidates, limit=20)

    async def _probe_domain_paths_v22(
        self,
        client: httpx.AsyncClient,
        domain: str,
        company_name: str,
        ...
        hosts = self._candidate_hosts_v22(domain, current.get("url") or f"https://{domain}")
        ...
        if not candidates:
            return None
        return await self._probe_url_candidates_v22(client, current, candidates, limit=36)

    async def _probe_url_candidates_v22(
        self,
        client: httpx.AsyncClient,
        current: dict,
        candidates: list[tuple[str, str, float]],
        ...
        for candidate_url, reason, weight in deduped[:limit]:
            ...
            if total_score > best_score or (
                _LISTING_URL_HINT_PATTERN_V22.search(candidate_url) and total_score >= best_score - 0.5
            ):
                best_score = total_score
                best = {
                    "url": str(resp.url),
                    "method": current.get("method", "") + f"+probe_v22:{reason}",
    ...
    def _candidate_hosts_v22(self, domain: str, current_url: str) -> list[str]:
        hosts: list[str] = []
        seen: set[str] = set()
    ...
    def _extract_script_urls_v22(self, current_url: str, html_body: str) -> list[str]:
        found: list[str] = []
        seen: set[str] = set()
        ...
        for m in _BASE_HREF_SITE_PATTERN_V22.finditer(html_body or ""):
            _add(m.group(1))
            _add(urljoin(m.group(1), "requisitions"))
        for m in _CANDIDATE_PORTAL_BASE_PATTERN_V22.finditer(html_body or ""):
            _add(m.group(1))
    ...
    async def _homepage_recovery_v22(
        self,
        client: httpx.AsyncClient,
        domain: str,
    ) -> Optional[dict]:
        best: Optional[dict] = None
        best_score = -10_000
        for host in self._candidate_hosts_v22(domain, f"https://{domain}"):
            ...
            home_disc = {
                "url": str(resp.url),
                "method": "homepage_recovery_v22",
                "candidates": [str(resp.url)],
                "html": body,
            }
            sub = await self._try_subpage_discovery_v22(client, home_disc)
    ...
    def _listing_page_score(self, url: str, html: str) -> int:
        ...
        if _ERROR_PAGE_PATTERN_V22.search(lower[:10000]):
            score -= 20
        if _LOGIN_PAGE_PATTERN_V22.search((url or "").lower()) or _LOGIN_PAGE_PATTERN_V22.search(lower[:15000]):
            score -= 22
        if _LISTING_URL_HINT_PATTERN_V22.search(url or ""):
            score += 4
        if _JOB_DETAIL_URL_PATTERN_V22.search(url or ""):
            score -= 6
        ...
        if "__next_data__" in lower and '<div id="__next"></div>' in lower:
            score += 2
        if _BASE_HREF_SITE_PATTERN_V22.search(html or ""):
            score += 4
        ...
        for a_el in root.iter("a"):
            ...
            if _CAREER_HREF_PATTERN_V22.search(href) or _CAREER_TEXT_PATTERN_V22.search(txt):
                career_links += 1
            if re.search(r"/jobs/search|/embed-jobs|/requisitions|/candidateportal", href, re.IGNORECASE):
                strong_listing_links += 1

            if _JOB_DETAIL_URL_PATTERN_V22.search(href):
                detail_links += 1

            if 6 <= len(txt) <= 140 and _LISTING_URL_HINT_PATTERN_V22.search(href):
                jobish_links += 1
        ...
        for el in root.iter():
            ...
            if not cls:
                continue
            if _ROW_CLASS_PATTERN_V22.search(cls):
    ...
    def _is_bad_target(self, url: str, html: str) -> bool:
        ...
        if _ERROR_PAGE_PATTERN_V22.search(lower):
            return True
        if _LOGIN_PAGE_PATTERN_V22.search((url or "").lower()) and self._listing_page_score(url, html) < 12:
            return True
        if _LOGIN_PAGE_PATTERN_V22.search(lower) and self._listing_page_score(url, html) < 12:
            return True
        if _JOB_DETAIL_URL_PATTERN_V22.search(url or "") and self._listing_page_score(url, html) < 8:
            return True
        return False
    ...
    @staticmethod
    def _is_rejected_link_v22(href: str, text: str) -> bool:
        href_l = (href or "").lower()
        text_l = (text or "").lower()

        if _REJECT_LINK_HREF_PATTERN_V22.search(href_l):
            return True
        if _REJECT_LINK_TEXT_PATTERN_V22.search(text_l):
            return True
        return False
