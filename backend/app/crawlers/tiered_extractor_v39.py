"""
Tiered Extraction Engine v3.9 — direct from v1.6.

High-impact improvements:
1. Keeps v3.8 quality gates and container scoring.
2. Adds Salesforce table-row fallback (`tr.dataRow` + `fRecruit__ApplyJob` links).
3. Restores Oracle CandidateExperience API fallback with tenant-aware `siteNumber` probing.
4. Adds lightweight Martian/MyRecruitmentPlus app-shell API fallback for `__NEXT_DATA__` pages.
5. Expands short-page recovery probes for `/careers` and Oracle requisitions endpoints.
"""
...
class TieredExtractorV39(TieredExtractorV16):
    """v3.9 extractor with API fallbacks for Oracle, Martian, and Salesforce rows."""
    ...
    async def extract(self, career_page, company, html: str) -> list[dict]:
        ...
        try:
            ...
            logger.warning("v3.9 parent extractor timeout for %s", page_url)
        except Exception:
            logger.exception("v3.9 parent extractor failed for %s", page_url)
        ...
        if best_label != "parent_v16" and any(
            ...
            try:
                ...
                logger.warning("v3.9 enrichment timeout for %s", page_url)
            except Exception:
                logger.exception("v3.9 enrichment failed for %s", page_url)
    ...
    async def _extract_oracle_jobs_v39(self, page_url: str, html_body: str) -> list[dict]:
        ...
        try:
            ...
            logger.debug("v3.9 oracle probing failed for %s", page_url)
        ...
        try:
            async with httpx.AsyncClient(
                ...
                for endpoint in endpoints[:30]:
                    ...
                    extracted = self._extract_jobs_from_json_payload_v39(payload, str(resp.url))
                    if extracted:
                        jobs.extend(extracted)
                        if len(jobs) >= MAX_JOBS_PER_PAGE:
                            break
        except Exception:
            logger.debug("v3.9 martian probing failed for %s", page_url)
    ...
    def _next_data_probe_urls_v39(self, host: str, page_url: str, context: dict[str, str]) -> list[str]:
        ...
        if context.get("client_code"):
            query_pairs.setdefault("client", context.get("client_code", ""))
    ...
    def _extract_jobs_from_json_payload_v39(self, payload: Any, page_url: str) -> list[dict]:
        ...
        for row in self._iter_json_dicts_v39(payload):
            ...
            if req_id and source_url.rstrip("/") == page_url.rstrip("/"):
                source_url = f"{page_url.rstrip('/')}/jobdetails?jobAdId={req_id}"
            if self._is_non_job_url_v38(source_url):
                continue
            ...
            jobs.append(
                {
                    ...
                    "description": description,
                    "extraction_method": "tier0_json_api_v39",
