"""
Tiered Extraction Engine v2.9 — direct from v1.6 with high-impact recovery.

High-impact changes:
1. Fix over-aggressive title rejection that dropped valid roles like "Digital & Design Intern".
2. Expand MartianLogic/MyRecruitmentPlus probing using Next.js board metadata (name/theme/recruiter).
3. Add Tailwind-style card-row extraction (`div.p-4` + title/apply structure) for API-backed listing HTML.
4. Allow short/acronym titles only with strong job-detail URL + apply-context evidence.
5. Improve detail enrichment by removing "How to Apply"/"Cara Melamar" tails from descriptions.
6. Keep v1.6 as the primary path and override only when fallback quality is clearly better.
"""
...
_MARTIAN_ENDPOINT_URL_PATTERN_V28 = re.compile(
    ...
    r"api/jobs/search|api/jobads/search|api/job-ads/search|"
    r"pageNumber=\d+[^\"'\\s]*)",
    re.IGNORECASE,
)
...
class TieredExtractorV29(TieredExtractorV16):
    """v2.9 extractor: v1.6-first with guarded structured/link/accordion fallbacks."""
    ...
    async def extract(self, career_page, company, html: str) -> list[dict]:
        ...
        try:
            ...
            logger.warning("v2.9 parent extractor timeout for %s", url)
        except Exception:
            logger.exception("v2.9 parent extractor failed for %s", url)
        ...
        if root is not None and (
            ...
            if cta_jobs and (
                ...
                best_label = "listing_cta_follow_v29"
                best_jobs = cta_jobs
        ...
        if (
            ...
            try:
                ...
                logger.warning("v2.9 enrichment timeout for %s", url)
            except Exception:
                logger.exception("v2.9 enrichment failed for %s", url)
            ...
            try:
                ...
                logger.debug("v2.9 selective enrichment skipped for %s", url)
            best_jobs = self._dedupe_jobs_v28(best_jobs, url)
    ...
    async def _extract_next_data_api_jobs_v28(self, page_url: str, html_body: str) -> list[dict]:
        ...
        try:
            ...
            logger.debug("v2.9 next-data probing failed for %s", page_url)
    ...
    async def _follow_listing_cta_pages_v28(self, page_url: str, root: etree._Element) -> list[dict]:
        ...
        try:
            ...
            logger.debug("v2.9 listing-cta follow failed for %s", page_url)
    ...
    async def _extract_candidate_jobs_from_html_v28(self, candidate_url: str, html_body: str) -> list[dict]:
        ...
        if root is not None:
            ...
            for job in tier2:
                cloned = dict(job)
                cloned["extraction_method"] = "tier2_heuristic_v29_cta"
            ...
            jobs.append(
                {
                    ...
                    "extraction_method": "tier2_links_v29",
            ...
            jobs.append(
                {
                    ...
                    "extraction_method": "tier2_accordion_v29",
                ...
                local_jobs.append(
                    {
                        ...
                        "extraction_method": "tier2_heading_rows_v29",
                ...
                jobs.append(
                    {
                        ...
                        "extraction_method": "tier2_repeating_rows_v29",
            ...
            jobs.append(
                {
                    ...
                    "extraction_method": "tier2_padded_rows_v29",
            ...
            jobs.append(
                {
                    ...
                    "extraction_method": "tier2_elementor_cards_v29",
        ...
        try:
            async with httpx.AsyncClient(
                ...
                for endpoint in endpoints[:24]:
                    pages = list(range(1, 9)) if ("pageNumber=" in endpoint or "page=" in endpoint) else [1]
            ...
            logger.exception("v2.9 MartianLogic fallback failed for %s", page_url)
    ...
    def _extract_jobs_from_probe_response_v28(self, body: str, response_url: str, page_url: str) -> list[dict]:
        ...
        if payload.startswith("{") or payload.startswith("["):
            try:
                ...
                jobs.extend(self._extract_jobs_from_json_obj_v28(parsed, response_url, "tier0_martian_api_v29"))
            except Exception:
                pass
        ...
        if root is not None:
            ...
            for job in tier2_jobs:
                cloned = dict(job)
                cloned["extraction_method"] = "tier2_heuristic_v29_martian"
                jobs.append(cloned)
    ...
    async def _extract_oracle_api_jobs_v28(self, page_url: str, html_body: str) -> list[dict]:
        ...
        try:
            ...
            logger.exception("v2.9 Oracle API fallback failed for %s", page_url)
    ...
    def _pick_best_jobset_v28(
        ...
        for label, jobs in candidates:
            ...
            logger.debug(
                "v2.9 candidate %s: jobs=%d score=%.2f valid=%s",
                label,
                len(deduped),
                score,
                valid,
